...
[antlr-report.yml](#antlr-report-workflow---antlr-reportyml)
...
## ANTLR Report workflow - antlr-report.yml

This workflow allows user/contributor to trigger ANTLR regression report (AST diff)
for grammar changes that they have in Pull Request(PR).

Workflow does:

- builds checkstyle from master branch,
- generates AST (abstract syntax tree) for every source file of the selected project,
- stores results of generation
- builds checkstyle from PR branch,
- generates AST for the same source files,
- stores results of generation
- compares AST from both execution, generate html report
- share html report link back to PR

Report is generated by [launch_diff_antlr.sh][launch-diff-antlr] script from
[contribution repository][contribution-tester].

There are few way to trigger ANTLR report.

### ANTLR Report over Checkstyle sources (self-test)
...
Workflow will generate AST diff over sources of checkstyle repository itself
(sources of PR branch are used).

### ANTLR Report over sources of a project

Make a comment in PR:

```txt
GitHub, generate ANTLR report for {{project_name}}
```

Workflow will take the project definition from
[config/projects-to-test/*-projects-to-test-on.config][projects-to-test] and
generate AST diff over sources of that project.
Example: `GitHub, generate ANTLR report for openjdk25`.

If project name is unknown, workflow fails and list of valid project names is printed
in the log of `Generate ANTLR report` step.

[launch-diff-antlr]:
https://github.com/checkstyle/contribution/blob/master/checkstyle-tester/launch_diff_antlr.sh
[contribution-tester]:
https://github.com/checkstyle/contribution/tree/master/checkstyle-tester
[projects-to-test]:
https://github.com/checkstyle/checkstyle/tree/master/config/projects-to-test
