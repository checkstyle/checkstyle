...
Notes:
 ...
 - When we visit a parser rule context, we have access to all of the parse tree elements that
 ...
 - In many cases, we can consider the ASTs yielded by visiting the subrule of a given rule to be
   the children of the AST node for that rule. This is a common pattern in the JavaAstVisitor.
