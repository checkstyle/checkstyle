...
window.addEventListener("load", function () {
  ...
  function trimLeadingWhitespace(node) {
    if (node.nodeType === Node.TEXT_NODE) {
      // Remove all whitespace at the beginning of the text node
      node.textContent = node.textContent.replace(/^\s+/, '');
    } else if (node.nodeType === Node.ELEMENT_NODE) {
      // Remove any leading child nodes that are empty (whitespace only)
      while (
        node.firstChild &&
      ...
      // Recursively trim the first child, if it exists
      if (node.firstChild) {
