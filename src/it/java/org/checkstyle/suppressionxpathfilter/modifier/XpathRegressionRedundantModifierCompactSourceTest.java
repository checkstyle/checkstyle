...
public class XpathRegressionRedundantModifierCompactSourceTest
    ...
    public void testPublicMethod() throws Exception {
        final File fileToProcess = new File(
                getNonCompilablePath("InputXpathRedundantModifierCompactSourcePublicMethod.java"));
        final DefaultConfiguration moduleConfig = createModuleConfig(CLAZZ);
        final String[] expected = {
            "2:1: " + getCheckMessage(CLAZZ,
                    RedundantModifierCompactSourceCheck.MSG_KEY, "public"),
        };
        final String methodXpath = "/COMPACT_COMPILATION_UNIT/METHOD_DEF"
                + "[./IDENT[@text='publicMethod']]";
        final List<String> expectedXpathQueries = Arrays.asList(
                "/COMPACT_COMPILATION_UNIT",
                methodXpath,
                methodXpath + "/MODIFIERS",
                methodXpath + "/MODIFIERS/LITERAL_PUBLIC");
        runVerifications(moduleConfig, fileToProcess, expected, expectedXpathQueries);
    }
    ...
    public void testProtectedMethod() throws Exception {
        final File fileToProcess = new File(
                getNonCompilablePath(
                        "InputXpathRedundantModifierCompactSourceProtectedMethod.java"));
        final DefaultConfiguration moduleConfig = createModuleConfig(CLAZZ);
        final String[] expected = {
            "2:1: " + getCheckMessage(CLAZZ,
                    RedundantModifierCompactSourceCheck.MSG_KEY, "protected"),
        };
        final String methodXpath = "/COMPACT_COMPILATION_UNIT/METHOD_DEF"
                + "[./IDENT[@text='protectedMethod']]";
        final List<String> expectedXpathQueries = Arrays.asList(
                "/COMPACT_COMPILATION_UNIT",
                methodXpath,
                methodXpath + "/MODIFIERS",
                methodXpath + "/MODIFIERS/LITERAL_PROTECTED");
        runVerifications(moduleConfig, fileToProcess, expected, expectedXpathQueries);
    }
