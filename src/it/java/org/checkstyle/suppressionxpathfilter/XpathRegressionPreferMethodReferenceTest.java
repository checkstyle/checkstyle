...
public class XpathRegressionPreferMethodReferenceTest
    ...
    public void testExpressions() throws Exception {
        final File fileToProcess = new File(
                getPath("SuppressionXpathRegressionPreferMethodReferenceExpression.java"));
        final DefaultConfiguration moduleConfig = createModuleConfig(clazz);
        moduleConfig.addAttribute("detectForExpression", "true");
        final String[] expectedViolation = {
            "6:38: " + getCheckMessage(clazz, PreferMethodReferenceCheck.MSG_METHOD_REF),
        };
        final List<String> expectedXpathQueries = Collections.singletonList(
            "/CLASS_DEF[./IDENT[@text='SuppressionXpathRegressionPreferMethodReferenceExpression']]"
                + "/OBJBLOCK/VARIABLE_DEF[./IDENT[@text='lambda']]"
                + "/ASSIGN/LAMBDA[./IDENT[@text='arg']]"
        );
        runVerifications(moduleConfig, fileToProcess, expectedViolation, expectedXpathQueries);
    }
}
