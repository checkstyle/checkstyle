...
class InputNoGetMessageInThrowCoverage {
    ...
    void throwMethodCallWithGetMessage() throws Exception {
        // ok - no new expression, thrown type unknown
        try {
            throw new IOException();
        }
        catch (IOException ex) {
            throw create(ex.getMessage());
        }
    }

    private static RuntimeException create(String msg) {
