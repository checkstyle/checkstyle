...
public enum InputIndentationLambdaInEnum { //indent:0 exp:0
	...
	GET(-1, false, 1, m -> m.isEmpty() //indent:8 exp:8
		&& (m.getClass().getSimpleName().startsWith("get"))), //indent:16 exp:16
	/** //indent:8 exp:8
	 * Setter //indent:9 exp:9
	 * void set(T) //indent:9 exp:9
	 */ //indent:9 exp:9
	SET(0, false, 1, m -> m.size() == 1), //indent:8 exp:8
	...
	if (m.size() == 1) { //indent:8 exp:8,12
	    if (m.getClass().getSimpleName().startsWith("add")) { //indent:12 exp:12
	        return m.getClass().getSimpleName().endsWith("AtTop"); //indent:16 exp:16
	    } //indent:12 exp:12
	} //indent:8 exp:8
	...
	ADD_X(0, true, 10, m -> { //indent:8 exp:8
	    if (m.size() == 1) { //indent:12 exp:8,12
	...
	ADD_Y(0, true, 1, m -> {  //indent:8 exp:8
			if (m.size() == 1) { //indent:24 exp:8,12 warn
				return m.getClass().getSimpleName().startsWith("add"); //indent:32 exp:12,16 warn
	...
	ADD_LAST(0, true, 1, m ->    {  //indent:8 exp:8
	    if (m.size() == 1) { //indent:12 exp:8,12
			return m.getClass().getSimpleName().startsWith("add"); //indent:24 exp:12,16 warn
	    } //indent:12 exp:12
	    return false; //indent:12 exp:12
	}); //indent:8 exp:8
    ...
    InputIndentationLambdaInEnum(int valueParameterIndex, boolean multi, //indent:4 exp:4
                                 int level, Predicate<List<?>> detector) { //indent:33 exp:33
