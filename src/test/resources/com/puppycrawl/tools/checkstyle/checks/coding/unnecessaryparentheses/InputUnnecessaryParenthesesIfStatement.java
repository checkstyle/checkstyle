...
public class InputUnnecessaryParenthesesIfStatement {
    ...
    void method(String sectionName) {
        if ("Content".equals(sectionName) || "Overview".equals(sectionName)
                || (!"AbbreviationAsWordInName".equals(sectionName) // violation '.* expression'
                                                                    // parenthesis
    ...
    private void method() {
        ...
        if ((Integer.valueOf(x) instanceof Integer) // violation '.* around expression'
                || Integer.valueOf(y) instanceof Integer) { // ok
            return;
        }
        if (x == ((y<z) ? y : z) &&
            ((x>y && y>z)                  // violation '.* parentheses around expression'
                    || (!(x<z) && y>z))) { // violation '.* parentheses around expression'
            return;
        }
