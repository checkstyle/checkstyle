...
public class InputUnnecessaryParenthesesOperatorsAndCasts {
    int f1() {
        ...
        for (int i = (0+1); ((i) < (6+6)); i += (1+0)) { // 4 violations
            x += (i + 100); // violation '.* parentheses around assignment right-hand side'
            (x) += (i + 100/**comment test*/); // 2 violations
            x = (x + i + 100); // violation '.* parentheses around assignment right-hand side'
    ...
    private int f2(int arg1, double arg2) {
        ...
        x += (i + 100 + arg1); // violation '.* parentheses around assignment right-hand side'
        a = (a + b) * (c + d);
        b = ((((a + b) * (c + d)))); // violation '.* around assignment right-hand side'
    ...
    public static int f4(int z, int a) {
        ...
        r = (a > z) ? a : z;
        r = ((a > z) ? a : z); // violation '.* parentheses around assignment right-hand side'
        r = (a > z) ? a : (z + z);
    ...
    private void f6(TypeA a) {
        TypeB b = (TypeB) a;
        TypeC c = ((TypeC) a); // violation '.* parentheses around assignment right-hand side'
        int r = 12345;
        r <<= (3); // 2 violations
        TypeParameterized<String> d = ((TypeParameterized<String>) a); // violation '.*right-h.*'
    }
