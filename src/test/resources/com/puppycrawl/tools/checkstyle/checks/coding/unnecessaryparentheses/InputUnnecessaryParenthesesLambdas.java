...
public class InputUnnecessaryParenthesesLambdas {
    int foo(int y) {
        MathOperation case1 = (x) -> x + x; // violation '.* parentheses around lambda value'
        MathOperation case2 = (x) -> { return x + x; }; // violation '.* around lambda value'
        ...
        call((x) -> x + x); // violation '.* parentheses around lambda value'
        new HashSet<Integer>().stream().filter((filter) -> filter > 0); // violation '.* lambda .*'
        return y;
    }
