...
public class InputWhenShouldBeUsedSwitchExpression {
    ...
    void test(Object j, int x) {
        int a = switch (j) {
            case Point(int _, int y): // violation, 'When should be used instead of a single if*.'
                if (y == 0) {
                    yield 0;
                }
        ...
        int m = switch (j) {
            case Point _: // violation, 'When should be used instead of a single if*.'
            case ColoredPoint _: // violation, 'When should be used instead of a single if*.'
                if (x == 0) {
