...
public class InputWhenShouldBeUsedSwitchStatements {
    ...
    void test(Object j, int x) {
        switch (j) {
            ...
            default: {} // violation above, 'When should be used instead of a single if*.'
        }
        switch (j) {
            case String s: { // violation, 'When should be used instead of a single if*.'
                if (s.isEmpty()) {
        ...
        switch (j) {
            case Integer _: // violation, 'When should be used instead of a single if*.'
            case String _: // violation, 'When should be used instead of a single if*.'
                if (x == 0) {
