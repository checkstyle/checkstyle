//non-compiled with javac: Compilable with Java14
package com.puppycrawl.tools.checkstyle.checks.naming.recordtypeparametername;

import java.io.Serializable;
import java.util.LinkedHashMap;

import org.w3c.dom.Node;

/* Config:
 * pattern = "^foo$"
 *
 */
public record InputRecordTypeParameterNameFoo<notfoo>(Integer x, String str) { // violation
    public <TT> void foo() { }

    <e_e> void foo(int i) {
    }
}

record Other <alsoNotfoo extends Serializable & Cloneable> // violation
        (LinkedHashMap<String, Node> linkedHashMap) {

    foo getOne() {
        return null;//comment
    }

    <Tfo$o2T extends foo> /*comment*/Tfo$o2T getTwo(Tfo$o2T a) {
        return null;
    }

    <foo extends Runnable> foo getShadow() { // ok
        return null;
    }

    static record Junk <foo>() { // ok
        <_fo extends foo> void getMoreFoo() {
        }
    }
}

record MoreOther <againNotfoo extends Cloneable>(char c, String string) { // violation

    interface Boo<Input> {
        Input boo();
    }

    interface FooInterface<T> {
        T foo();
    }

    interface FooInterface2 {
        Input foo();
    }

    record Input(int x, int y) {

    }
}
