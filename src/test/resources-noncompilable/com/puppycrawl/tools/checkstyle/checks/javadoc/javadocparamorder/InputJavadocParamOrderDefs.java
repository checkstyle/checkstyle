...
class InputJavadocParamOrderDefs<T, E> { // ok
     ...
     */
    public <K> int Test(int x, int y) { // ok
        return 9;
    }
    ...
    record Record1<E>(Integer z, Integer... numberList) implements Interface1<String> { // ok
         ...
         */
        @Override
        public String getUserId(String str){ // ok
            return "New";
        }
     ...
     * @return Description.
     */
    <W, V extends T> V convert(String message, Class<W> type) { // ok
        return null;
    }
    ...
    public interface VehicleJourney<
            ID,
            ID_JP, OP_JP extends Comparable<OP_JP>, JP extends Iterable<ID_JP, OP_JP>
        > { // ok
        JP Iterable();
    }
