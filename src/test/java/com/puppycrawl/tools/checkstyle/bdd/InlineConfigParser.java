...
public final class InlineConfigParser {
    ...
    public static InputConfiguration parse(String inputFilePath) throws Exception {
        ...
        setViolationLines(inputConfigBuilder, lines);
        setViolationMessages(inputConfigBuilder, lines);
    ...
    private static void setViolationLines(InputConfiguration.Builder inputConfigBuilder,
                                          List<String> lines) {
        for (int lineNo = 2; lineNo < lines.size(); lineNo++) {
            if (VIOLATION_PATTERN.matcher(lines.get(lineNo)).matches()) {
                inputConfigBuilder.addViolationLine(lineNo + 1);
            }
        }
    }

    private static void setViolationMessages(InputConfiguration.Builder inputConfigBuilder,
                                             List<String> lines) {
        for (int lineNo = 2; lineNo < lines.size(); lineNo++) {
            final Matcher violationMatcher = VIOLATION_PATTERN.matcher(lines.get(lineNo));
            if (violationMatcher.matches()) {
                inputConfigBuilder.addViolationMessage(violationMatcher.group(1));
            }
        }
    }
}
