...
public class PatternVariableAssignmentCheckTest extends AbstractModuleTestSupport {
    ...
    /**
     * Test that pattern variable scopes are properly cleaned up when leaving blocks.
     * After leaving the block where a pattern variable is defined, assignments
     * to same-named fields must not be flagged.
     */
    @Test
    public void testScopeCleanup() throws Exception {

        final String[] expected = {
            "23:17: " + getCheckMessage(MSG_KEY, "a"),
            "31:17: " + getCheckMessage(MSG_KEY, "number"),
            "41:17: " + getCheckMessage(MSG_KEY, "a"),
        };

        verifyWithInlineXmlConfig(getPath(
            "InputPatternVariableAssignmentScopeCleanup.java"), expected);
    }
