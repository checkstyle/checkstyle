...
public class OverloadMethodsDeclarationOrderCheckTest
    ...
    public void testOverloadMethodsDeclarationOrderPublicOrProtectedAndStatic() throws Exception {
        final String[] expected = {
            ...
            "30:5: " + getCheckMessage(MSG_KEY, 26),
        };
        verifyWithInlineConfigParser(
    ...
    @Test
    void testOverloadGroupEquals() {
        final Pattern pattern1 = Pattern.compile(".*");
        final OverloadMethodsDeclarationOrderCheck.OverloadDescriptor descriptor =
            new OverloadMethodsDeclarationOrderCheck.OverloadDescriptor(pattern1,
                                                                        "foo",
                                                                        null,
                                                                        1);
        assertWithMessage("Same object reference should be equal")
            .that(descriptor.equals(descriptor))
            .isTrue();

        assertWithMessage("Should not equal object of different type")
            .that(descriptor.equals(new Object()))
            .isFalse();

        assertWithMessage(
            "Overload descriptor should not equal another that is null"
        ).that(descriptor.equals(null))
         .isFalse();

        final OverloadMethodsDeclarationOrderCheck.OverloadDescriptor descriptor2 =
            new OverloadMethodsDeclarationOrderCheck.OverloadDescriptor(pattern1,
                                                                        "bar",
                                                                        null,
                                                                        1);
        ...
        assertWithMessage("Different method name should break equality")
            .that(descriptor.equals(descriptor2))
            .isFalse();

        final Pattern pattern2 = Pattern.compile("^public$");
        final OverloadMethodsDeclarationOrderCheck.OverloadDescriptor descriptor3 =
            new OverloadMethodsDeclarationOrderCheck.OverloadDescriptor(pattern2,
                                                                        "foo",
                                                                        null,
                                                                        1);

        assertWithMessage("Different pattern should break equality")
            .that(descriptor.equals(descriptor3))
            .isFalse();
    }
}
