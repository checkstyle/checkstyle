...
public class UnusedImportsCheckTest extends AbstractModuleTestSupport {
    ...
    public void testImportJavaLinkTagWithMethod() throws Exception {
        final String[] expected = {
            ...
            "13:8: " + getCheckMessage(MSG_KEY, "java.util.PriorityQueue"),
            "16:8: " + getCheckMessage(MSG_KEY, "java.util.Set"),
            "25:8: " + getCheckMessage(MSG_KEY, "java.util.concurrent.TimeUnit"),
        };
        verifyWithInlineConfigParser(
    ...
    public void testJavadocSetters() throws Exception {
        final String[] expected = {
            "10:8: " + getCheckMessage(MSG_KEY, "java.util.List"),
            "18: " + getCheckMessage(MSG_KEY_UNCLOSED_HTML_TAG, "p"),
        };
        verifyWithInlineConfigParser(
    ...
    /**
     * Verifies that the check fails on unsupported AST tokens.
     *
     * <p>This test does not use {@code verifyWithInlineConfigParser} because such
     * tokens are never dispatched to this check during normal Checkstyle execution
     * and must be tested via manual AST construction.</p>
     */
    @Test
    ...
    /**
     * Verifies that {@link UnusedImportsCheck#visitJavadocToken} safely handles
     * a {@code {@link ...}} Javadoc reference with an empty identifier.
     *
     * <p>This case cannot be reliably tested via {@code verifyWithInlineConfigParser},
     * because an empty {@code {@linkplain ...}} reference cannot be expressed in an
     * input file in a way that both reaches this code path and kills the corresponding
     * PIT mutation.</p>
     */
    @Test
    public void testIsValidIdentifier() {
        final UnusedImportsCheck check = new UnusedImportsCheck();
        final JavadocNodeImpl reference = new JavadocNodeImpl();
        reference.setType(JavadocCommentsTokenTypes.REFERENCE);

        final JavadocNodeImpl child = new JavadocNodeImpl();
        child.setType(JavadocCommentsTokenTypes.IDENTIFIER);
        child.setText("");
        reference.addChild(child);

        assertDoesNotThrow(
            () -> check.visitJavadocToken(reference),
            "visitJavadocToken must not throw for REFERENCE with empty identifier text"
        );
    }
    ...
    public void testCache() throws Exception {
        final String[] expected = {
            "10:8: " + getCheckMessage(MSG_KEY, "java.util.TreeSet"),
        };
        verifyWithInlineConfigParser(getPath("InputUnusedImportsCache1.java"),
            getPath("InputUnusedImportsCache2.java"), expected);
    }
