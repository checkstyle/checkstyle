...
public class JavadocRegexpCheckTest extends AbstractModuleTestSupport {
    ...
    public void testOrderOfProperties() {
        ...
        final Pattern actual = TestUtil.getInternalState(check, "regexp", Pattern.class);

        assertWithMessage("Invalid regexp flags")
            .that(actual.flags())
            .isEqualTo(Pattern.CASE_INSENSITIVE);
        assertWithMessage("Invalid regexp pattern")
            .that(actual.pattern())
            .isEqualTo("AKA");
    }
