...
public class UnusedPrivateFieldCheckTest extends AbstractModuleTestSupport {
    ...
    public void testIllegalStateException() {
        ...
        assertWithMessage("Expected error message for unsupported token type")
                .that(exception)
                .hasMessageThat()
                .contains("Unexpected token type");
    }

    @Test
    public void testFindEnclosingNonAnonClassWhenScopeIsEmpty() throws Exception {
        final UnusedPrivateFieldCheck check = new UnusedPrivateFieldCheck();
        final java.lang.reflect.Method method = UnusedPrivateFieldCheck.class
                .getDeclaredMethod("findEnclosingNonAnonClass");
        method.setAccessible(true);

        final Object result = method.invoke(check);

        assertWithMessage("Expected null when class scope stack is empty")
                .that(result)
                .isNull();
    }

    @Test
    public void testFindClassByNameFallbackToCurrentClass() throws Exception {
        final UnusedPrivateFieldCheck check = new UnusedPrivateFieldCheck();

        final DetailAstImpl classDefAst = new DetailAstImpl();
        classDefAst.setType(TokenTypes.CLASS_DEF);

        final DetailAstImpl identAst = new DetailAstImpl();
        identAst.setType(TokenTypes.IDENT);
        identAst.setText("CurrentClass");
        classDefAst.addChild(identAst);

        check.visitToken(classDefAst);

        final java.lang.reflect.Method method = UnusedPrivateFieldCheck.class
                .getDeclaredMethod("findClassByName", String.class);
        method.setAccessible(true);

        final Object result = method.invoke(check, "MissingClass");

        assertWithMessage("Expected fallback to current class when name is missing")
                .that(result)
                .isSameInstanceAs(classDefAst);
    }
    ...
    @Test
    public void testDotLhs() throws Exception {
        final String[] expected = {
            "10:9: " + getCheckMessage(MSG_UNUSED_PRIVATE_FIELD, "source"),
        };
        verifyWithInlineConfigParser(
                getPath("InputUnusedPrivateFieldDotLhs.java"), expected);
    }
    ...
    @Test
    public void testChainedDot() throws Exception {
        final String[] expected = {};
        verifyWithInlineConfigParser(
                getPath("InputUnusedPrivateFieldChainedDot.java"), expected);
    }
