...
public class EmptyStatementCheckTest
    ...
    public void testEnumConstantsTerminatorGuards() throws Exception {
        ...
        assertWithMessage("Empty statement without semicolon child is not enum terminator")
            .that(isEnumConstantsTerminator(emptyStatWithoutSemicolon))
            .isFalse();
        ...
        assertWithMessage("Semicolon without parent is not enum constants terminator")
            .that(isEnumConstantsTerminator(semicolonWithoutParent))
            .isFalse();
        ...
        assertWithMessage("Semicolon outside enum object block is not terminator")
            .that(isEnumConstantsTerminator(semicolonWithCompUnitParent))
            .isFalse();
        ...
        assertWithMessage("Semicolon in orphan object block is not enum constants terminator")
            .that(isEnumConstantsTerminator(semicolonWithOrphanObjBlock))
            .isFalse();
        ...
        assertWithMessage("Semicolon in class body is not enum constants terminator")
            .that(isEnumConstantsTerminator(nonEnumSemicolon))
            .isFalse();
    }
    ...
    public void testEnumConstantsTerminatorForConstantsList() throws Exception {
        ...
        assertWithMessage("Semicolon after enum constant should be constants terminator")
            .that(isEnumConstantsTerminator(semicolonAfterConstant))
            .isTrue();
        ...
        assertWithMessage("Semicolon after comma in enum constants list should be terminator")
            .that(isEnumConstantsTerminator(semicolonAfterComma))
            .isTrue();
    }
    ...
    public void testEnumConstantsTerminatorForNoConstantsCases() throws Exception {
        ...
        assertWithMessage(
            ...
            .that(isEnumConstantsTerminator(semicolonWithMembers))
            .isTrue();
        ...
        assertWithMessage("Standalone semicolon in empty enum should not be constants terminator")
            .that(isEnumConstantsTerminator(semicolonOnly))
            .isFalse();
    }
    ...
    public void testEnumConstantsTerminatorEdgeShapes() throws Exception {
        ...
        assertWithMessage("Semicolon with null previous sibling should not be enum terminator")
            .that(isEnumConstantsTerminator(leadingSemicolon))
            .isFalse();
        ...
        assertWithMessage("Semicolon without next sibling should not be enum terminator")
            .that(isEnumConstantsTerminator(trailingSemicolonOnly))
            .isFalse();
        ...
        assertWithMessage("Synthetic EMPTY_STAT semicolon child is not enum constants terminator")
            .that(isEnumConstantsTerminator(emptyStat))
            .isFalse();
    }

    @Test
    public void testGetSemicolonTokenWithSemiReturnsSameToken() throws Exception {
        final DetailAstImpl semicolon = createAst(TokenTypes.SEMI);

        assertWithMessage("SEMI input should return same token")
            .that(getSemicolonToken(semicolon))
            .isSameInstanceAs(semicolon);
    }

    @Test
    public void testGetSemicolonTokenWithEmptyStatReturnsSemicolonChild() throws Exception {
        final DetailAstImpl emptyStat = createAst(TokenTypes.EMPTY_STAT);
        final DetailAstImpl semicolonChild = createAst(TokenTypes.SEMI);
        emptyStat.addChild(semicolonChild);

        assertWithMessage("EMPTY_STAT input should resolve to semicolon child")
            .that(getSemicolonToken(emptyStat))
            .isSameInstanceAs(semicolonChild);
    }

    @Test
    public void testGetSemicolonTokenWithEmptyStatWithoutSemiReturnsNull() throws Exception {
        final DetailAstImpl emptyStat = createAst(TokenTypes.EMPTY_STAT);

        assertWithMessage("EMPTY_STAT without semicolon child should return null")
            .that(getSemicolonToken(emptyStat))
            .isNull();
    }

    @Test
    public void testEnumConstantsTerminatorWithEmptyStatInputIsNotTerminator() throws Exception {
        final DetailAstImpl enumObjectBlock = createEnumObjectBlock();
        final DetailAstImpl leftCurly = createAst(TokenTypes.LCURLY);
        final DetailAstImpl emptyStat = createAst(TokenTypes.EMPTY_STAT);
        final DetailAstImpl semicolonChild = createAst(TokenTypes.SEMI);
        final DetailAstImpl fieldAfterSemicolon = createAst(TokenTypes.VARIABLE_DEF);
        emptyStat.addChild(semicolonChild);
        enumObjectBlock.addChild(leftCurly);
        enumObjectBlock.addChild(emptyStat);
        enumObjectBlock.addChild(fieldAfterSemicolon);

        assertWithMessage("EMPTY_STAT input should not be treated as enum constants terminator")
            .that(isEnumConstantsTerminator(emptyStat))
            .isFalse();
    }

    @Test
    public void testIsNoConstantsWithMembersRejectsNonLeftCurlyPreviousSibling() throws Exception {
        final DetailAstImpl previousSibling = createAst(TokenTypes.ENUM_CONSTANT_DEF);
        final DetailAstImpl nextSibling = createAst(TokenTypes.VARIABLE_DEF);

        assertWithMessage("Only LCURLY previous sibling can indicate no-constants section")
            .that(isNoConstantsWithMembers(previousSibling, nextSibling))
            .isFalse();
    }

    @Test
    public void testIsSemicolonInEnumObjectBlockRejectsNullInput() throws Exception {
        assertWithMessage("Null semicolon should not be in enum object block")
            .that(isSemicolonInEnumObjectBlock(null))
            .isFalse();
    }

    @Test
    public void testIsSemicolonInEnumObjectBlockRejectsNonObjectBlockParent() throws Exception {
        final DetailAstImpl semicolon = createAst(TokenTypes.SEMI);
        final DetailAstImpl nonObjectBlockParent = createAst(TokenTypes.CLASS_DEF);
        final DetailAstImpl enumDefGrandParent = createAst(TokenTypes.ENUM_DEF);
        enumDefGrandParent.addChild(nonObjectBlockParent);
        nonObjectBlockParent.addChild(semicolon);

        assertWithMessage("Semicolon parent must be OBJBLOCK to be inside enum object block")
            .that(isSemicolonInEnumObjectBlock(semicolon))
            .isFalse();
    }
    ...
    private static boolean isEnumConstantsTerminator(DetailAST ast) throws Exception {
    ...
    private static DetailAST getSemicolonToken(DetailAST ast) throws Exception {
        return TestUtil.invokeStaticMethod(
            EmptyStatementCheck.class,
            "getSemicolonToken",
            DetailAST.class,
    ...
    private static boolean isNoConstantsWithMembers(DetailAST previousSibling,
                                                     DetailAST nextSibling) throws Exception {
        return TestUtil.invokeStaticMethod(
            EmptyStatementCheck.class,
            "isNoConstantsWithMembers",
            Boolean.class,
            previousSibling,
            nextSibling);
    }

    private static boolean isSemicolonInEnumObjectBlock(DetailAST semicolon) throws Exception {
        return TestUtil.invokeStaticMethod(
            EmptyStatementCheck.class,
            "isSemicolonInEnumObjectBlock",
            Boolean.class,
            semicolon);
    }
