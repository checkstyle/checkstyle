...
public class EmptyStatementCheckTest
    ...
    public void testSemicolonInCompilationUnitVisitToken() throws Exception {
        ...
        final DetailAstImpl compilationUnit = createAst(TokenTypes.COMPILATION_UNIT);
        final DetailAstImpl semicolon = createAst(TokenTypes.SEMI);
        compilationUnit.addChild(semicolon);

        assertWithMessage("Expected SEMI token")
            .that(semicolon.getType())
            .isEqualTo(TokenTypes.SEMI);
    ...
    public void testSemicolonOutsideCompilationUnitVisitToken() throws Exception {
        final EmptyStatementCheck check = newCheckWithFileContents();
        final DetailAstImpl methodDef = createAst(TokenTypes.METHOD_DEF);
        final DetailAstImpl semicolon = createAst(TokenTypes.SEMI);
        methodDef.addChild(semicolon);

        assertWithMessage("Expected method to be parent")
            .that(semicolon.getParent().getType())
    ...
    public void testStandaloneSemicolonInTypeBodyVisitToken() throws Exception {
        ...
        final DetailAstImpl objBlock = createAst(TokenTypes.OBJBLOCK);
        classDef.addChild(objBlock);
        final DetailAstImpl semicolon = createAst(TokenTypes.SEMI);
        objBlock.addChild(semicolon);

        assertWithMessage("Expected SEMI in class body")
            .that(semicolon.getParent().getType())
    ...
    public void testEnumSemicolonInTypeBodyVisitToken() throws Exception {
        ...
        assertWithMessage("Expected SEMI in enum body")
            .that(semicolon.getParent().getType())
            .isEqualTo(TokenTypes.OBJBLOCK);
        check.visitToken(semicolon);
    }
    ...
    public void testSemicolonAfterTypeMemberVisitToken() throws Exception {
        ...
        assertWithMessage("Expected previous sibling to be method")
            .that(semicolon.getPreviousSibling().getType())
            .isEqualTo(TokenTypes.METHOD_DEF);
        check.visitToken(semicolon);
    }
    ...
    public void testEmptyStatUnderMethodDefVisitToken() throws Exception {
        ...
        assertWithMessage("Expected EMPTY_STAT token")
            .that(emptyStat.getType())
            .isEqualTo(TokenTypes.EMPTY_STAT);
    ...
    public void testEmptyStatUnderStatementListVisitToken() throws Exception {
        final EmptyStatementCheck check = newCheckWithFileContents();
        final DetailAstImpl slist = createAst(TokenTypes.SLIST);
        final DetailAstImpl emptyStat = createAst(TokenTypes.EMPTY_STAT);
        slist.addChild(emptyStat);

        assertWithMessage("Expected SLIST parent")
            .that(emptyStat.getParent().getType())
            .isEqualTo(TokenTypes.SLIST);
        check.visitToken(emptyStat);
    }
