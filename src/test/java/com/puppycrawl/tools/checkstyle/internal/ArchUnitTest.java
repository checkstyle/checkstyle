...
public class ArchUnitTest {
    ...
    public void nonProtectedCheckMethodsTest() {
        // This list contains methods which have been overriden and are set to ignore in this test.
        ...
        final StringBuilder ignoreMethodList = new StringBuilder(".*(");
        for (int i = 0; i < methodsWithOverrideAnnotation.length; i++) {
            if (i == 0) {
                ignoreMethodList.append(methodsWithOverrideAnnotation[0]);
            }
            else {
                ignoreMethodList.append('|').append(methodsWithOverrideAnnotation[i]);
            }
        }
        ignoreMethodList.append(')');
        ...
        final ArchRule checkMethodsShouldNotBeProtectedRule =
                methods().that()
                .haveNameNotMatching(String.valueOf(ignoreMethodList)).and()
