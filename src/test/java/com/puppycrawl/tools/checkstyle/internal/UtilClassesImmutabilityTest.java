...
public class UtilClassesImmutabilityTest {
    ...
    /**
     * Thread safe primitive types. {@code long} and {@code double} aren't thread safe unless
     * used with volatile keyword.
     */
    ...
    private static final ArchCondition<JavaField> BE_IMMUTABLE = new ArchCondition<>(
        ...
        public void check(JavaField item, ConditionEvents events) {
            ...
            else if ("long".equals(rawTypeName) || "double".equals(rawTypeName)) {
                addViolation = item.getModifiers().contains(JavaModifier.VOLATILE);
            }
