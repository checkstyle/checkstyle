...
public class ImmutabilityTest {
    ...
    public void testUtilClassesImmutability() {
        ...
        final ArchCondition<JavaField> beSuppressed = new JavaFieldSuppressionArchCondition(
    ...
    public void testModuleImmutability() {
        ...
        final ArchCondition<JavaClass> beAddedToSuppressionList =
            new JavaClassSuppressionArchCondition(
                SUPPRESSED_CLASSES, "be added to the suppression list");
        ...
        final ArchRule classesWithImmutableFieldsShouldBeStateless = classes()
            ...
            .orShould(beAddedToSuppressionList);
    ...
    /**
     * ArchCondition checking if a class is present in the suppression list.
     */
    private static final class JavaClassSuppressionArchCondition extends ArchCondition<JavaClass> {

        private final Set<String> suppressedClasses;

        private JavaClassSuppressionArchCondition(Set<String> suppressedClasses,
                                                  String description) {
            super(description);
            this.suppressedClasses = suppressedClasses;
        }

        @Override
        public void check(JavaClass item, ConditionEvents events) {
            if (!suppressedClasses.contains(item.getFullName())) {
    ...
    /**
     * ArchCondition checking if a field is present in the suppression list.
     */
    private static final class JavaFieldSuppressionArchCondition extends ArchCondition<JavaField> {

        private final Set<String> suppressedFields;

        private JavaFieldSuppressionArchCondition(Set<String> suppressedFields,
                                                  String description) {
            super(description);
        ...
        @Override
        public void check(JavaField item, ConditionEvents events) {
            if (!suppressedFields.contains(item.getFullName())) {
                final String message = String.format(
                    Locale.ROOT, "should %s or resolved.", getDescription());
                events.add(SimpleConditionEvent.violated(item, message));
            }
