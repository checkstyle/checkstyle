...
public final class TestUtil {
    ...
    public static Field getClassDeclaredField(Class<?> clss, String fieldName) {
        return Arrays.stream(clss.getDeclaredFields())
            ...
            .orElseGet(() -> getClassDeclaredField(clss.getSuperclass(), fieldName));
    }
     ...
     * @param parameters the expected number of parameters or {@code -1} for any
     * @return the class' method
     */
    public static Method getClassDeclaredMethod(Class<?> clss, String methodName, int parameters) {
        return Arrays.stream(clss.getDeclaredMethods())
            .filter(method -> {
            ...
            .orElseGet(() -> getClassDeclaredMethod(clss.getSuperclass(), methodName, parameters));
    }
     ...
     * @throws RuntimeException if the field  can't be read
     */
    public static <T> T getInternalState(Class<?> clss, String fieldName) {
        try {
     ...
     * @throws Exception if the method failed
     */
    public static <T> T invokeMethod(Class<?> clss,
            String methodToExecute, Object... arguments) throws Exception {
        final Method method = getClassDeclaredMethod(clss, methodToExecute, arguments.length);
        try {
