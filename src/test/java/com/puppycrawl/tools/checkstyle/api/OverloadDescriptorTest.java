...
public class OverloadDescriptorTest {
    ...
    public void testConstructor() {
        try {
            createDescriptor(null, "foo", new DetailAstImpl(), 1);
        ...
        try {
            createDescriptor(Pattern.compile(".*"), null,
                             new DetailAstImpl(), 2);
            assertWithMessage("exception is expected").fail();
        }
        // -@cs[IllegalCatch] Required for 100% test coverage.
    ...
    private static OverloadDescriptor createDescriptor(Pattern pattern, String methodName,
                                                DetailAST ast, int index) {
        return new OverloadDescriptor(pattern, methodName, ast, index);
    }

    @Test
    public void testGetIndex() {
        final OverloadDescriptor descriptor =
            createDescriptor(Pattern.compile(".*"), "foo",
                             new DetailAstImpl(), 1);
