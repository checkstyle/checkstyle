...
public class SuppressionFileGeneratorAuditListenerTest {
    ...
    public void testFileStarted() {
        ...
        final SuppressionFileGeneratorAuditListener listener =
                new SuppressionFileGeneratorAuditListener(out, OutputStreamOptions.CLOSE);
    ...
    /**
     * Verifies the addException method to throw the
     * "UnsupportedOperationException" exception.
     */
    @Test
    public void testAddException() {
        final OutputStream out = new ByteArrayOutputStream();
        final SuppressionFileGeneratorAuditListener logger =
                new SuppressionFileGeneratorAuditListener(out, OutputStreamOptions.CLOSE);
    ...
    /**
     * Verifies the close count through "CloseAndFlushTestByteArrayOutputStream"
     * which is not supported by OutputStreams used in "Main.java".
     */
    @Test
    public void testCloseStream() {
        final SuppressionFileGeneratorAuditListener listener =
                new SuppressionFileGeneratorAuditListener(outStream, OutputStreamOptions.CLOSE);
    ...
    public void testNoCloseStream() {
        final SuppressionFileGeneratorAuditListener listener =
                new SuppressionFileGeneratorAuditListener(outStream, OutputStreamOptions.NONE);
        listener.finishLocalSetup();
        listener.auditStarted(null);
        listener.auditFinished(null);
    ...
    public void testCorrectOne() {
        final AuditEvent event = createAuditEvent(
                "InputSuppressionFileGeneratorAuditListener.java", FIRST_MESSAGE);

        final String expected = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>" + EOL
                + "<!DOCTYPE suppressions PUBLIC" + EOL
                + "    \"-//Checkstyle//DTD Checkstyle Configuration 1.3"
                + "//EN\"" + EOL
                + "    \"https://checkstyle.org/dtds/configuration_1_3.dtd\">"
                + EOL
                + "<suppressions>" + EOL
                + "<suppress" + EOL
                + "       files=\"InputSuppressionFileGeneratorAuditListener.java\"" + EOL
    ...
    public void testCorrectTwo() {
        final AuditEvent event1 = createAuditEvent(
                "InputSuppressionFileGeneratorAuditListener.java",
                5, 5, JavadocVariableCheck.class);

        final AuditEvent event2 = createAuditEvent(
                "InputSuppressionFileGeneratorAuditListener.java", SECOND_MESSAGE);
    ...
    private static void verifyOutput(String expected, AuditEvent... events) {
        try (TestByteArrayOutputStream out = new TestByteArrayOutputStream()) {
            final SuppressionFileGeneratorAuditListener listener =
                new SuppressionFileGeneratorAuditListener(out, OutputStreamOptions.CLOSE);
