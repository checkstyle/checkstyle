...
public class ParseTreeTableModelTest extends AbstractPathTestSupport {
    ...
    public void testCommentChildCountInJavaAndJavadocMode() {
        final ParseTreeTableModel parseTree = new ParseTreeTableModel(null);
        parseTree.setParseMode(ParseMode.JAVA_WITH_JAVADOC_AND_COMMENTS);
        final DetailAST commentContentNode = classDef.getLastChild().getLastChild()
    ...
    public void testGetIndexOfChild() {
        DetailAST ithChild = classDef.getFirstChild();
        ...
        int index = 0;
        while (ithChild != null) {
            final int indexOfChild = parseTree.getIndexOfChild(classDef, ithChild);
     ...
     *  - Comment classDef
     *  - LITERAL_CLASS
     *  - IDENT -> this is the classDef that holds the class name
    ...
    public void testGetValueAt() {
        classDef = classDef.getLastChild().getPreviousSibling();
        assertWithMessage("Expected a non-null identifier classDef here")
            .that(classDef)
            .isNotNull();
        assertWithMessage("Expected identifier token")
            .that(classDef.getType())
            .isEqualTo(TokenTypes.IDENT);

        final ParseTreeTableModel parseTree = new ParseTreeTableModel(null);
        final Object treeModel = parseTree.getValueAt(classDef, 0);
        final String type = (String) parseTree.getValueAt(classDef, 1);
        final int line = (int) parseTree.getValueAt(classDef, 2);
        final int column = (int) parseTree.getValueAt(classDef, 3);
        final String text = (String) parseTree.getValueAt(classDef, 4);
        ...
        try {
            parseTree.getValueAt(classDef, parseTree.getColumnCount());
