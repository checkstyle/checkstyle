...
public class ParseTreeTableModelTest extends AbstractPathTestSupport {
    ...
    public void loadTree() throws Exception {
        node = JavaParser.parseFile(new File(getPath("InputParseTreeTablePresentation.java")),
            JavaParser.Options.WITH_COMMENTS).getFirstChild().getNextSibling();
    }

    @Test
    public void testChildCount() {
        final int childCount = new ParseTreeTableModel(null).getChildCount(node);
    ...
    public void testChildCountInJavaAndJavadocMode() {
        ...
        final int childCount = parseTree.getChildCount(node);
        assertWithMessage("Invalid child count")
            .that(childCount)
            .isEqualTo(5);
    }
    ...
    public void testChild() {
        final Object child = new ParseTreeTableModel(null).getChild(node, 1);
    ...
    public void testChildInJavaAndJavadocMode() {
        final ParseTreeTableModel parseTree = new ParseTreeTableModel(null);
        parseTree.setParseMode(ParseMode.JAVA_WITH_JAVADOC_AND_COMMENTS);
        final Object child = parseTree.getChild(node, 1);
        assertWithMessage("Invalid child type")
                .that(child)
                .isInstanceOf(DetailAST.class);
        final int type = ((DetailAST) child).getType();
    ...
    public void testCommentChildCount() {
        final DetailAST commentContentNode = node.getFirstChild().getNextSibling().getFirstChild();
        final ParseTreeTableModel parseTree = new ParseTreeTableModel(null);
    ...
    public void testCommentChildCountInJavaAndJavadocMode() {
        final ParseTreeTableModel parseTree = new ParseTreeTableModel(null);
        parseTree.setParseMode(ParseMode.JAVA_WITH_JAVADOC_AND_COMMENTS);
        final DetailAST commentContentNode = node.getLastChild().getLastChild()
    ...
    public void testGetIndexOfChild() {
        DetailAST ithChild = node.getFirstChild();
        ...
        int index = 0;
        while (ithChild != null) {
            final int indexOfChild = parseTree.getIndexOfChild(node, ithChild);
        ...
        final int indexOfChild = parseTree.getIndexOfChild(node, new DetailAstImpl());
        assertWithMessage("Invalid child index")
            .that(indexOfChild)
            .isEqualTo(-1);
    }
    ...
    public void testGetValueAtDetailNode() {
        ...
        assertWithMessage("Child has to be leaf")
                .that(parseTree.isLeaf(node.getFirstChild()))
                .isTrue();
