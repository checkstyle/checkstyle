...
public class InlineConfigUtilsTest {
    ...
    public void testIsProperUtilsClass() throws ReflectiveOperationException {
        assertWithMessage("Constructor is not private")
                .that(isUtilsClassHasPrivateConstructor(InlineConfigUtils.class))
                .isTrue();
    }
    ...
    public void testMatchDelimiterPropertiesTargetStyle() {
        ...
        assertWithMessage("Should match properties target style config")
                .that(result)
                .isEqualTo(new MatchedDelimiter(null, true));
    }
    ...
    @Test
    public void testMatchDelimiterPropertiesFileWithCommentPrefix() {
        final MatchedDelimiter result = InlineConfigUtils.matchDelimiter(
                List.of("#<module name=\"Checker\">"), "Example.properties");

        assertWithMessage("Should match when first line starts with #")
                .that(result)
                .isEqualTo(new MatchedDelimiter(null, true));
    }
    ...
    public void testDescribeExpectedDelimitersPropertiesFile() {
        ...
        assertWithMessage("Description should mention properties convention")
                .that(result)
                .isEqualTo("\"/*xml\" or \"/*\" (Java-comment style), or \"#\""
                    + " leading comment block, ending at the first blank line");
    }
    ...
    @Test
    public void testStripPropertiesCommentPrefixMixedLines() {
        final List<String> result = InlineConfigUtils.stripPropertiesCommentPrefix(
                List.of("#line with hash", "line without hash", "#another hash"));

        assertWithMessage("Mixed lines should be processed correctly")
                .that(result)
                .containsExactly("line with hash", "line without hash", "another hash")
                .inOrder();
    }
    ...
    public void testMatchDelimiterPropertiesMarkerIgnoredForNonPropertiesExtension() {
        final MatchedDelimiter result = InlineConfigUtils.matchDelimiter(
                List.of("#<module name=\"Checker\">"), "Example.xml");
    ...
    @Test
    public void testGetConfigEndIndexWithNullEndDelimiter() {
        final int result = InlineConfigUtils.getConfigEndIndex(
                List.of(
                    "#<module name=\"Checker\">",
                    "#  <module name=\"UniqueProperties\"/>",
                    "",
                    "key=value"),
                new MatchedDelimiter(null, true));

        assertWithMessage("Should find end at first blank line")
                .that(result)
                .isEqualTo(2);
    }

    @Test
    public void testGetConfigEndIndexWithNullEndDelimiterNoBlankLine() {
        final int result = InlineConfigUtils.getConfigEndIndex(
                List.of(
                    "#<module name=\"Checker\">",
                    "#  <module name=\"UniqueProperties\"/>",
        ...
        assertWithMessage("Should find end at end of file when no blank line")
                .that(result)
                .isEqualTo(3);
    }

    @Test
    public void testGetConfigEndIndexWithExplicitEndDelimiter() {
        final int result = InlineConfigUtils.getConfigEndIndex(
                List.of("/*xml", "#<module name=\"Checker\">", "*/", "key=value"),
                new MatchedDelimiter(InlineConfigUtils.JAVA_CONFIG_END, true));

        assertWithMessage("Should find end at explicit delimiter")
                .that(result)
                .isEqualTo(2);
    }

    @Test
    public void testGetConfigEndIndexWithExplicitEndDelimiterNotFound() {
        final int result = InlineConfigUtils.getConfigEndIndex(
                List.of("/*xml", "#<module name=\"Checker\">", "key=value"),
                new MatchedDelimiter(InlineConfigUtils.JAVA_CONFIG_END, true));

        assertWithMessage("Should return -1 when end delimiter not found")
                .that(result)
                .isEqualTo(-1);
    }
