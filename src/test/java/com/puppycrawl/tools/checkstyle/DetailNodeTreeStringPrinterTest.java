...
public class DetailNodeTreeStringPrinterTest extends AbstractTreeTestSupport {
    ...
    @Test
    public void testNullEncodingForGetEncoding() {
        try {
            ...
            assertWithMessage(
                    "Expected getEncoding(null, ISO_8859_1.name()) to return ISO_8859_1")
                    .that(returnValue)
                    .isEqualTo(StandardCharsets.ISO_8859_1.name());

            final String returnValue2 = TestUtil.invokeStaticMethod(
                DetailNodeTreeStringPrinter.class, "getEncoding",
                "", StandardCharsets.ISO_8859_1.name());

            assertWithMessage("Expected getEncoding(\"\", ISO_8859_1.name()) to return ISO_8859_1")
                    .that(returnValue2)
                    .isEqualTo(StandardCharsets.ISO_8859_1.name());
        }
        catch (ReflectiveOperationException ex) {

            assertWithMessage("Unexpected exception occurred: " + ex.getMessage()).fail();
        }
    }

    @Test
    public void testBlankEncodingForGetEncoding() {
        try {
            final String returnValue = TestUtil.invokeStaticMethod(
                    DetailNodeTreeStringPrinter.class, "getEncoding",
                    "", StandardCharsets.ISO_8859_1.name());
            ...
            assertWithMessage(
                    "Expected getEncoding(\"\", ISO_8859_1.name()) to return ISO_8859_1")
                    .that(returnValue)
                    .isEqualTo(StandardCharsets.ISO_8859_1.name());
        }
        catch (ReflectiveOperationException ex) {
