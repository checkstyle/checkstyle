...
public class AvoidEscapedUnicodeCharactersCheck
    ...
    public void visitToken(DetailAST ast) {
        ...
        switch (tokenType) {
            case TokenTypes.STRING_LITERAL, TokenTypes.CHAR_LITERAL,
                 TokenTypes.TEXT_BLOCK_CONTENT -> {
            ...
            case TokenTypes.SINGLE_LINE_COMMENT -> processSingleLineComment(ast);
            case TokenTypes.BLOCK_COMMENT_BEGIN -> processBlockComment(ast);
            default -> {
    ...
    private void processBlockComment(DetailAST ast) {
        if (pendingLiteral != null && ast.getLineNo() == pendingLineNo) {
            final boolean isTrailing = isTrailingComment(ast, TokenTypes.BLOCK_COMMENT_BEGIN);
            if (isTrailing) {
                pendingLineHasComment = true;
            }
        }
