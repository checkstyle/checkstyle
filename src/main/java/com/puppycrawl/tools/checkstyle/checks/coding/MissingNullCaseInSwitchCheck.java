 ...
 * even if it is a reference type, i.e. {@code Integer}.
 * </li>
 * <li>
...
public class MissingNullCaseInSwitchCheck extends AbstractCheck {
    ...
    private static boolean hasNullCaseLabel(DetailAST detailAST) {
        return TokenUtil.findFirstTokenByPredicate(detailAST, ast -> {
            ...
            return caseLabel != null && hasNullCaseInCaseGroup(caseLabel);
        }).isPresent();
    }

    /**
     * Checks if the given case group contains a 'null' case label.
     *
     * @param caseLabel the AST node representing the first {@code LITERAL_CASE} in the case group
     * @return true if the case group contains a 'null' case label
     */
    private static boolean hasNullCaseInCaseGroup(DetailAST caseLabel) {
        return TokenUtil.findFirstTokenByPredicate(caseLabel.getParent(), ast -> {
    ...
    private static boolean hasStringLiteral(DetailAST caseLabel) {
        final DetailAST expr = caseLabel.findFirstToken(TokenTypes.EXPR);
