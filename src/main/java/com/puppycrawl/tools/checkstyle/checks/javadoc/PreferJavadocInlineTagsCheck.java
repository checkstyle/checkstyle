...
public class PreferJavadocInlineTagsCheck extends AbstractJavadocCheck {
    ...
    public void visitJavadocToken(DetailNode ast) {
        if (!isInsidePreTag(ast) && !isInsideCodeOrLiteralTag(ast)) {
    ...
    /**
     * Checks anchor tags and flags internal references that should use {@link ...}.
     *
    ...
    private void checkForAngleBracketEntities(DetailNode textNode) {
        ...
        int index = text.indexOf(HTML_ENTITY_LT);
        while (index >= 0) {
            log(textNode.getLineNumber(), textNode.getColumnNumber() + index,
                MSG_KEY, "{@literal <}", HTML_ENTITY_LT);
            index = text.indexOf(HTML_ENTITY_LT, index + HTML_ENTITY_LT.length());
        }

        index = text.indexOf(HTML_ENTITY_GT);
        while (index >= 0) {
            log(textNode.getLineNumber(), textNode.getColumnNumber() + index,
                MSG_KEY, "{@literal >}", HTML_ENTITY_GT);
            index = text.indexOf(HTML_ENTITY_GT, index + HTML_ENTITY_GT.length());
        }
    }
    ...
    private static boolean isInsidePreTag(DetailNode node) {
        ...
        while (parent != null && !insidePre) {
            if (parent.getType() == JavadocCommentsTokenTypes.HTML_ELEMENT) {
                final String tagName = getHtmlTagName(parent);
                insidePre = "pre".equalsIgnoreCase(tagName);
            }
    ...
    /**
     * Checks if the node is inside a {@code } or {@literal } inline tag.
    ...
    private static boolean isInsideCodeOrLiteralTag(DetailNode node) {
        boolean insideCodeOrLiteral = false;
        DetailNode parent = node.getParent();
        while (parent != null && !insideCodeOrLiteral) {
            final int type = parent.getType();
            insideCodeOrLiteral = type == JavadocCommentsTokenTypes.CODE_INLINE_TAG
                    || type == JavadocCommentsTokenTypes.LITERAL_INLINE_TAG;
            parent = parent.getParent();
        }
        return insideCodeOrLiteral;
    }
}
