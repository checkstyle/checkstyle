...
public final class NPathComplexityCheck extends AbstractCheck {
    ...
    private final Deque<Boolean> belongsToRangeValues = new ArrayDeque<>();
     ...
     */
    private final ExpressionRange processedExpressionRange = new ExpressionRange();
    ...
    public void visitToken(DetailAST ast) {
        switch (ast.getType()) {
            ...
            case TokenTypes.LITERAL_ELSE:
                branchVisited = true;
                if (currentRangeValue == BigInteger.ZERO) {
    ...
    private void visitUnitaryOperator(DetailAST ast, int basicBranchingFactor) {
        final boolean isBelongsToRange = processedExpressionRange.belongsToRange(ast);
        belongsToRangeValues.push(isBelongsToRange);
        if (!isBelongsToRange) {
    ...
    private void leaveUnitaryOperator() {
        if (!belongsToRangeValues.pop()) {
            ...
            if (expressionValue == BigInteger.ZERO) {
                expressionValue = BigInteger.ONE;
            }
            if (basicRangeValue == BigInteger.ZERO) {
    ...
    private void leaveBranch() {
        ...
        final BigInteger expressionValue = valuePair.getExpressionValue();
        if (branchVisited && currentRangeValue == BigInteger.ZERO) {
            currentRangeValue = BigInteger.ONE;
        }
    ...
    /**
     * A range between tokens. Used to prevent inline ternary operator from being processed
     * twice.
     */
    private static class ExpressionRange {
         ...
         * @param startToken starting token of the range.
         * @param endToken ending token of the range.
         */
        ...
        public boolean belongsToRange(DetailAST ast) {
            ...
            // comparision (lineNo < startLineNo) and (columnNo <= startColumnNo)
            // is not used, cause check visit tokens consecutive by line
            if (lineNo > endLineNo
                    || lineNo == endLineNo
