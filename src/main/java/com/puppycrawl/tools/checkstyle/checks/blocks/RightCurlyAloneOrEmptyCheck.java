...
public class RightCurlyAloneOrEmptyCheck extends AbstractCheck {
     ...
     * must use K&R style braces instead.
     */
    ...
    public void visitToken(DetailAST ast) {
        ...
        switch (ast.getType()) {
            case TokenTypes.CLASS_DEF, TokenTypes.INTERFACE_DEF, TokenTypes.ANNOTATION_DEF,
                ...
                final DetailAST child2 = ast.findFirstToken(TokenTypes.OBJBLOCK);
                rcurly = child2.getLastChild();
            }
            ...
            default -> {
                final DetailAST child2 = ast.findFirstToken(TokenTypes.SLIST);
                if (child2 != null) {
                    rcurly = child2.getLastChild();
                }
            }
    ...
    private void checkRcurly(DetailAST rcurly, DetailAST ast) {
        if (checkOnRightSide(rcurly)) {
            log(rcurly, MSG_KEY_ALONE_OR_EMPTY,
                ...
                && !isBlockEmptyAndAllowedConsise(rcurly, ast)) {
            log(rcurly, MSG_KEY_ALONE_OR_EMPTY,
                    rcurly.getText(), rcurly.getColumnNo() + 1);
        }
    }
     ...
     */
    private static boolean isBlockEmptyAndAllowedConsise(DetailAST rcurly, DetailAST block) {
