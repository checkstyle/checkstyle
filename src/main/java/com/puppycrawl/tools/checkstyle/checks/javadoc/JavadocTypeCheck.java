...
public class JavadocTypeCheck
    ...
    private static boolean isTagInsideCodeOrLiteralBlock(String[] lines,
        ...
        final int maxLineIndex = Math.min(tagLineIndex, lines.length - 1);

        final int estimatedCapacity = (maxLineIndex + 1) * 80;
        final StringBuilder textUpToTag = new StringBuilder(estimatedCapacity);
        for (int index = 0; index <= maxLineIndex; index++) {
            if (index == tagLineIndex) {
                final int endIndex = Math.min(tagCol, lines[index].length());
                textUpToTag.append(lines[index], 0, endIndex);
            }
            else {
