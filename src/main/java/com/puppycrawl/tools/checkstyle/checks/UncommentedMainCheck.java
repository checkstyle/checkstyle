...
public class UncommentedMainCheck
    ...
    private void visitClassOrRecordDef(DetailAST classOrRecordDef) {
        ...
        if (classDepth == 0) {
            final DetailAST ident =
                    NullUtil.notNull(classOrRecordDef.findFirstToken(TokenTypes.IDENT));
    ...
    private static boolean checkName(DetailAST method) {
        final DetailAST ident = NullUtil.notNull(method.findFirstToken(TokenTypes.IDENT));
    ...
    private static boolean checkModifiers(DetailAST method) {
        final DetailAST modifiers =
            NullUtil.notNull(method.findFirstToken(TokenTypes.MODIFIERS));
    ...
    private static boolean checkType(DetailAST method) {
        final DetailAST type =
            NullUtil.notNull(method.findFirstToken(TokenTypes.TYPE)).getFirstChild();
    ...
    private static boolean checkParams(DetailAST method) {
        ...
        final DetailAST params =
                NullUtil.notNull(method.findFirstToken(TokenTypes.PARAMETERS));

        if (params.getChildCount() == 1) {
            final DetailAST parameterType =
                    NullUtil.notNull(params.getFirstChild()
