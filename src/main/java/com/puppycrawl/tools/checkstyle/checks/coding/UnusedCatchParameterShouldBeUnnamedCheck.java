...
public class UnusedCatchParameterShouldBeUnnamedCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        else if (isCatchParameterIdentifierCandidate(ast) && !isLeftHandSideValue(ast)) {
            catchParameters.stream()
                    .filter(c -> c.getName().equals(ast.getText()))
                    .findFirst()
    ...
    public void leaveToken(DetailAST ast) {
        CatchParameterDetails catchParameter = catchParameters.peekLast();
        if (catchParameter != null && ast == catchParameter.getEnclosingCatchClause()) {
            if (!"_".equals(catchParameter.getName()) && !catchParameter.isUsed()) {
                log(catchParameter.getParameterDefinition(),
                            MSG_UNUSED_CATCH_PARAMETER,
                            catchParameter.getName());
            }
            catchParameters.pollLast();
        }
    }
     ...
     * @return true if the given {@link TokenTypes#IDENT} is a catch parameter ident
     */
    private boolean isCatchParameterIdentifierCandidate(DetailAST identAST) {
        // we should ignore the ident if it is in the exception declaration
        final boolean catchParameterDeclaration = identAST.getParent().getParent()
                                                .getType() == TokenTypes.LITERAL_CATCH;

        return !catchParameterDeclaration
                && (!TokenUtil.isOfType(identAST.getParent(), INVALID_CATCH_PARAM_IDENT_PARENTS)
     ...
     */
    private static boolean shouldCheckDotChild(DetailAST identAst) {
    ...
    private static final class CatchParameterDetails {
        ...
        /**
         * The catch parameter is enclosed by the ast of type
         * {@link TokenTypes#LITERAL_CATCH}.
         */
        private final DetailAST enclosingCatchClause;
        ...
        private CatchParameterDetails(DetailAST enclosingCatchClause) {
            this.enclosingCatchClause = enclosingCatchClause;
            this.parameterDefinition = enclosingCatchClause.findFirstToken(TokenTypes.PARAMETER_DEF);
            this.name = this.parameterDefinition.findFirstToken(TokenTypes.IDENT).getText();
        }
        ...
        /**
         * Get the enclosing token of the catch parameter
         * represented by ast of type {@link TokenTypes#LITERAL_CATCH}.
         *
         * @return the enclosing token of the catch parameter
         */
        private DetailAST getEnclosingCatchClause() {
            return enclosingCatchClause;
        }
