    ...
    private static boolean containsNestedMethodCallOnNewLine(DetailAST elist, DetailAST mainAst) {
        ...
        for (DetailAST expr = elist.getFirstChild();
            ...
            if (expr.getType() == TokenTypes.EXPR
                && expr.getFirstChild().getType() == TokenTypes.METHOD_CALL
                && expr.getFirstChild().getLineNo() != mainAst.getLineNo()
                && !TokenUtil.areOnSameLine(expr.getFirstChild(),
                    expr.getFirstChild().findFirstToken(TokenTypes.RPAREN))) {
                result = true;
                break;
            }
        }
        ...
        for (DetailAST expr = elist.getFirstChild();
            ...
            if (expr.getType() == TokenTypes.EXPR) {
                ...
                final boolean isMultiLineNestedCall =
                    ...
                    && !TokenUtil.areOnSameLine(inner,
                        inner.findFirstToken(TokenTypes.RPAREN));
