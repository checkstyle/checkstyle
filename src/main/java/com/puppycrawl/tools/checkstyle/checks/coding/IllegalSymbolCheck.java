...
public class IllegalSymbolCheck extends AbstractCheck {
    ...
    /**
     * Parses the configured symbolCodes string.
     *
    ...
    private void parseSymbolCodes() {
        ...
        final boolean hasSymbols = !symbolCodes.isEmpty();
                ...
                if (!trimmed.isEmpty()) {
                    try {
                        if (trimmed.contains(RANGE_SEPARATOR)) {
                            ...
                            final int codePoint = parseCodePoint(trimmed);
                            singleCodePoints.add(codePoint);
                        }
                    }
    ...
    /**
     * Checks text for illegal symbols.
     *
    ...
    private void checkText(String text, DetailAST ast) {
        ...
        while (offset < length && !violationFound) {
     ...
     *
     * @param rangeStr range definition string
    ...
    private void parseRange(String rangeStr) {
        ...
        final CodePointRange newRange =
                new CodePointRange(start, end);
        codePointRanges.add(newRange);
    }

    /**
     * Parses a Unicode code point from string.
     * Supported formats:
     * 0x1234, \\u1234, U+1234, or plain hex.
     *
        ...
        if (hasPrefix) {
            if (cleaned.length() <= 2) {
                throw new NumberFormatException(
                        "Invalid code point format: " + cleaned);
            }
