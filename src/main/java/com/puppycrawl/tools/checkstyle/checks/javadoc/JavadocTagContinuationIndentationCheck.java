...
public class JavadocTagContinuationIndentationCheck extends AbstractJavadocCheck {
        ...
        if (isBlockDescription(ast) && !isInlineDescription(ast)) {
            ...
            for (DetailNode newlineNode : textNodes) {
                ...
                if (newlineNode.getType() == JavadocTokenTypes.HTML_TAG_NAME) {
                    isTextPreTagChildren = !isTextPreTagChildren;
                }
                else if (textNode.getType() != JavadocTokenTypes.NEWLINE && !isTextPreTagChildren
    ...
    /**
     * Checks if the given HTML_TAG_NAME node contains {@code <pre>} tag.
     *
    ...
    private static boolean containsPreTag(DetailNode ast) {
        ...
        return "pre".equals(node.getText());
    }
