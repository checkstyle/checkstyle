...
public class RedundantModifierCompactSourceCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        if (ast.getParent().getType() == TokenTypes.COMPACT_COMPILATION_UNIT) {
            ...
            for (DetailAST child = modifiers.getFirstChild(); child != null;
                ...
                if (isRedundantModifier(ast, child)) {
     ...
     *
     * @param methodDef direct compact-source method
     * @param modifierAst modifier to check
     * @return whether the modifier is redundant
     */
    private static boolean isRedundantModifier(DetailAST methodDef, DetailAST modifierAst) {
        return TokenUtil.isOfType(modifierAst, TokenTypes.LITERAL_PUBLIC,
                TokenTypes.LITERAL_PROTECTED, TokenTypes.STRICTFP)
                || modifierAst.getType() == TokenTypes.FINAL
                    && !isFinalRequiredForSafeVarargs(methodDef);
    }

    /**
     * Checks whether {@code final} is necessary for a variable-arity method annotated
     * with {@code SafeVarargs}.
     *
     * @param methodDef method definition
     * @return whether {@code final} is necessary
     */
    private static boolean isFinalRequiredForSafeVarargs(DetailAST methodDef) {
        final DetailAST modifiers =
