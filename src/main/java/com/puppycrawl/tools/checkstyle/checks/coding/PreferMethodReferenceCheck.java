 ...
 *     Consumer&lt;Object&gt; f = arg -&gt; arg.voidMethod(); // violation
...
public class PreferMethodReferenceCheck extends AbstractCheck {
    ...
    private boolean isArgsMatch(DetailAST ast, List<String> args) {
        ...
        final boolean result;
        if (dot == null || isCalledOnLiteral(dot) || isCalledOnClassLiteral(dot)) {
            result = true;
        }
    ...
    /**
     * Check that dot token has class literal like {@code String.class} as first part.
     *
     * @param dot token to check
     * @return true if first part is class literal
     */
    private boolean isCalledOnClassLiteral(DetailAST dot) {
        return Optional.ofNullable(dot.getFirstChild())
            .filter(child -> child.getType() == TokenTypes.DOT)
            .map(DetailAST::getLastChild)
            .map(DetailAST::getType)
            .map(type -> type.equals(TokenTypes.LITERAL_CLASS))
            .orElse(false);
    }

    /**
     * Check that dot token has {@code super}, {@code this} or string literal as first part.
     *
     * @param dot token to check
     * @return true if first part is literal
     */
    private static boolean isCalledOnLiteral(DetailAST dot) {
        return LITERAL_CALL_ARGS.contains(dot.getFirstChild().getType());
    }
