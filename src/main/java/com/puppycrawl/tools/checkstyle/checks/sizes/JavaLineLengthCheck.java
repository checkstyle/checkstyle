...
public class JavaLineLengthCheck extends AbstractCheck {
    ...
    private void collectAstNodes(DetailAST ast) {
        ...
        while (!stack.isEmpty()) {
            ...
            final DetailAST existing = lineToAst.get(current.getLineNo());
            if (existing == null || current.getColumnNo() < existing.getColumnNo()) {
                lineToAst.put(current.getLineNo(), current);
            }

            final DetailAST nextSibling = current.getNextSibling();
            if (nextSibling != null) {
