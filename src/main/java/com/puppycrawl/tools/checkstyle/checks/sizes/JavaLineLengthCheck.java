...
public class JavaLineLengthCheck extends AbstractCheck {
     ...
     */
    private NavigableMap<Integer, DetailAST> lineToAst = new TreeMap<>();
    ...
    private void collectAstNodes(DetailAST ast) {
        if (ast != null) {
            lineToAst.putIfAbsent(ast.getLineNo(), ast);
            collectAstNodes(ast.getFirstChild());
            collectAstNodes(ast.getNextSibling());
        }
    }
}
