 ...
 * <ul>
 * <li>
 * Property {@code customImportOrderRules} - Specify list of import groups.
...
public class CustomImportOrderCheck extends AbstractCheck {
    ...
    /**
     * Setter to specify list of import groups.
     *
     * @param rules
     *        user value.
     * @since 5.8
    ...
    public final void setCustomImportOrderRules(String... rules) {
        if (rules.length == 1) {
            // keep backwards compatibility with single '###' separated string
            ...
            Arrays.stream(rules).filter(rule -> !rule.isBlank())
