...
public class NeedBracesCheck extends AbstractCheck {
    ...
    private boolean isBracesNeeded(DetailAST ast) {
        final boolean result = switch (ast.getType()) {
    ...
    private static boolean isSingleLineStatement(DetailAST statement) {
        final boolean result = switch (statement.getType()) {
            ...
            case TokenTypes.LITERAL_CASE, TokenTypes.LITERAL_DEFAULT -> isSingleLineSwitchMember(statement);
