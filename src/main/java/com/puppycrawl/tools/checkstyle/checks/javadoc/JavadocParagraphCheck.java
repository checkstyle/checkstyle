 ...
 * </ul>
 * <p>
 * <b>ATTENTION:</b> This Check ignores HTML comments.
 * </p>
 * <ul>
 * <li>
...
public class JavadocParagraphCheck extends AbstractJavadocCheck {
    ...
    private static boolean isNestedParagraph(DetailNode tag) {
        ...
        if (JavadocUtil.getFirstChild(tag).getType() == JavadocTokenTypes.PARAGRAPH) {
            ...
            while (parent != null) {
                final boolean isHtmlElement = parent.getType() == JavadocTokenTypes.HTML_ELEMENT;
                final boolean isParagraph =
                        JavadocUtil.getFirstChild(parent).getType() == JavadocTokenTypes.PARAGRAPH;
                if (isHtmlElement && isParagraph) {
                    isNested = true;
                    break;
                }
