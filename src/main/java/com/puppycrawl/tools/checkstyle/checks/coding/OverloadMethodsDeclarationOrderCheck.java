...
public class OverloadMethodsDeclarationOrderCheck extends AbstractCheck {
    ...
    private void checkOverloadMethodsGrouping(DetailAST objectBlock) {
        ...
        TokenUtil.forEachChild(objectBlock, TokenTypes.METHOD_DEF, currentToken -> {
            processMethod(currentToken, methodIndexMap, methodLineNumberMap, currentIndex);
        });
    }

    /**
     * Processes method definition token to check if overload methods are grouped together.
     *
     * @param currentToken method definition token.
     * @param methodIndexMap map of method names to their index.
     * @param methodLineNumberMap map of method names to their line number.
     * @param currentIndex array containing the current method index.
     */
    private void processMethod(DetailAST currentToken,
                               Map<String, Integer> methodIndexMap,
                               Map<String, Integer> methodLineNumberMap,
                               int[] currentIndex) {
