...
public class TypeBodyPaddingCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (!shouldSkip(ast) && hasBodyToProcess(objBlock)) {
            if (hasStartViolation(objBlock)) {
                log(objBlock.findFirstToken(TokenTypes.LCURLY), MSG_START_REQUIRED);
            }
            if (hasEndViolation(objBlock)) {
                log(objBlock.findFirstToken(TokenTypes.RCURLY), MSG_END_REQUIRED);
            }
        }
    }
    ...
    private boolean hasBodyToProcess(DetailAST ast) {
        final DetailAST startCurly = ast.findFirstToken(TokenTypes.LCURLY);
        final DetailAST endCurly = ast.findFirstToken(TokenTypes.RCURLY);
        final boolean bodyIsEmpty = startCurly.getNextSibling() == endCurly;
        boolean result = true;
        if (bodyIsEmpty && allowEmpty) {
            result = false;
        }
        return result;
    }
    ...
    private boolean hasStartViolation(DetailAST objBlock) {
        ...
        if (starting) {
            final DetailAST startCurly = objBlock.findFirstToken(TokenTypes.LCURLY);
            final DetailAST endCurly = objBlock.findFirstToken(TokenTypes.RCURLY);
            if (TokenUtil.areOnSameLine(startCurly, endCurly)) {
                violation = true;
            }
            else if (!nextLineIsBlankFor(startCurly)) {
                violation = true;
            }
        }
        return violation;
    ...
    private boolean hasEndViolation(DetailAST ast) {
        ...
        if (ending) {
            ...
            else if (!previousLineIsBlankFor(endCurly)) {
                violation = true;
            }
        }
        return violation;
