...
public class JavadocParamOrderCheck extends AbstractJavadocCheck {
    ...
    private void checkParamOrder(DetailNode tagNode,
        ...
        final List<String> paramsList = getMethodParamsInList(methodParams, methodTypeParams);
        helperMethod(javadocParamsList, paramsList);
        checkAndLog(javadocParamsList, javadocParamsLineNumList);
    }

    /**
     * Make a list consists of elements present in  paramsList and not already exists in the
     * list itself before and store unique elements from javadoc param list.
     *
     * @param javadocParamsList list of javadoc parameters.
     * @param paramsList list of javadoc parameters.
     */
    private void helperMethod(List<String> javadocParamsList,
    ...
    /**
     * Method to convert and merge DetailAST List of method parameters in String List.
     *
     * @param methodParams list of method parameters.
     * @param methodTypeParams list of method type parameters.
     * @return list of method parameters in String List.
     */
    private static List<String> getMethodParamsInList(List<DetailAST> methodParams,
                                                          List<DetailAST> methodTypeParams) {
    ...
    /**
     * Checks the order of method/type definitions and log violations.
     *
     * @param javadocParamsList list of method parameters.
     * @param javadocParamsLineNumList list of method type parameters.
     */
    private void checkAndLog(List<String> javadocParamsList,
                             List<Integer> javadocParamsLineNumList) {
        for (int i = 0; i < javadocParamsList.size(); i++) {
            final String tagText = javadocParamsList.get(i);
            if (stringList.contains(tagText)) {
                if (integerList.get(stringList.indexOf(tagText)) == 1) {
                    log(javadocParamsLineNumList.get(i), MSG_JAVADOC_TAG_ORDER, tagText);
                }
                else if (i < javadocParamsList.size() - 1
                    && !tagText.equals(javadocParamsList.get(i + 1))) {
                    integerList.set(stringList.indexOf(tagText), 1);
                }
            }
        }
        stringList.clear();
