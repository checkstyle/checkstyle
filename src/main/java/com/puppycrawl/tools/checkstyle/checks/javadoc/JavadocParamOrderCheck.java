...
public class JavadocParamOrderCheck extends AbstractJavadocCheck {
            ...
            if (targetParent.contains(ast.getType())) {
                ...
                for (Integer type : targetSibiling) {
                    if (typeDef.getType() == type) {
                        ...
                        method1(node, new ArrayList<>(), methodTypeParams);
                    }
                    if (typeDef.findFirstToken(type) != null) {
                        ...
                        final List<DetailAST> methodTypeParams =
                            CheckUtil.getTypeParameters(typeDef);
                        method1(node, methodParams, methodTypeParams);
                    }
                }
            }
        }
     ...
     * @return descr.
     */
    private static LinkedHashMap<String, Integer> method2(List<String> javadocParamsList,
                                                          List<String> paramsList) {
        Set<String> javadocIntersectionUniqueTags = new LinkedHashSet<>();
        for (String str : javadocParamsList) {
            if (paramsList.contains(str)) {
                javadocIntersectionUniqueTags.add(str);
            }
        }
        //convert LinkedHashSet to List
        List<String> javadocIntersectionUniqueTagsList =
            new ArrayList<>(javadocIntersectionUniqueTags);
        ...
        int count = 0;
        for (String str : javadocIntersectionUniqueTags) {
     ...
     */
    private void method1(DetailNode tagNode,
                                  final List<DetailAST> methodParams,
                                  final List<DetailAST> methodTypeParams) {
        ...
        List<String> javadocParamsList = new ArrayList<>();
        List<Integer> javadocParamsLineNumList = new ArrayList<>();
        ...
        LinkedHashMap<String, Integer> intersectionMap = method2(javadocParamsList, paramsList);
        for (int it = 0; it < javadocParamsList.size(); it++) {
            String tagText = javadocParamsList.get(it);
            if (intersectionMap.containsKey(tagText)) {
                if (intersectionMap.get(tagText) == 1) {
                    log(javadocParamsLineNumList.get(it), MSG_JAVADOC_TAG_ORDER, tagText);
                }
                else if (it < javadocParamsList.size()-1) {
                    if (!tagText.equals(javadocParamsList.get(it+1))) {
