    ...
    public void visitJavadocToken(DetailNode node) {
        ...
        if (ast != null) {
            ...
            if (targetParent.contains(ast.getType())) {
                ...
                final DetailAST prevSib = getBlockCommentAst().getPreviousSibling();
                if (prevSib != null && prevSib.getType() == TokenTypes.MODIFIERS) {
                    typeDef = ast;
                }
                for (Integer type : targetSibiling) {
                    ...
                    if (typeDef.findFirstToken(type) != null) {
                        final List<DetailAST> methodParams = getParameters(typeDef.findFirstToken(type));
                        final List<DetailAST> methodTypeParams = CheckUtil.getTypeParameters(typeDef);
