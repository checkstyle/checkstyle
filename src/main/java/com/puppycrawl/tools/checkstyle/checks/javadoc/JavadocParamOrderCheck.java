...
public class JavadocParamOrderCheck extends AbstractJavadocCheck {
    ...
    public void visitJavadocToken(DetailNode node) {
        ...
        final DetailAST ast = getBlockCommentAst().getParent();
        if (ast != null && ast.getType() == TokenTypes.MODIFIERS) {
            final DetailAST typeDef = ast.getParent();
            final DetailAST parameterAst = typeDef.findFirstToken(TokenTypes.PARAMETERS);
            final DetailAST recordsParameterAst = typeDef.findFirstToken(TokenTypes.RECORD_COMPONENTS);
            if (parameterAst != null) {
                final List<DetailAST> methodParams = getMethodParameters(parameterAst);
                final List<DetailAST> methodTypeParams = CheckUtil.getTypeParameters(typeDef);
                checkParamsOrder(node, methodParams, methodTypeParams);
            }
            else if (recordsParameterAst != null) {
                final List<DetailAST> methodParams = getRecordParameters(recordsParameterAst);
                //final List<DetailAST> methodTypeParams = CheckUtil.getTypeParameters(typeDef);
                checkParamsOrder(node, methodParams, new ArrayList<>());
            }
        }
    }
     ...
     */
    private static List<DetailAST> getRecordParameters(DetailAST ast) {
        //final DetailAST recordParameters =ast.findFirstToken(TokenTypes.RECORD_COMPONENT_DEF);
