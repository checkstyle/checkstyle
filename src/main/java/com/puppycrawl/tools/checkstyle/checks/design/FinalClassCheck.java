...
public class FinalClassCheck
    ...
    private void registerNestedSubclassToOuterSuperClasses(String qualifiedClassName,
        ...
        if (superClassName != null) {
            getNearestClassWithSameName(superClassName, classDesc -> {
                return CheckUtil.typeDeclarationNameMatchingCount(qualifiedClassName,
                                                                  classDesc.getQualifiedName());
            }).ifPresentOrElse(ClassDesc::registerNestedSubclass, () -> {
                Optional
                    .ofNullable(innerClasses.get(superClassName))
    ...
    private void registerAnonymousInnerClassToSuperClass(DetailAST literalNewAst,
        ...
        }).ifPresentOrElse(ClassDesc::registerSuperClassOfAnonymousInnerClass, () -> {
            Optional
                .ofNullable(innerClasses.get(superClassName))
     ...
     * {@code Main.a.b} to be the super class of the anonymous inner class.
     *
     ...
     *
     * <p>
     * Suppose our pattern class is {@code Main.j} and class to be matched is
     * {@code Main.j.a.b} then type declaration name matching count would be calculated by
     * comparing every character, and updating main counter when we hit "." or when it is the last
     * character of the pattern class and certain conditions are met. This is done so that matching
     * count is 6 instead of 5. This is due to the fact that pattern class can contain anonymous
     * inner class object of a nested class which isn't true in case of extending classes as you
     * can't extend nested classes.
     * </p>
     *
    ...
    private static int getAnonSuperTypeMatchingCount(String patternTypeDeclaration,
        ...
        for (int i = 0; i < minLength
            && patternTypeDeclaration.charAt(i) == typeDeclarationToBeMatched.charAt(i); ++i) {
            if (i == minLength - 1
                && shouldCountBeUpdatedAtLastCharacter
                || patternTypeDeclaration.charAt(i) == packageSeparator) {
                result = i;
            }
        }
        return result;
