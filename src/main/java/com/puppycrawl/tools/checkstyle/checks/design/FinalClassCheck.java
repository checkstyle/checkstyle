...
public class FinalClassCheck
    ...
    private void registerNestedSubclassToOuterSuperClasses(String qualifiedClassName,
        ...
        if (superClassName != null) {
            final Optional<ClassDesc> nearest =
                getNearestClassWithSameName(superClassName,
                                            className -> {
                                                return CheckUtil.typeDeclarationNameMatchingCount(
                                                    qualifiedClassName, className);
                                            });
            nearest.ifPresentOrElse(ClassDesc::registerNestedSubclass,
                                    () -> {
                                        Optional
    ...
    private void registerAnonymousInnerClassToSuperClass(DetailAST literalNewAst,
        ...
        final Optional<ClassDesc> superClassDesc =
            getNearestClassWithSameName(superClassName,
                                        className -> {
                                            return CheckUtil
                                                .getAnonSuperTypeDeclarationMatchingCount(
                                                    outerTypeDeclName, className);
                                        });
        superClassDesc.ifPresentOrElse(ClassDesc::registerSuperClassOfAnonymousInnerClass,
                                       () -> {
     ...
     * {@link CheckUtil#getAnonSuperTypeDeclarationMatchingCount} to
    ...
    private Optional<ClassDesc> getNearestClassWithSameName(String className,
        Function<String, Integer> countProvider) {
        ...
        return innerClasses.entrySet().stream()
                ...
                .min((first, second) -> {
                    int diff = Integer.compare(
                        countProvider.apply(second.getQualifiedName()),
                        countProvider.apply(first.getQualifiedName())
                    );
                    if (diff == 0) {
