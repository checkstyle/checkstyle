...
public class FinalClassCheck
    ...
    private static String getClassToBeRegisteredAsSuperClass(String currentClassName,
        ...
        ClassDesc classToBeRegisteredAsSuperClass;
        if (classesWithSameName.size() > 1) {
            ...
            for (ClassDesc desc : classesWithSameName) {
                ...
                desc.setClassNameMatchingCount(classNameMatchingCount(
                        currentClassArray, classWithSameName));
            }
        }
        classToBeRegisteredAsSuperClass = classesWithSameName.get(0);
     ...
     */
    private static int classNameMatchingCount(char[] patternClass, String classToBeMatched) {
        final char[] classToBeMatchedArray = classToBeMatched.toCharArray();
        final int minLength = Math.min(classToBeMatchedArray.length, patternClass.length);
        int counter = 0;
        int countToBeRegistered = 0;
        for (int i = 0; i < minLength; i++) {
            if (patternClass[i] == classToBeMatchedArray[i]) {
                counter++;
                if (patternClass[i] == PACKAGE_SEPARATOR.toCharArray()[0]) {
