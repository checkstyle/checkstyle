...
public class JavadocLeadingAsteriskAlignCheck extends AbstractJavadocCheck {
    ...
    public void visitJavadocToken(DetailNode ast) {
        ...
        if (!isJavadocStartingLine) {
            final OptionalInt asteriskColumnNumber =
     ...
     */
    private OptionalInt getAsteriskColumnNumberWithTabsExpanded(String line) {
        OptionalInt columnNumber = getAsteriskColumnNumber(line);

        if (columnNumber.isPresent()) {
            singleSpaceAsteriskColNumber = columnNumber.orElseThrow();
            columnNumber = OptionalInt.of(CommonUtil.lengthExpandedTabs(
    ...
    private static OptionalInt getAsteriskColumnNumber(String line) {
        ...
        final OptionalInt columnNumber;
        ...
        if (matcher.find()) {
            columnNumber = OptionalInt.of(matcher.group(1).length() + 1);
        }
        else {
            columnNumber = OptionalInt.empty();
        }
