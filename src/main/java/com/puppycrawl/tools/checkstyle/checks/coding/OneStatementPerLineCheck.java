 ...
 * block statements, variable declaration statements, empty statements,
 * import statements, assignment statements, expression statements,
 * increment statements, object creation statements, 'for loop' statements,
 * 'break' statements, 'continue' statements, 'return' statements,
 * resources statements (optional).
 * </p>
 *
 * @since 5.3
 */
...
public final class OneStatementPerLineCheck extends AbstractCheck {
    ...
    public void beginTree(DetailAST rootAST) {
        lastStatementEnd = 0;
        lastVariableResourceStatementEnd = lastStatementEnd;
    ...
    public void visitToken(DetailAST ast) {
        switch (ast.getType()) {
            ...
            default -> {
                // Expected block statements
                DetailAST previousNode = ast.getPreviousSibling();
                if (previousNode != null) {
                    previousNode = getLastNestedLeafNode(previousNode);
    ...
    public void leaveToken(DetailAST ast) {
        switch (ast.getType()) {
            case TokenTypes.SEMI -> {
                if (getStatementStart(ast).getType() != TokenTypes.SEMI) {
    ...
    private void checkIfSemicolonIsInDifferentLineThanPrevious(DetailAST ast) {
        ...
        else if (!isFirstStatementOfLambdaOrAnonymous
                && statementStart.getType() != TokenTypes.SEMI) {
    ...
    private boolean isValidStatement(DetailAST statementStart) {
        ...
        return !blockStatementBefore && statementStart.getLineNo() != lastStatementInCurrentScope;
    }
     ...
     *
     * @param startToken start token of statement.
     * @return the last nested leaf {@code DetailAST} node
     */
    private static DetailAST getLastNestedLeafNode(DetailAST startToken) {
        DetailAST current = startToken;
