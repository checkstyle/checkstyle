...
public final class OneStatementPerLineCheck extends AbstractCheck {
     ...
     */
    private int lastStatementAnnonClassOrLambda;

    /**
     * Tracks the current nesting depth of annonymous class.
     */
    private int nestingDepthAnnonymous;
    ...
    public void visitToken(DetailAST ast) {
        switch (ast.getType()) {
            ...
            case TokenTypes.OBJBLOCK -> {
                final boolean isAnnonymus = ast.getParent().getType() == TokenTypes.LITERAL_NEW;
                if (isAnnonymus) {
                    nestingScope.push(lastStatementEnd);
                    nestingDepthAnnonymous++;
                }
            }
    ...
    private boolean onPreviousLine(DetailAST ast) {
        return lastStatementEnd == ast.getLineNo() && forStatementEnd != ast.getLineNo()
                && lastStatementAnnonClassOrLambda != ast.getLineNo();
    ...
    private boolean isStatementStartOnPreviousLine(DetailAST ast) {
        ...
        if (nestingDepthLambda + nestingDepthAnnonymous == nestingScope.size()) {
            lastStatementInScope = lastStatementEnd;
            lastStatementColumnInScope = lastStatementColumn;
        }
        else {
            lastStatementInScope = nestingScope.peekFirst();
