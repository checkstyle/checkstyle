...
public final class OneStatementPerLineCheck extends AbstractCheck {
    ...
    /**
     * Stack indicating whether each object block belongs to an anonymous class
     * and should restore the previous nesting scope when exited.
     */
    private final Deque<Boolean> anonymousBlockStack = new ArrayDeque<>();
    ...
    public void visitToken(DetailAST ast) {
        switch (ast.getType()) {
            ...
            case TokenTypes.LAMBDA, TokenTypes.OBJBLOCK -> {
                ...
                final boolean shouldTrackNesting = ast.getType() == TokenTypes.LAMBDA
                        || ast.getParent().getType() == TokenTypes.LITERAL_NEW;
                if (ast.getType() == TokenTypes.OBJBLOCK) {
                    anonymousBlockStack.push(shouldTrackNesting);
                }
                if (shouldTrackNesting) {
    ...
    public void leaveToken(DetailAST ast) {
        switch (ast.getType()) {
            ...
            case TokenTypes.LAMBDA, TokenTypes.OBJBLOCK -> {
                final boolean shouldPopNesting = ast.getType() == TokenTypes.LAMBDA
                        || anonymousBlockStack.pop();
