...
public class JavadocVariableCheck
    ...
    private static boolean hasJavadocComment(DetailAST variableDefAst) {
        ...
        if (variableDefAst.getType() == TokenTypes.VARIABLE_DEF) {
            result = hasJavadocCommentInTree(variableDefAst);
        }
        else {
            ...
            result = hasBlockCommentJavadoc(variableDefAst.getFirstChild())
                || hasJavadocCommentOnAnnotations(
                    Objects.requireNonNull(variableDefAst.findFirstToken(TokenTypes.ANNOTATIONS)));
        }
        return result;
    }
     ...
     *
     * @param ast AST of the variable definition.
     * @return true if a Javadoc comment is found, false otherwise.
     */
    private static boolean hasJavadocCommentInTree(DetailAST ast) {
        boolean found = false;
        for (DetailAST child = ast.getFirstChild();
            ...
            if (hasBlockCommentJavadoc(child.getFirstChild())) {
                ...
                found = hasJavadocCommentInTree(child);
            }
        }
        return found;
    }
