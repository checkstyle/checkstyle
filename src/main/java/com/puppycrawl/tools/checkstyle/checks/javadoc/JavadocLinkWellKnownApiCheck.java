 ...
 * For example, {@code {@link java.lang.String}} contains the package name, so it can be
 * matched through {@code wellKnownQualifiedPackages}. However, {@code {@link String}}
...
public class JavadocLinkWellKnownApiCheck extends AbstractJavadocCheck {
    ...
    public void visitJavadocToken(DetailNode ast) {
        ...
        final String reference = getNodeText(referenceNode);
        final String apiName = extractApiName(reference);
    ...
    /**
     * Extracts the API name from a reference, discarding the member part and type arguments.
     *
     * @param reference the raw reference text
     * @return the API name
     */
    private static String extractApiName(String reference) {
        final int memberSeparatorIndex = reference.indexOf('#');
        final String nameWithTypeArguments;
        if (memberSeparatorIndex == -1) {
            nameWithTypeArguments = reference;
        }
        else {
            nameWithTypeArguments = reference.substring(0, memberSeparatorIndex);
        }
        final int typeArgumentsIndex = nameWithTypeArguments.indexOf('<');
        final String result;
        if (typeArgumentsIndex == -1) {
            result = nameWithTypeArguments;
        }
        else {
            result = nameWithTypeArguments.substring(0, typeArgumentsIndex);
        }
        return result;
    }
    ...
    private boolean isWellKnownQualified(String apiName) {
        ...
        for (String packageName : wellKnownQualifiedPackages) {
            if (apiName.startsWith(packageName + DOT)) {
                result = true;
                break;
            }
        }
    ...
    /**
     * Recursively builds the full text of a node by concatenating the text of all its leaf
     * descendants.
     *
