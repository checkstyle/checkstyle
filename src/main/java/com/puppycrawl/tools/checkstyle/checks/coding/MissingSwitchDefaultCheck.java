...
public class MissingSwitchDefaultCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (!containsDefaultSwitch(firstSwitchMemberAst)
                && !containsCaseLabelWithPattern(firstSwitchMemberAst)
                && !containsCaseLabelWithNullReference(firstSwitchMemberAst)
    ...
    /**
     * Checks if a switch block contains a case label with a pattern variable defintion.
     ...
     */
    private static boolean containsCaseLabelWithPattern(DetailAST caseGroupAst) {
        final Predicate<DetailAST> astPredicate = ast -> ast.getFirstChild() != null
     ...
     */
    private static boolean containsCaseLabelWithNullReference(DetailAST caseGroupAst) {
        DetailAST nextAst = caseGroupAst;
        boolean found = false;

        while (nextAst.getFirstChild() != null) {
            ...
            final DetailAST nullReference = findAstByPredicate(grandChild,astPredicate);
    ...
    private static boolean containsDefaultSwitch(DetailAST caseGroupAst) {
        final Predicate<DetailAST> astPredicate = ast ->
                ast.findFirstToken(TokenTypes.LITERAL_DEFAULT) != null;
        return findAstByPredicate(caseGroupAst, astPredicate) != null;
    }
