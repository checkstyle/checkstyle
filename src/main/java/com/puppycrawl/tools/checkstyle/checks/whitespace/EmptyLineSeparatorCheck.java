...
public class EmptyLineSeparatorCheck extends AbstractCheck {
     ...
     */
    private static boolean isCommentNotOnPrevSiblingLines(DetailAST child, DetailAST token) {
        DetailAST prevSiblingLastestChild = token.getPreviousSibling();
        for (DetailAST tokenChild = prevSiblingLastestChild; tokenChild != null;
             tokenChild = tokenChild.getLastChild()) {
            prevSiblingLastestChild = tokenChild;
        }

        return (child.getType() == TokenTypes.BLOCK_COMMENT_BEGIN
