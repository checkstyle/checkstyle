...
public class EmptyLineSeparatorCheck extends AbstractCheck {
    ...
    private boolean isPrePreviousLineEmpty(DetailAST token) {
        ...
        final int number = 3;
        if (lineNo >= number) {
            final int[] codePoints = getLineCodePoints(lineNo - number);
            result = CommonUtil.isBlank(codePoints);
        }
        return result;
    }
    ...
    private boolean hasEmptyLineBefore(DetailAST token) {
        ...
        if (lineNo != 1) {
            ...
            final int[] codePoints = getLineCodePoints(lineNo - 2);
            result = CommonUtil.isBlank(codePoints);
        }
        return result;
    }
    ...
    private boolean isCommentInBeginningOfLine(DetailAST comment) {
        ...
        boolean result = false;
        if (comment != null) {
            final int[] codePoints = CodePointUtil.trim(getLineCodePoints(comment.getLineNo() - 1));
            result = CodePointUtil.startsWith(codePoints, "//", 0)
                     || CodePointUtil.startsWith(codePoints, "/*", 0);
        }
        return result;
    }
