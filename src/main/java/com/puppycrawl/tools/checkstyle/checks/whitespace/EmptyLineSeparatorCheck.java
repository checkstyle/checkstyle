...
public class EmptyLineSeparatorCheck extends AbstractCheck {
    ...
    private DetailAST getLastElementBeforeEmptyLines(DetailAST ast, int line) {
        ...
        if (travelAST.getNextSibling() != null && isPostFix(travelAST.getNextSibling())) {
     ...
     */
    private boolean isPostFix(DetailAST ast) {
        boolean isPostFix = false;
        final DetailAST travelAST = ast;
        if (travelAST.getType() == TokenTypes.EXPR
            && travelAST.getFirstChild().getType() == TokenTypes.METHOD_CALL
            && travelAST.getFirstChild().getFirstChild().getType() == TokenTypes.DOT) {
            isPostFix = true;
        }
        return isPostFix;
    }
