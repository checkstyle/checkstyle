...
public class EmptyLineSeparatorCheck extends AbstractCheck {
    ...
    private boolean isPrePreviousLineEmpty(DetailAST token) {
        ...
        if (lineNo >= number) {
            ...
            if (!result) {
                if (token.getPreviousSibling() != null
                    ...
                    if (beginningCommentBlock != null
                        && beginningCommentBlock.getType() == TokenTypes.BLOCK_COMMENT_BEGIN
                        && beginningCommentBlock.getLineNo() >= number) {
        ...
        if (lineNo != 1) {
            ...
            if (!result) {
                if (token.getPreviousSibling() != null
                    ...
                    // 3 is the number of the pre-previous to make sure it is present to work with as multiline
                    final int number = 3;
