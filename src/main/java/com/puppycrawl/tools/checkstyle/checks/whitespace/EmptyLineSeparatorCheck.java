    ...
    private boolean isPrePreviousLineEmpty(DetailAST token) {
        ...
        if (lineNo >= number) {
            ...
            if (lineBefore.contains("*/")) {
                DetailAST commentBlockBeginning = token.getFirstChild().getNextSibling()
                    .getFirstChild();
                if (commentBlockBeginning.getType() == BLOCK_COMMENT_BEGIN) {
                    prePreviousLine = getLine(commentBlockBeginning.getLineNo() - 3);
                }
            }
        ...
        if (lineNo != 1) {
            ...
            if (lineBefore.contains("*/")) {
                DetailAST commentBlockBeginning = token.getFirstChild().getNextSibling()
                    .getFirstChild();
                if (commentBlockBeginning.getType() == BLOCK_COMMENT_BEGIN) {
                    lineBefore = getLine(commentBlockBeginning.getLineNo() - 2);
                }
            }
