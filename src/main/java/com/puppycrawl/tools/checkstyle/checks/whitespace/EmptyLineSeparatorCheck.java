        ...
        if (lineNo >= number) {
            ...
            if (!result && token.findFirstToken(TokenTypes.TYPE) != null) {
                ...
                if (beginningCommentBlock != null) {
                    final int commentBlockPrePreviousLine = beginningCommentBlock.getLineNo() - 3;
                    result = CommonUtil.isBlank(
                        getLine(commentBlockPrePreviousLine));
                }
            }
        }
        return result;
    ...
    private boolean hasEmptyLineBefore(DetailAST token) {
        ...
        if (lineNo != 1) {
            ...
            if (!result && token.findFirstToken(TokenTypes.TYPE) != null) {
                ...
                if (beginningCommentBlock != null) {
                    final int commentBlockLineBefore = beginningCommentBlock.getLineNo() - 2;
                    result = CommonUtil.isBlank(
                        getLine(commentBlockLineBefore));
                }
            }
        }
        return result;
