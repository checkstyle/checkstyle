...
public class MissingLeadingAsteriskCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (JavadocUtil.isJavadocComment(commentContent)) {
            ...
            for (int i = 0; i < commentLines.length; i++) {
                ...
                final boolean isLast = i + 1 == commentLines.length;
                if (!isValidLine(commentLine, isLast)) {
    ...
    /**
     * Checks whether the given line is a valid comment line in javadoc or not.
     *
     * @param line a single comment line.
     * @param isLast if the given comment is the last line in the javadoc.
     * @return true if the given line is a valid comment line in javadoc.
     */
    private static boolean isValidLine(String line, boolean isLast) {
        return isLast && CommonUtil.isBlank(line)
            || !CommonUtil.isBlank(line) && line.trim().charAt(0) == '*';
    }
}
