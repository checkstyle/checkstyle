...
public class MissingLeadingAsteriskCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (JavadocUtil.isJavadocComment(commentContent) && !isInlineComment(ast)) {
     ...
     * @param blockStart block comment start token
     * @return true, if the given comment is an inline comment.
     */
    private static boolean isInlineComment(DetailAST blockStart) {
        final DetailAST blockEnd = blockStart.findFirstToken(TokenTypes.BLOCK_COMMENT_END);
        return TokenUtil.areOnSameLine(blockStart, blockEnd);
    }
}
