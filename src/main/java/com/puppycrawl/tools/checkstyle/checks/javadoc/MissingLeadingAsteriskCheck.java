...
public class MissingLeadingAsteriskCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (JavadocUtil.isJavadocComment(commentContent)) {
            ...
            // We need to ignore the last string.
            // As a java doc always has a empty string at the end, which is not a violation.
            for (int i = 0; i < commentLines.length; i++) {
                final String commentLine = commentLines[i];
                if (i + 1 != commentLines.length && CommonUtil.isBlank(commentLine)
                    || !CommonUtil.isBlank(commentLine) && commentLine.trim().charAt(0) != '*') {
