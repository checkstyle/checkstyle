...
public final class MissingOverrideCheck extends AbstractCheck {
    ...
    public void visitToken(final DetailAST ast) {
        final boolean containsTag = containsJavadocTag(ast);
    ...
    /**
     * Checks to see if the ast has Override annotation.
     *
     * @param ast AST node
     * @return true if has Override annotation
     */
    private static boolean hasOverrideAnnotation(DetailAST ast) {
        return AnnotationUtil.containsAnnotation(ast, OVERRIDE)
     ...
     */
    private static boolean containsJavadocTag(DetailAST ast) {
        final DetailAST startNode = TokenUtil.findFirstTokenByPredicate(ast.getFirstChild(),
            node -> node.getType() == TokenTypes.ANNOTATION)
            .orElse(ast.getFirstChild());
        final Optional<String> javadoc =
            TokenUtil.findFirstTokenByPredicate(startNode,
                        node -> node.getType() == TokenTypes.BLOCK_COMMENT_BEGIN)
