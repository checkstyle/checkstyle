 ...
 * <ul>
 * <li>
 * groups type/static imports: ensures that groups of imports come in a specific
 * order
 * (e.g., java. comes first, javax. comes second, then everything else)
 * </li>
 * <li>
 * adds a separation between type import groups : ensures that a blank line sit
 * between each group
 * </li>
 * <li>
 * type/static import groups aren't separated internally: ensures that each
 * group aren't separated
 * internally by blank line or comment
 * </li>
 * <li>
 * sorts type/static imports inside each group: ensures that imports within each
 * group are in
 * lexicographic order
 * </li>
 * <li>
 * sorts according to case: ensures that the comparison between imports is
 * case-sensitive, in
 * <a href="https://en.wikipedia.org/wiki/ASCII#Order">ASCII sort order</a>
 * </li>
 * <li>
 * arrange static imports: ensures the relative order between type imports and
 * static imports
 * (see
 * <a href=
 * "https://checkstyle.org/property_types.html#ImportOrderOption">ImportOrderOption</a>)
 * </li>
 * </ul>
 * <ul>
 * <li>
 ...
 * </li>
 * <li>
 * Property {@code groups} - Specify list of <b>type import</b> groups. Every
 * group identified
 * either by a common prefix string, or by a regular expression enclosed in
 * forward slashes
 * (e.g. {@code /regexp/}). If an import matches two or more groups, the best
 * match is selected.
 * All type imports, which does not match any group, falls into an
 * additional group, located at the end.
 * Thus, the empty list of type groups (the default value) means one group for
 * all type imports.
 * Type is {@code java.lang.String[]}.
 * Default value is {@code ""}.
 * </li>
 * <li>
 * Property {@code option} - Specify policy on the relative order between type
 * imports and static
 * imports.
 * Type is
 * {@code com.puppycrawl.tools.checkstyle.checks.imports.ImportOrderOption}.
 * Default value is {@code under}.
 * </li>
 * <li>
 * Property {@code ordered} - Control whether type imports within each group
 * should be
 * sorted.
 ...
 * </li>
 * <li>
 * Property {@code separated} - Control whether type import groups should be
 * separated
 ...
 * </li>
 * <li>
 * Property {@code separatedStaticGroups} - Control whether static import groups
 * should
 * be separated by, at least, one blank line or comment and aren't separated
 * internally.
 * This property has effect only when the property {@code option} is set to
 * {@code top}
 ...
 * <b>static imports</b> located at <b>top</b> or <b>bottom</b> are sorted
 * within the group.
 * Type is {@code boolean}.
 * Default value is {@code false}.
 * </li>
 * <li>
 * Property {@code staticGroups} - Specify list of <b>static</b> import groups.
 * Every group
 * identified either by a common prefix string, or by a regular expression
 * enclosed in forward
 * slashes (e.g. {@code /regexp/}). If an import matches two or more groups, the
 * best match is selected. All static imports, which does not match any group,
 * fall into
 * an additional group, located at the end. Thus, the empty list of static
 * groups (the default
 * value) means one group for all static imports. This property has effect only
 * when the property
 ...
 * </li>
 * <li>
 * Property {@code useContainerOrderingForStatic} - Control whether to use
 * container
...
public class ImportOrderCheck
     ...
     * {@code /regexp/}).
     * If an import matches two or more groups, the best match is selected.
     * All type imports, which does not match any group, falls into an additional
     * group,
     * located at the end. Thus, the empty list of type groups (the default value)
     * means one group
    ...
    /**
     * Specify list of <b>static</b> import groups. Every group identified either by
     * a common prefix
     * string, or by a regular expression enclosed in forward slashes (e.g.
     * {@code /regexp/}).
     * If an import matches two or more groups, the best match is selected.
     * All static imports, which does not match any group, fall into an additional
     * group, located
     * at the end. Thus, the empty list of static groups (the default value) means
     * one group for all
     * static imports. This property has effect only when the property
     ...
     * blank
     * line or comment and aren't separated internally. It doesn't affect
     * separations for static
     * imports.
     */
     ...
     * blank
     * line or comment and aren't separated internally. This property has effect
     * only when the
     * property {@code option} is set to {@code top} or {@code bottom} and when
     * property
     ...
     * Case-sensitive
     * sorting is in <a href="https://en.wikipedia.org/wiki/ASCII#Order">ASCII sort
     * order</a>.
     * It affects both type imports and static imports.
     */
     ...
     * When the {@code option} property is set to {@code INFLOW}, {@code ABOVE} or
     * {@code UNDER},
     * both the type and static imports use the properties {@code groups} and
     * {@code separated}.
     * When the {@code option} property is set to {@code TOP} or {@code BOTTOM},
     * static imports
    ...
    /**
     * Control whether <b>static imports</b> located at <b>top</b> or <b>bottom</b>
     * are
    ...
    /**
     * Setter to specify policy on the relative order between type imports and
     * static imports.
     *
    ...
    /**
     * Setter to specify list of <b>type import</b> groups. Every group identified
     * either by a
     * common prefix string, or by a regular expression enclosed in forward slashes
     * (e.g. {@code /regexp/}). If an import matches two or more groups, the best
     * match is selected. All type imports, which does not match any group, falls
     * into an
     * additional group, located at the end. Thus, the empty list of type groups
     * (the default value)
    ...
    /**
     * Setter to specify list of <b>static</b> import groups. Every group identified
     * either by a
     * common prefix string, or by a regular expression enclosed in forward slashes
     * (e.g. {@code /regexp/}). If an import matches two or more groups, the best
     * match is selected. All static imports, which does not match any group, fall
     * into an
     * additional group, located at the end. Thus, the empty list of static groups
     * (the default
     * value) means one group for all static imports. This property has effect only
     * when
    ...
    /**
     * Setter to control whether type import groups should be separated by, at
     * least,
     * one blank line or comment and aren't separated internally.
     * It doesn't affect separations for static imports.
     *
     * @param separated
     *                  whether groups should be separated by one blank line or
     *                  comment.
     * @since 3.2
     */
    ...
    /**
     * Setter to control whether static import groups should be separated by, at
     * least,
     * one blank line or comment and aren't separated internally.
     * This property has effect only when the property
     * {@code option} is set to {@code top} or {@code bottom} and when property
     * {@code staticGroups}
     * is enabled.
     *
    ...
    /**
     * Setter to control whether to use container ordering (Eclipse IDE term) for
     * static
     * imports or not.
     *
     * @param useContainerOrdering whether to use container ordering for static
     *                             imports or not.
     * @since 7.1
     */
    ...
    public void beginTree(DetailAST rootAST) {
        ...
        staticImportsApart = option == ImportOrderOption.TOP || option == ImportOrderOption.BOTTOM;
    }
    ...
    public void visitToken(DetailAST ast) {
        ...
        // using set of IF instead of SWITCH to analyze Enum options to satisfy
        // coverage.
     ...
     *
     * @param line the line of the current import.
     * @return true if there is separator before current import which isn't the
     *         first import.
     */
     ...
     * We first compare the container of the static import, container being the type
     * enclosing
     * the static element being imported. When this returns 0, we compare the
     * qualified
     ...
     * An import container name is the type which encloses the static element being
     * imported.
     * For example, HttpConstants, HttpHeaders, HttpHeaders.Names are import
     * container names:
     * 
     * <pre>
     * import static HttpConstants.COLON     =&gt; HttpConstants
    ...
    /**
     * Compiles the list of package groups and the order they should occur in the
     * file.
     *
     * @param packageGroups a comma-separated list of package names/prefixes.
