...
public class ImportOrderCheck
    ...
    private void doVisitToken(FullIdent ident, boolean isStatic,
        ...
        if (groupIdx > lastGroup) {
            if (!beforeFirstImport && line - lastImportLine < 2
                && shouldSeparateImportGroups(isStatic)) {
     ...
     */
    private boolean shouldSeparateImportGroups(boolean isStatic) {
        ...
        final boolean isSeparatedBetween = isStatic != lastImportStatic && separated
            && !separatedStaticGroups && staticImportsIndependent;
        // to be compatible with previous versions without separatedStaticGroups property
    ...
    private boolean isSeparatorInGroup(int groupIdx, boolean isStatic, int line) {
        return !needSeparatorBeforeImport(groupIdx, isStatic) && isSeparatorBeforeImport(line);
    }

    /**
     * Checks whether there should be a separator before current import.
     * @param groupIdx group number.
     * @param isStatic whether the token is static or not.
     * @return true if there should be a separator before current import.
     */
    private boolean needSeparatorBeforeImport(int groupIdx, boolean isStatic) {
        final boolean result;
        if (staticImportsIndependent) {
            result = isStatic != lastImportStatic || groupIdx != lastGroup
                && (!isStatic || separatedStaticGroups) && (isStatic || separated);
        }
        else {
            result = groupIdx != lastGroup && separated;
        }
        return result && (separated || separatedStaticGroups);
    }
    ...
    private boolean isInSameGroup(int groupIdx, boolean isStatic) {
        final boolean result;
        if (staticImportsIndependent && !separatedStaticGroups) {
