...
public final class BooleanExpressionComplexityCheck extends AbstractCheck {
    ...
    /**
     * Token types this check instance is actually configured to count, resolved
     * lazily from {@link #getTokenNames()} (falling back to {@link #getDefaultTokens()}
     * when no explicit {@code tokens} property was set). Distinct from
     * {@link #isBooleanOperatorType}, which recognizes all five boolean-operator
     * token types regardless of what the user configured; this set is used to
     * decide whether a given operator node should contribute to the complexity
     * count.
     */
    private Set<Integer> configuredTokens;
     ...
     *
     * @param treatUniformSimpleSequentialExpressionsAsOne whether to treat
     *     uniform operator chains as one.
     * @since 13.10.0
     */
    public void setTreatUniformSimpleSequentialExpressionsAsOne(
            boolean treatUniformSimpleSequentialExpressionsAsOne) {
        this.treatUniformSimpleSequentialExpressionsAsOne =
     ...
     * operator node, recursing into operands that are themselves qualifying
    ...
    private int childComplexity(DetailAST ast) {
        ...
        if (left != null && isBooleanOperatorType(left.getType())) {
            sum += complexityOf(left);
        }
        if (right != null && isBooleanOperatorType(right.getType())) {
            sum += complexityOf(right);
        }
        return sum;
    }
     ...
     * any leading {@code LPAREN} tokens.
     * returns {@code null} rather than throwing if no such operand exists.
     *
    ...
    private static String leafKey(DetailAST leaf) {
        ...
        if (isBooleanOperatorType(leaf.getType())) {
            ...
            final DetailAST lhs = leftOperand(leaf);
            final String canonical;
            if (lhs == null) {
                canonical = null;
            }
            else {
                canonical = canonicalText(lhs);
            }
            if (canonical == null) {
                key = null;
            }
    ...
    private static String canonicalText(DetailAST ast) {
        final String result;
        switch (ast.getType()) {
            case TokenTypes.IDENT -> result = ast.getText();
            case TokenTypes.DOT -> {
                final DetailAST left = leftOperand(ast);
                final DetailAST right = rightOperand(ast);
