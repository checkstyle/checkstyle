...
public class PreferCodeOrSnippetJavadocInlineTagCheck extends AbstractJavadocCheck {
    ...
    public static List<DetailNode> collectTextNodes(DetailNode node) {
        ...
        final List<DetailNode> textNodes = new ArrayList<>(JavadocUtil.getAllNodesOfType(rootNode,
                JavadocCommentsTokenTypes.TEXT));

        final List<DetailNode> inlineTags = new ArrayList<>(JavadocUtil.getAllNodesOfType(
                rootNode, JavadocCommentsTokenTypes.JAVADOC_INLINE_TAG));

        final Deque<DetailNode> htmlElements = new ArrayDeque<>(JavadocUtil.getAllNodesOfType(
