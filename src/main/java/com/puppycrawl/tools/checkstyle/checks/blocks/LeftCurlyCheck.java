...
public class LeftCurlyCheck
    ...
    /**
     * Checks if the given brace is a child of an block.
     *
    ...
    private static boolean isOnLineWithBlockPreviousToken(DetailAST brace) {
        ...
        if (!onSameLine) {
            for (DetailAST child = parent.getFirstChild();
                ...
                if (child.getType() == TokenTypes.LCURLY || child.getType() == TokenTypes.SLIST
                    || child.getType() == TokenTypes.OBJBLOCK) {
                    continue;
                }
                onSameLine = TokenUtil.areOnSameLine(child, brace);
                if (child.hasChildren()) {
                    onSameLine = TokenUtil.areOnSameLine(brace, child.getLastChild());
                }
                if (onSameLine) {
                    break;
                }
