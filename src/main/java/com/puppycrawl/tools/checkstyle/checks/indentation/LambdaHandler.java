...
public class LambdaHandler extends AbstractExpressionHandler {
    ...
    public void checkIndentation() {
        ...
        if (isSwitchRuleLambda
            ...
            // Likely a single-statement switch rule lambda, e.g.:
            // case ONE ->
            //      1;
     ...
     * when the statement is not on the same line as the lambda operator (`->`).
     * This applies to single-statement switch rules without curly braces `{}`.
     * Example:
     * <pre>
     * case ONE ->
     *     1; // Indentation checked here (relative to `->`)
     * </pre>
     *
     * @param isLambdaFirstInLine if "->" is the first element on the line
     */
    private void checkSingleStatementSwitchRuleIndentation(boolean isLambdaFirstInLine) {
        ...
        checkExpressionSubtree(mainAst.getNextSibling(), level, true, false);
    }
}
