...
public class SingleSpaceSeparatorCheck extends AbstractCheck {
    ...
    private static boolean isBlockCommentEnd(int[] line, int columnNo) {
        ...
        while (CommonUtil.isCodePointWhitespace(line, posOfWhitespace)) {
            posOfWhitespace--;
        }
        boolean isBlockCommentEnd = true;
        if (posOfWhitespace > 0) {
            isBlockCommentEnd = Arrays.equals(Arrays.copyOfRange(line, posOfWhitespace - 1,
                                    posOfWhitespace + 1), "*/".codePoints().toArray());
        }
