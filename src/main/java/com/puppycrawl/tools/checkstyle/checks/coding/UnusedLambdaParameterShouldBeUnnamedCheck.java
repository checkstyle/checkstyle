 ...
 * unnamed lambda parameters came out as the first preview in Java 21.
 * </p>
 * <p>
...
public class UnusedLambdaParameterShouldBeUnnamedCheck extends AbstractCheck {
    ...
    public void leaveToken(DetailAST ast) {
        if (ast.getType() == TokenTypes.LAMBDA) {
            ...
            while (!lambdaParameters.isEmpty()
                        && lambdaParameters.peek().enclosingLambda.equals(ast)) {
    ...
    private static boolean isLambdaParameterIdentifierCandidate(DetailAST identifierAst) {
        ...
        final boolean hasValidParentToken =
                !TokenUtil.isOfType(identifierAst.getParent(), INVALID_LAMBDA_PARAM_IDENT_PARENTS);

        final boolean isMethodInvocation = isMethodInvocation(identifierAst);

        return !isLambdaParameterDeclaration && (hasValidParentToken || isMethodInvocation);
    }
    ...
    private static final class LambdaParameterDetails {
        ...
        /**
         * Create a new instance of {@link LambdaParameterDetails}.
         *
