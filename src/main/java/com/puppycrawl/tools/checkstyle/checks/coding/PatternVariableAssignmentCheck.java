...
public class PatternVariableAssignmentCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        final List<DetailAST> patternVariableIdents = getArrayOfPatternVariableIdents(ast);
        final List<DetailAST> reassignedVariableIdents = getArrayOfReassignedVariableIdents(ast);

        for (DetailAST patternVariableIdent : patternVariableIdents) {
            for (DetailAST assignTokenIdent : reassignedVariableIdents) {
                if (patternVariableIdent.getText().equals(assignTokenIdent
                    .getText())) {

                    log(assignTokenIdent, MSG_KEY, assignTokenIdent
                        .getText());
                    break;
                }
    ...
    private static DetailAST getNeededAssignIdent(DetailAST assignToken) {
        ...
        while (traverseUntilNeededBranchType(assignIdent, assignToken.getFirstChild(),
            TokenTypes.IDENT) != null) {

            assignIdent = traverseUntilNeededBranchType(assignIdent, assignToken.getFirstChild(),
                TokenTypes.IDENT);
        }
     ...
     */
    private static List<DetailAST> getArrayOfPatternVariableIdents(DetailAST ast) {
        ...
        if (outermostPatternVariable != null) {
            patternVariablesArray.add(outermostPatternVariable.findFirstToken(TokenTypes.IDENT));
        }
            ...
            if (recordPatternComponents != null) {
                for (DetailAST innerPatternVariable = recordPatternComponents.getFirstChild();
                    ...
                    if (innerPatternVariable.getType() == TokenTypes.PATTERN_VARIABLE_DEF) {
                        patternVariablesArray.add(innerPatternVariable.findFirstToken(
                            TokenTypes.IDENT));
                    }
                    else {
                        patternVariablesArray.addAll(getArrayOfPatternVariableIdents(
     ...
     */
    private static List<DetailAST> getArrayOfReassignedVariableIdents(DetailAST ast) {
        ...
        if (branchLeadingToReassignedVar != null) {
            if (ASSIGN_TOKEN_TYPES.contains(branchLeadingToReassignedVar.getType())) {
                reassignedVariables.add(getNeededAssignIdent(branchLeadingToReassignedVar));
            }
            ...
            for (DetailAST expressionBranch = branchLeadingToReassignedVar;
                ...
                if (assignToken != null) {
                    reassignedVariables.add(getNeededAssignIdent(assignToken));
                }
    ...
    private static DetailAST getBranchLeadingToReassignedVars(DetailAST ast) {
        ...
        for (DetailAST conditionalStatement = ast;
            ...
            if (conditionalStatement.getType() == TokenTypes.LITERAL_IF
                ...
                leadingToReassignedVarBranch = getMatchedAssignToken(conditionalStatement);
            }
        }
