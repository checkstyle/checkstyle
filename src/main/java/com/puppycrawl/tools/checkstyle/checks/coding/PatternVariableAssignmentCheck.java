...
public class PatternVariableAssignmentCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        for (DetailAST patternVariable : patternVariables) {
            ...
            for (DetailAST assignToken : reassignedVariables) {
    ...
    private static DetailAST getNeededAssignIdent(DetailAST assignToken) {
        DetailAST assignIdent = null;

        for (DetailAST iteratedBranch = assignToken;
             null != iteratedBranch;
             iteratedBranch = traverseUntilNeededBranchType(iteratedBranch,
                 assignToken.getFirstChild(), TokenTypes.IDENT)) {
     ...
     */
    private static List<DetailAST> getArrayOfPatternVariables(DetailAST ast) {

        final DetailAST outermostPatternVariable = ast.findFirstToken(
            TokenTypes.PATTERN_VARIABLE_DEF);

        final DetailAST recordPatternDef;
        if (TokenTypes.LITERAL_INSTANCEOF == ast.getType()) {
     ...
     */
    private static List<DetailAST> getArrayOfReassignedVariables(DetailAST ast) {
    ...
    private static DetailAST shiftToNextTraversedBranch(DetailAST ast, DetailAST boundAst) {
        ...
        if (null != ast.getFirstChild()) {
            ...
            while (null == newAst.getNextSibling() && !newAst.equals(boundAst)) {
    ...
    private static DetailAST getBranchLeadingToReassignedVars(DetailAST ast) {
        DetailAST conditionalStatement = ast;

        while (null != conditionalStatement
            && TokenTypes.LITERAL_IF != conditionalStatement.getType()
            && TokenTypes.LITERAL_ELSE != conditionalStatement.getType()
            && TokenTypes.QUESTION != conditionalStatement.getType()) {
        ...
        if (null != leadingToReassignedVarBranch) {
            if (TokenTypes.LITERAL_IF == conditionalStatement.getType()
                || false) {
