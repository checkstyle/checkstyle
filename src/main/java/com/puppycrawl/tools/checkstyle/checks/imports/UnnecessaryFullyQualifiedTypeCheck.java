...
public class UnnecessaryFullyQualifiedTypeCheck extends AbstractCheck {
    ...
    /**
     * Creates a new {@code UnnecessaryFullyQualifiedTypeCheck} instance.
     */
    ...
    private void processDot(DetailAST ast) {
        if (isInTypeContext(ast)) {
            final String fullName =
                    FullIdent.createFullIdent(ast).getText().replace(ARRAY_MARKERS, "");
    ...
    private boolean isUnnecessary(String simpleName) {
        ...
        for (QualifiedTypeReference reference : qualifiedReferences) {
            if (reference.simpleName().equals(simpleName)) {
                final String fullName = reference.fullName();
                identities.add(fullName);
                if (resolvesUnqualified(fullName)) {
                    unqualifiedResolvedToReference = true;
                }
            }
        }
        ...
        // A type referenced by its simple name with no import resolves to a
        // same-package or java.lang type (or, when present, the locally declared
        // type already recorded above). That type is distinct from this fully
        // qualified reference and thus requires it to be qualified, unless this
        // reference is itself that same-package or java.lang type (in which case
        // the qualification is simply redundant).
        if (imported == null && !unqualifiedResolvedToReference
