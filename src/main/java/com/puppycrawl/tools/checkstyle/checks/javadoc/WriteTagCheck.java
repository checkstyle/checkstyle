...
public class WriteTagCheck extends AbstractJavadocCheck {
    ...
    private final ThreadLocal<Boolean> tagFound = ThreadLocal.withInitial(() -> Boolean.FALSE);

    /** Parent line number for the missing tag report in the current Javadoc tree. */
    private final ThreadLocal<Integer> parentLineNo = ThreadLocal.withInitial(() -> 0);
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (javadocComment != null) {
            parentLineNo.set(ast.getLineNo());
    ...
    public void finishJavadocTree(DetailNode rootAst) {
        if (tag != null && !tagFound.get()) {
            log(parentLineNo.get(), MSG_MISSING_TAG, tag);
        }
    }
    ...
    private static DetailAST findJavadoc(DetailAST ast) {
        ...
        if (cmt == null) {
            ...
            DetailAST annotation = null;
            if (modifiers != null) {
                annotation = modifiers.findFirstToken(TokenTypes.ANNOTATION);
