...
public class CommentsIndentationCheck extends Check
{
    ...
    private void visitSingleLineComment(DetailAST singleLineComment)
    {
        ...
        if (nextStatement.getType() != TokenTypes.RCURLY
    ...
    {
        final int commentLineNo = singleLineComment.getLineNo();
        final String lineWithComment = getFileContents().getText().get(commentLineNo - 1);
        final String commentedLineRegex = "^.*\\S.+/{2}.*$";
        return lineWithComment.matches(commentedLineRegex);
    }
    ...
    {
        final int commentLineNo = blockComment.getLineNo();
        final String line = getFileContents().getText().get(commentLineNo - 1);
        final String commentedLineRegex =
                 "^(.*\\s+/\\*.*\\*/.*\\S+.*)|(.*\\S+.*/\\*.*\\*/\\s{0,})$";
        return line.matches(commentedLineRegex);
    }
