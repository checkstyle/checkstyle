...
public class VariableDeclarationUsageDistanceCheck extends AbstractCheck {
    ...
    private static boolean isVariableUsedInInitializationSequence(
        ...
        boolean isVarNotInInitializationSequence = true;
            ...
            switch (currentSiblingAst.getType()) {
                case TokenTypes.EXPR:
                    if (currentSiblingAst.getFirstChild().getType() == TokenTypes.METHOD_CALL) {
                        ...
                        isVarNotInInitializationSequence = isVarNotInInitializationSequence
                            && !isVariableUsed(variableName, methodCallAst);
        ...
        if (variableUsageAst != null) {
            ans = result && !isVarNotInInitializationSequence;
        }
        return ans;
    }
    ...
    private static boolean isVariableUsed(String variableName, DetailAST ast) {
        ...
        while (curNode.getType() != TokenTypes.SEMI) {
            if (curNode.getText().equals(variableName)) {
                bool = true;
                break;
            }
