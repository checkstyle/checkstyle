...
public class VariableDeclarationUsageDistanceCheck extends AbstractCheck {
    ...
    private static List<DetailAST> getVariableUsageExpressionsInsideSwitchBlock(DetailAST block,
                                                                            DetailAST variable) {
        final Optional<DetailAST> firstToken = TokenUtil.findFirstTokenByPredicate(block, (DetailAST child) -> {
        ...
        firstToken.ifPresent((DetailAST token) -> {
            TokenUtil.forEachChild(block, token.getType(), (DetailAST child) -> {
