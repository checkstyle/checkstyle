...
public class MethodLengthCheck extends AbstractCheck {
    ...
    private int getLengthOfBlock(DetailAST openingBrace, DetailAST closingBrace) {
        ...
        if (!countEmpty
            ...
            length = countUsedLines(openingBrace.getFirstChild(), startLineNo);
        }
        return length;
    }
     ...
     */
    private static int countUsedLines(DetailAST ast, int startLine) {
        ...
        while (!nodes.isEmpty()) {
            ...
            if (node.getType() == TokenTypes.TEXT_BLOCK_CONTENT) {
                ...
                IntStream.range(1, contentLinesCount - 1)
                    .map(idx -> currNodeLineNumber + idx)
