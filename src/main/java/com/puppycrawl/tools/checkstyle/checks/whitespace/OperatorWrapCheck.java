...
public class OperatorWrapCheck
    ...
    public void visitToken(DetailAST ast) {
        if (isTargetNode(ast)) {
            if (option == WrapOption.NL && isNewLineViolation(ast)) {
                log(ast, MSG_LINE_NEW, ast.getText());
            }
            else if (option == WrapOption.EOL && isEndOfLineViolation(ast)) {
    ...
    /**
     * Checks whether operator violates {@link WrapOption#NL} style.
     *
     * @param ast the DetailAst of an operator
     * @return {@code true} if style violated
     */
    private static boolean isNewLineViolation(DetailAST ast) {
        return TokenUtil.areOnSameLine(ast, getLeftNode(ast))
                && !TokenUtil.areOnSameLine(ast, getRightNode(ast));
    }

    /**
     * Checks whether operator violates {@link WrapOption#EOL} style.
     *
     * @param ast the DetailAst of an operator
     * @return {@code true} if style violated
     */
    private static boolean isEndOfLineViolation(DetailAST ast) {
        return !TokenUtil.areOnSameLine(ast, getLeftNode(ast));
    }
