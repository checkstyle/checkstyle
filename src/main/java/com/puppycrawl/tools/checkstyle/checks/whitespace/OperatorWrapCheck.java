...
public class OperatorWrapCheck
    ...
    public void visitToken(DetailAST ast) {
        ...
        // and star in star imports
        if (!TokenUtil.isOfType(parent,
                TokenTypes.LITERAL_DEFAULT, TokenTypes.LITERAL_CASE, TokenTypes.DOT)) {
    ...
    /**
     * Checks if a node is assign to variable expression.
     *
     * @param node the node to check
     * @return {@code true} if node matches
     */
    private static boolean isAssignToVariable(DetailAST node) {
        return node.getType() == TokenTypes.ASSIGN
            && TokenUtil.isOfType(node.getParent(), TokenTypes.VARIABLE_DEF, TokenTypes.RESOURCE);
    }
     ...
     * grandchild of the right child or sibling.
     *
     * @param node the binary operator
    ...
    private static DetailAST getRightNode(DetailAST node) {
        ...
        if (node.getLastChild() == null) {
            ...
            result = adjustParens(node.getLastChild(), DetailAST::getPreviousSibling);
        }
     ...
     */
    private static DetailAST adjustParens(DetailAST node, Function<DetailAST, DetailAST> step) {
