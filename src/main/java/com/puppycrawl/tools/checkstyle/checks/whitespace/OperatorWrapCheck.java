...
public class OperatorWrapCheck
    ...
    /** Precedence level for shift operators ({@code <<}, {@code >>}, {@code >>>}). */
    private static final int PRECEDENCE_SHIFT = 4;

    /** Precedence level for relational operators ({@code <}, {@code >}, {@code <=}, {@code >=}, instanceof). */
    ...
    /** Precedence level for bitwise AND ({@code &}). */
    ...
    /** Precedence level for logical AND ({@code &&}). */
