...
public class NoWhitespaceBeforeCaseDefaultColonCheck
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (isColonOnDifferentLine(ast, isColonOfCase, isColonOfDefault)) {
     ...
     *
     * @param colonAst DetailAST to check.
     * @param isColonOfCase boolean parameter representing if the colon is after case.
     * @param isColonOfDefault boolean parameter representing if the colon is after default.
     * @return true, if colon case is in different line as of case or default.
     */
    private static boolean isColonOnDifferentLine(DetailAST colonAst, boolean isColonOfCase,
                                      boolean isColonOfDefault) {
        boolean sameLine = true;
        if (isColonOfCase) {
            final DetailAST previousSibling = colonAst.getPreviousSibling();
            sameLine = TokenUtil.areOnSameLine(previousSibling, colonAst);
        }
        else if (isColonOfDefault) {
            final DetailAST literalDefault = colonAst.getParent();
            sameLine = TokenUtil.areOnSameLine(literalDefault, colonAst);
        }
        return !sameLine;
    }
