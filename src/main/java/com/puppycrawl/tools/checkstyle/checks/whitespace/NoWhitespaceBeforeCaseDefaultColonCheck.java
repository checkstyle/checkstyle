...
public class NoWhitespaceBeforeCaseDefaultColonCheck
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (astIsColonOfCase || astIsColonOfDefault) {
            final boolean onSameLine = TokenUtil.areOnSameLine(parent, ast);
            boolean whitespaceDetected = false;
            if (onSameLine) {
                if (astIsColonOfCase) {
                    whitespaceDetected = whitespaceDetectedBeforeColonOfCase(ast);
                }
                else {
                    whitespaceDetected = whitespaceDetectedBeforeColonOfDefault(ast);
                }
            }

            if (!onSameLine || whitespaceDetected) {
    ...
    /**
     * Checks if there's a whitespace before the colon of a switch case.
     * @param colonAst DetailAST to check.
     * @return whether there is a whitespace before colonAst or not.
     */
    private boolean whitespaceDetectedBeforeColonOfCase(DetailAST colonAst) {
        final boolean whitespaceDetected;
        final DetailAST previousSibling = colonAst.getPreviousSibling();
        if (previousSibling.getLastChild().getType() == TokenTypes.BLOCK_COMMENT_END) {
            whitespaceDetected = colonAst.getColumnNo() != getLastColumnOf(previousSibling) + 1;
        }
        else {
            whitespaceDetected = colonAst.getColumnNo() != getLastColumnOf(previousSibling);
        }
        return whitespaceDetected;
    }

    /**
     * Checks if there's a whitespace before the colon of a switch default.
     * @param colonAst DetailAST to check.
     * @return whether there is a whitespace before colonAst or not.
     */
    private boolean whitespaceDetectedBeforeColonOfDefault(DetailAST colonAst) {
        final boolean whitespaceDetected;
        final DetailAST previousSibling = colonAst.getPreviousSibling();
    ...
    /**
     * Return the last column of an ast.
     * @param ast DetailAST to check.
     * @return ast's last column.
     */
    private int getLastColumnOf(DetailAST ast) {
