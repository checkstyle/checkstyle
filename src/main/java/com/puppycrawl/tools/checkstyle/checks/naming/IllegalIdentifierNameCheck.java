 ...
 * that are restricted or contextual keywords. Examples include "yield", "record", "_", "var" and
 * "when". Please read more at
 ...
 * Default value is {@code "(?i)^(?!(record|yield|var|permits|sealed|when|_)$).+$"}.
 * </li>
 * <li>
...
public class IllegalIdentifierNameCheck extends AbstractNameCheck {
    ...
    public IllegalIdentifierNameCheck() {
        super("(?i)^(?!(record|yield|var|permits|sealed|when|_)$).+$");
    }
