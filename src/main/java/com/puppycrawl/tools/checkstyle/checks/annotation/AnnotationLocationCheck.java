...
public class AnnotationLocationCheck extends AbstractCheck {
     ...
     *
     * @param annotatedNodeAst node being visited.
     ...
     */
    private static DetailAST getAnnotationParent(DetailAST annotatedNodeAst) {
        DetailAST node = annotatedNodeAst.findFirstToken(TokenTypes.MODIFIERS);
        if (node == null) {
            node = NullUtil.notNull(annotatedNodeAst.findFirstToken(TokenTypes.ANNOTATIONS));
        }
        return node;
    }
