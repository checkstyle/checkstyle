...
public class UnnecessaryParenthesesCheck extends AbstractCheck {
    ...
    private static boolean unnecessaryParenAroundOperators(DetailAST ast) {
        ...
        if (isConditional) {
            result = checkConditionalOperator(ast);
        }
        else if (isBitwise) {
            result = checkBitwiseBinaryOperator(ast);
        }
        else {
            result = (TokenUtil.isOfType(ast.getParent().getType(), CONDITIONALS_AND_RELATIONAL)
                    || TokenUtil.isOfType(ast.getParent().getType(), BITWISE_BINARY_OPERATORS))
     ...
     */
    private static boolean checkConditionalOperator(DetailAST ast) {
        final int type = ast.getType();
        ...
        return result && !TokenUtil.isOfType(parentType, TokenTypes.EQUAL, TokenTypes.NOT_EQUAL)
                && (TokenUtil.isOfType(parentType, CONDITIONALS_AND_RELATIONAL)
                || TokenUtil.isOfType(parentType, BITWISE_BINARY_OPERATORS));
    }
