...
public class UnnecessaryParenthesesCheck extends AbstractCheck {
    ...
    private static List<DetailAST> getParenthesesChildrenAroundQuestion(DetailAST questionToken) {
        final List<DetailAST> allChildren = new ArrayList<>();
        ...
        for (DetailAST child : allChildren) {
            if (child.getType() == TokenTypes.LPAREN
                    && !TokenUtil.isOfType(child.getNextSibling(), LITERALS)) {
                surroundedChildren.add(child);
            }
        }
        return surroundedChildren;
    }
