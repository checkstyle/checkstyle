...
/**
 * Checks for a blank line at the end of a code block.
 *
 * <p> By default this check validates presence of a blank line before the right curly brace.
 * To configure this check to validate absence of any blank lines at the end of the code block,
 ...
 * AfterLeftCurlyCheck if they are configured in the opposite way.
 * </p>
 *
 ...
 * </pre>
 *
 * <p> Example of declarations with a blank line at the end of the block that is expected
 ...
 *
 * <p>
 * An example how to allow no blank lines before the right curly brace:
 * </p>
 *
 * <pre>
 * &lt;module name="BeforeRightCurly"&gt;
 ...
 */
@StatelessCheck
public class BeforeRightCurlyCheck extends AbstractCheck {

    /**
     * A key is pointing to the warning message blank.line.before.rcurly
     * in "messages.properties" file.
     */
    public static final String MSG_SHOULD_BE_BLANK_LINE_BEFORE_RCURLY = "blank.line.before.rcurly";

    /**
     * A key is pointing to the warning message no.blank.line.before.rcurly
     * in "messages.properties" file.
     */
    public static final String MSG_SHOULD_BE_NO_BLANK_LINE_BEFORE_RCURLY =
        "no.blank.line.before.rcurly";

    /** The policy to enforce presence or absence of a blank line at the end of the block. */
    private boolean blankLine = true;

    /**
     * Sets the policy to enforce presence or absence of a blank line the end of the block.
    ...
    public void visitToken(DetailAST ast) {
        final DetailAST rightCurly = findRightCurly(ast);
        if (rightCurly != null) {
            final DetailAST previousSibling = rightCurly.getPreviousSibling();
            final boolean isEmpty = previousSibling == null
                || previousSibling.getType() == TokenTypes.LCURLY;

            if (!isEmpty && blankLine != hasBlankLineBefore(rightCurly)) {
                if (blankLine) {
                    log(rightCurly, MSG_SHOULD_BE_BLANK_LINE_BEFORE_RCURLY,
                        rightCurly.getText());
                }
                else {
                    log(rightCurly, MSG_SHOULD_BE_NO_BLANK_LINE_BEFORE_RCURLY,
                        rightCurly.getText());
                }
            }
        }
    ...
    /**
     * Verifies that there is a blank line before the token.
     * @param ast the token to process
     * @return {@code true} if found no other tokens before the {@code ast} on the same line
     *     and a blank line preceding it, {@code false} otherwise
     */
    private boolean hasBlankLineBefore(DetailAST ast) {
        final FileContents contents = getFileContents();
        final String line = contents.getLine(ast.getLineNo() - 1);
        final boolean blankBefore = CommonUtils.isBlank(line.substring(0, ast.getColumnNo()));
        return blankBefore && contents.lineIsBlank(ast.getLineNo() - 2);
    }
