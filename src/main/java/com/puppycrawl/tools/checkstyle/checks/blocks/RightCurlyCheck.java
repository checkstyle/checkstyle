...
public class RightCurlyCheck extends AbstractCheck {
    ...
    private static boolean skipDoubleBraceInstInit(Details details) {
        ...
        if (tokenAfterNextToken != null) {
            ...
            skipDoubleBraceInstInit = rcurly.orElse(null).getParent().getParent()
                    ...
                    && rcurly.get().getLineNo() != Details.getNextToken(tokenAfterNextToken)
                        .getLineNo();
        }
    ...
    private static boolean isBlockAloneOnSingleLine(Details details) {
        ...
        if (nextToken != null) {
            ...
            isBlockAloneOnSingleLine = TokenUtil.areOnSameLine(rcurly.orElse(null),
                    lcurly.orElse(null))
                && (!TokenUtil.areOnSameLine(rcurly.orElse(null), nextToken)
