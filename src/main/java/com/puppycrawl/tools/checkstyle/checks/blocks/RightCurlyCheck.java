...
public class RightCurlyCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (rcurly != null) {
            ...
            if (!violation.isEmpty()) {
                if (violation.equals(MSG_KEY_LINE_BREAK_AFTER)) {
                    final DetailAST semiColon = Details.getNextToken(rcurly);
                    log(semiColon, violation, ";", semiColon.getColumnNo() + 1);
                }
                else {
    ...
    private static boolean shouldBeAloneOnLine(RightCurlyOption bracePolicy,
        ...
        return bracePolicy == RightCurlyOption.ALONE
                ...
                || (bracePolicy == RightCurlyOption.ALONE_OR_SINGLELINE
                    ...
                    && !checkIfItIsAnonymousInnerClass(details);
    }

    /**
     * Checks whether the right curly is a part of anonymous inner class declaration.
     *
     * @param details details for validation.
     * @return true, if right curly is a part of the anonymous inner class
     */
    private static boolean checkIfItIsAnonymousInnerClass(Details details) {

        boolean checkIfItIsAnonymousInnerClass = false;

        if (details.nextToken != null) {
            final DetailAST parentNode = details.nextToken.getParent();
            final DetailAST objParentNode = details.rcurly.getParent();
            final DetailAST semiColon = details.nextToken;
            checkIfItIsAnonymousInnerClass = semiColon.getType() == TokenTypes.SEMI
                                             && objParentNode.getType() == TokenTypes.OBJBLOCK
                                             && parentNode.getType() == TokenTypes.VARIABLE_DEF;
        }

        return checkIfItIsAnonymousInnerClass;
    }
    ...
    private static boolean isRightcurlyFollowedBySemicolonHasLineBreakAfter(RightCurlyOption
                                                                              bracePolicy,
                                                                              Details details) {
        final DetailAST tokenAfterTheNextToken = Details.getNextToken(details.nextToken);
    ...
    private static final class Details {
        ...
        private static Details getDetailsForAnonymousInnerClass(DetailAST ast) {
            ...
            final boolean checkStatusOfAncestor = checkIfAncestorIsVariableDef(ast);

            if (checkStatusOfAncestor) {
                rcurly = ast.getLastChild();
                lcurly = ast.getFirstChild();
        ...
        /**
         * Checks whether VARIABLE_DEF is an ancestor of OBJBLOCK and nextToken is SEMI.
         *
         * @param ast a {@code DetailAST} value
         * @return true if VARIABLE_DEF is ancestor of OBJBLOCK
         */
        private static boolean checkIfAncestorIsVariableDef(DetailAST ast) {
            boolean conditionChecker = false;

            final DetailAST nextToken = Details.getNextToken(ast);
            DetailAST ancestorVariableDef = null;

            if (nextToken != null) {
                ancestorVariableDef = nextToken.getParent();
                conditionChecker = TokenTypes.VARIABLE_DEF == ancestorVariableDef.getType()
                                    && TokenTypes.SEMI == nextToken.getType();
            }
