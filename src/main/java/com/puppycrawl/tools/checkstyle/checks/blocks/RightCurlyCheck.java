...
public class RightCurlyCheck extends AbstractCheck {
    ...
    private static final class Details {
        ...
        private static Details getDetailsForCase(DetailAST caseNode) {
            ...
            // case group may have SLIST but not lcurly
            final Optional<DetailAST> caseGroupLcurly = Optional.ofNullable(ast)
                    .filter(node -> node.getFirstChild().getType() == TokenTypes.SLIST)
                    .map(DetailAST::getFirstChild);

            final Optional<DetailAST> switchRuleLcurly = Optional.ofNullable(ast)
                    .filter(node -> parentType == TokenTypes.SWITCH_RULE);

            final DetailAST lcurly = caseGroupLcurly.orElse(switchRuleLcurly.orElse(null));

            DetailAST rcurly = null;
            if (lcurly != null && !isSwitchExpression(caseParent)) {
