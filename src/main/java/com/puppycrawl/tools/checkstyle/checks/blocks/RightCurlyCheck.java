...
public class RightCurlyCheck extends AbstractCheck {
    ...
    private static final class Details {
        ...
        private static Details getDetailsForSwitch(DetailAST switchNode) {
            ...
            final DetailAST nextToken = getNextToken(switchNode);
        ...
        private static DetailAST getNextToken(DetailAST ast) {
            ...
            while (next == null && parent != null) {
                ...
                if (next != null && next.getType() == TokenTypes.EMPTY_STAT
                        && ast.getType() == TokenTypes.LITERAL_SWITCH) {
                    next = next.getNextSibling();
                }
                if (next != null && ast.getType() == TokenTypes.LITERAL_SWITCH
                        && (next.getType() == TokenTypes.RPAREN
                        || next.getType() == TokenTypes.SEMI)) {
                    next = parent;
                }
