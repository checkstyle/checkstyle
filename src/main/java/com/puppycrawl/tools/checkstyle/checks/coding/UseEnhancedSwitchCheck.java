...
public class UseEnhancedSwitchCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        // If we find no CASE_GROUPs, either the switch is empty or already enhanced.
        // Otherwise we can convert it as long as all case groups terminate.
     ...
     */
    private boolean allCaseGroupsTerminate(List<DetailAST> caseGroups) {
        boolean allCaseGroupsTerminate = true;
        // Check all case groups except the last one for fall-through.
        // If the last case group falls through, it can still be converted.
        for (int index = 0; index < caseGroups.size() - 1; index++) {
            ...
            // Note: statementList can never be null, as a non-last case group always has a
            // statement list, or it would be grouped with the next case group.
            if (!FallThroughCheck.isTerminated(statementList, true, true,
                    new HashSet<>())) {
                // Found a case group that doesn't terminate, so the switch cannot be converted.
