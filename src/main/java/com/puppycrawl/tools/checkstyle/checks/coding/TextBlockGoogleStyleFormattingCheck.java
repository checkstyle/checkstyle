...
public class TextBlockGoogleStyleFormattingCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        final int[] indentationErrorLineAndColumn = textBlockIsIndented(ast);
        if (indentationErrorLineAndColumn[0] != -1) {
            log(indentationErrorLineAndColumn[0],
                    indentationErrorLineAndColumn[1],
    ...
    /**
     * Determine if the Text Block content indentation is equal or more than
     * opening quotes indentation.
     *
     * @param openingQuotes openingQuotes
     * @return {-1, -1} if the text block contents are correctly indented.
     */
    private static int[] textBlockIsIndented(DetailAST openingQuotes) {
        final int quoteIndent = openingQuotes.getColumnNo();
        final DetailAST textAst = openingQuotes.getFirstChild();
        final String content = textAst.getText();

        final String[] lines = content.split("\n", -1);
        int currentLine = openingQuotes.getLineNo() + 1;
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (indentation < quoteIndent) {
                return new int[]{currentLine, indentation};
            }
            currentLine++;
        }
        return new int[]{-1, -1};
