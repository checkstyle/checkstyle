...
public class ConstructorsDeclarationGroupingCheck extends AbstractCheck {
    ...
    /**
     * Specifies parent's scope.
     */
    private final Map<DetailAST, Integer> parents = new HashMap<>();
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (parents.containsKey(parent)) {
            ...
            if (!isCtor && !isCompactCtor) {
                final int previousCtorLineNo = parents.get(parent);
                log(ast, MSG_KEY, previousCtorLineNo);
            }

            parents.put(parent, ast.getLineNo());
        }
        else {
            parents.put(parent, ast.getLineNo());
        }
