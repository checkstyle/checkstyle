 ...
 * </ol>
 *
 * <p>Inside each member group, fields come first, then constructors, then
 * methods. Overloaded constructors and methods with the same name must be
 * grouped together and ordered by the number of parameters, with the fewest
 * parameters first.</p>
 *
 ...
 * the correct order. References using the {@code Type##fragment} syntax to
 ...
 * or member references and cannot be meaningfully compared to one.</p>
 *
 * @since 14.2.0
 */
@FileStatefulCheck
public class JavadocSeeTagOrderCheck extends AbstractJavadocCheck {
    ...
    /** Number of {@link Kind} values, used to combine category and kind into one key. */
    private static final int KIND_COUNT = 3;

    /** Token types that represent a type declaration. */
    private static final BitSet TYPE_DEFINITION_TOKENS = TokenUtil.asBitSet(
    ...
    public void beginJavadocTree(DetailNode rootAst) {
        ...
        enclosingTypeName = findEnclosingTypeName(getBlockCommentAst());
    }

    /**
     * Finds the simple name of the type declaration that most closely encloses the
     * given block comment, so that local {@code @see #Name()} references can be
     * recognized as referring to a constructor of that type.
     *
     * @param commentBlock the block comment to start searching from
     * @return the enclosing type's simple name, or an empty string if none is found
     */
    private static String findEnclosingTypeName(DetailAST commentBlock) {
        DetailAST current = commentBlock;
        while (current != null && !isTypeDefinition(current)) {
    ...
    /**
     * Checks whether the given node is a type declaration.
     *
     * @param ast the node to check
     * @return {@code true} if the node is a class, interface, enum, record, or
     *     annotation declaration
     */
    private static boolean isTypeDefinition(DetailAST ast) {
        return TYPE_DEFINITION_TOKENS.get(ast.getType());
    }

    @Override
    public void visitJavadocToken(DetailNode ast) {
        final SeeReference current = SeeReference.from(ast, enclosingTypeName);
        if (current != null) {
            if (maxReference == null) {
                maxReference = current;
            }
            else if (isStructuralViolation(current, maxReference)) {
                log(ast, MSG_KEY, current.text(), maxReference.text());
            }
            else if (isTelescopingViolation(current)) {
                log(ast, MSG_KEY, current.text(), lastByName.get(lastByKey(current)).text());
            }
            else if (isGroupingViolation(current)) {
                log(ast, MSG_KEY, current.text(), previousReference.text());
            }
     ...
     * (category and field-before-constructor-before-method).
     *
    ...
    private boolean isGroupingViolation(SeeReference current) {
        return lastByName.containsKey(lastByKey(current))
                && previousReference.kind() == current.kind()
    ...
    /**
     * Kind of a member {@code @see} reference, ordered from the closest to the most
     * distant access. Type and package references are not members and are always
     * classified as {@link #METHOD}, since that kind never needs to be compared
    ...
    private enum Kind {
        ...
        CONSTRUCTOR(1),
        /** A method or type/package reference such as {@code #getName()}. */
        METHOD(2);
     ...
     *
     * @param category category that defines the primary structural ordering
     * @param kind whether the reference is a field, constructor, or method
     * @param qualifier owning type name, or an empty string for local and
     *     non-member references
     * @param name simple member or type name used for grouping
     * @param parameterCount number of parameters, used to order overloaded methods
     * @param text full reference text used for messages
     */
    private record SeeReference(Category category, Kind kind, String qualifier,
            String name, int parameterCount, String text) {
         ...
         * @param enclosingTypeName simple name of the type that most closely encloses
         *     the Javadoc comment, used to recognize local constructor references
         * @return the parsed reference, or {@code null} if it cannot be classified
         */
        private static SeeReference from(DetailNode seeBlock, String enclosingTypeName) {
            ...
            if (reference != null && !isFragmentReference(reference)) {
                ...
                if (firstType == JavadocCommentsTokenTypes.HASH) {
                    ...
                    result = parseMember(member, "", Category.LOCAL, enclosingTypeName);
                }
                else {
        ...
        /**
         * Checks whether the given reference uses the {@code Type##fragment} syntax to
         * link to a named fragment within a page, recognizable by two consecutive hash
         * signs among its children. Such references are not javadoc type or member
         * references and cannot be meaningfully compared to one.
         *
         * @param reference the reference node
         * @return {@code true} if the reference contains two consecutive hash signs
         */
        private static boolean isFragmentReference(DetailNode reference) {
            boolean sawHash = false;
            boolean result = false;
            DetailNode child = reference.getFirstChild();
            while (child != null) {
                if (child.getType() == JavadocCommentsTokenTypes.HASH) {
                    if (sawHash) {
                        result = true;
                        break;
                    }
        ...
        private static SeeReference parseIdentifierReference(DetailNode reference,
            ...
            if (member != null) {
                ...
                result = parseMember(member, typeName, category, lastIdentifier(typeName));
            }
                ...
                result = new SeeReference(category, Kind.METHOD, "",
                        lastIdentifier(typeName), 0, typeName);
            }
            else if (typeName.indexOf(DOT) != -1 && isPackageReference(typeName)) {
                result = new SeeReference(Category.PACKAGE, Kind.METHOD, "",
                        typeName, 0, typeName);
            }
            return result;
        }
         ...
         * @param ownerSimpleName simple name of the owning type, compared against the
         *     member name to recognize constructor references
         * @return the parsed member reference
         */
        private static SeeReference parseMember(DetailNode member, String qualifier,
                Category category, String ownerSimpleName) {
            ...
            final Kind kind;
            if (callable) {
                if (memberName.equals(ownerSimpleName)) {
            ...
            return new SeeReference(category, kind, qualifier, memberName,
        ...
        /**
         * Returns the structural ordering key combining the category and, for member
         * references, whether the reference is a field, constructor, or method.
         *
         * @return the structural ordering key
         */
        /* package */ int structuralKey() {
            return category.order() * KIND_COUNT + kind.order();
        }
