...
public class JavadocSeeTagOrderCheck extends AbstractJavadocCheck {
    ...
    private boolean isGroupingViolation(SeeReference current) {
        return lastByName.containsKey(lastByKey(current))
                && previousReference.field() == current.field()
     ...
     * @param field whether the reference points to a field (has no parameters)
     ...
     */
    private record SeeReference(Category category, boolean field, String qualifier,
        ...
        /* package */ int structuralKey() {
            ...
            return (category.order() << 1) + kindOffset;
        }
