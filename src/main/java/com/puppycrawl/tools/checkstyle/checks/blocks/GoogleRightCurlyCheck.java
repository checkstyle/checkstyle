 ...
 * </p>
 * <p>
 * For empty blocks, either {@code K&R} style or the concise {@code {}} form is
...
public class GoogleRightCurlyCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        switch (ast.getType()) {
            case TokenTypes.CLASS_DEF, TokenTypes.INTERFACE_DEF,
                 TokenTypes.ANNOTATION_DEF, TokenTypes.RECORD_DEF, TokenTypes.ENUM_DEF -> {
    ...
    private void checkRightBrace(DetailAST currentBlock, DetailAST brace) {
        ...
        final boolean hasContentAround = contentAround(brace, nextToken);
        if (nextToken != null && isPartOfMultiBlock(currentBlock, nextToken)) {
            ...
            verifyEmptyBlock(brace, nextToken);
        }
    ...
    private void verifyEmptyBlock(DetailAST brace, @Nullable DetailAST nextToken) {
        if (isNotConcise(brace)) {
            logViolations(MSG_KEY_CONCISE_BLOCK, brace);
                ...
                && hasContentOnRightSide(brace, nextToken)) {
    ...
    private static boolean hasContentOnLeftSide(DetailAST brace) {
        ...
        return TokenUtil.areOnSameLine(brace, previousToken)
                && !TokenUtil.isOfType(brace.getPreviousSibling(),
                TokenTypes.ENUM_CONSTANT_DEF, TokenTypes.COMMA);
    }
    ...
    private static boolean hasContentOnRightSide(DetailAST brace, DetailAST nextToken) {
        final boolean nextIsValid = TokenUtil.isOfType(nextToken,
                TokenTypes.SEMI, TokenTypes.RPAREN, TokenTypes.COMMA)
                && TokenUtil.isOfType(nextToken.getPreviousSibling(), TokenTypes.EXPR,
                TokenTypes.VARIABLE_DEF, TokenTypes.ELIST, TokenTypes.LAMBDA);
        return !nextIsValid && TokenUtil.areOnSameLine(brace, nextToken);
    }
    ...
    /**
     * Checks if the block is not {@code K&R} style or concise {@code {}}.
     *
