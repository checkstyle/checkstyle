...
public class GoogleRightCurlyCheck extends AbstractCheck {
    ...
    private void logViolations(String message, DetailAST brace) {
        if (MSG_KEY_CONCISE_BLOCK.equals(message)) {
            DetailAST leftCurly = brace.getPreviousSibling();
            if (leftCurly == null) {
                leftCurly = brace.getParent();
            }
            log(leftCurly, message);
        }
        else {
    ...
    private void checkRightBrace(DetailAST currentBlock, DetailAST brace) {
        ...
        final boolean notAloneOnLine = isNotAloneOnLine(currentBlock, brace, nextToken);
     ...
     */
    private void verifyEmptyBlock(DetailAST currentBlock, DetailAST brace, DetailAST nextToken) {
    ...
    /**
     * Checks if the right curly is not alone on the line.
     *
     * @param currentBlock the current block
     * @param brace the right curly brace
     * @param nextToken the next token after right curly
     * @return {@code true} if right curly is not alone on the line.
     */
    private boolean isNotAloneOnLine(DetailAST currentBlock, DetailAST brace,
        DetailAST nextToken) {
     ...
     */
    private boolean isEmpty(DetailAST brace) {
     ...
     */
    private boolean isNotConcise(DetailAST brace) {
        DetailAST previousSibling = brace.getPreviousSibling();
        if (previousSibling == null) {
            previousSibling = brace.getParent();
        }
        return previousSibling.getLineNo() != brace.getLineNo()
                || previousSibling.getColumnNo() + 1 != brace.getColumnNo();
    }
