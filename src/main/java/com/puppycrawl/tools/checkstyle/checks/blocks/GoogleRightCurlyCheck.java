...
public class GoogleRightCurlyCheck extends AbstractCheck {
    ...
    private static boolean hasContentOnRightSide(DetailAST block, DetailAST brace,
        ...
        return onSameNext && !nextIsValid
                || onSameAfterNext
                || isFollowedByComment(brace, nextToken);
    }
     ...
     */
    private static boolean isFollowedByComment(DetailAST rcurly, DetailAST nextToken) {
        ...
        final DetailAST modifier = nextToken.findFirstToken(TokenTypes.MODIFIERS);
        if (type != null) {
            comment = type.findFirstToken(TokenTypes.SINGLE_LINE_COMMENT);
        }
        if (TokenUtil.isOfType(nextToken, TokenTypes.METHOD_DEF)) {
            blockComment = NullUtil.notNull(modifier)
