...
public class FallThroughCheck extends AbstractCheck {
    ...
    private boolean isTerminated(final DetailAST ast, boolean useBreak,
        ...
        switch (ast.getType()) {
            ...
            case TokenTypes.LITERAL_BREAK:
                terminated =
                        useBreak || hasLabels(ast, labelsForCurrentSwitchScope);
                break;
            case TokenTypes.LITERAL_CONTINUE:
                terminated =
                        useContinue || hasLabels(ast, labelsForCurrentSwitchScope);
                break;
     ...
     */
    private static boolean hasLabels(DetailAST ast, Set<String> labelsForCurrentSwitchScope) {
        return Optional.of(ast)
                .filter(node -> node.getChildCount() == 2)
                .map(node -> node.getFirstChild())
                .map(DetailAST::getText)
                .filter(text -> !labelsForCurrentSwitchScope.contains(text))
                .isPresent();
    }
