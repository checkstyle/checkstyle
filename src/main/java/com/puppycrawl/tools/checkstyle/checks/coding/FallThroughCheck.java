...
public class FallThroughCheck extends AbstractCheck {
    ...
    private static boolean hasInfiniteLoop(DetailAST slist) {
        ...
        for (DetailAST child = slist.getFirstChild();
            ...
            final DetailAST body = child.findFirstToken(TokenTypes.SLIST);
            if (body != null && CheckUtil.isTerminated(body)) {
                continue;
            }
            switch (child.getType()) {
                case TokenTypes.LITERAL_FOR -> {
                    final DetailAST lastChild = child.getLastChild();
                    hasInfiniteLoop = !TokenUtil.isOfType(lastChild, TokenTypes.LITERAL_BREAK)
                            && checkForLoop(child);
                }
                case TokenTypes.LITERAL_WHILE -> {
                    final DetailAST lastChild = child.getLastChild();
                    hasInfiniteLoop = !TokenUtil.isOfType(lastChild, TokenTypes.LITERAL_BREAK)
                            && checkWhileCondition(child);
                }
                case TokenTypes.LITERAL_DO -> {
                    final DetailAST lparen =
                        NullUtil.notNull(child.findFirstToken(TokenTypes.LPAREN));
                    final DetailAST nextSibling = child.getFirstChild();
                    hasInfiniteLoop = !TokenUtil.isOfType(nextSibling, TokenTypes.LITERAL_BREAK)
                            && checkWhileCondition(lparen);
                }
                default -> {
                    // do nothing
                }
