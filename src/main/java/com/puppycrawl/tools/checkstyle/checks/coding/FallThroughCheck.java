...
public class FallThroughCheck extends AbstractCheck {
    ...
    private boolean hasFallThroughComment(DetailAST currentCase) {
        ...
        while (ast != null) {
            if (ast.getFirstChild() != null
                    && reliefPattern.matcher(ast.getFirstChild().getText()).find()) {
                allThroughComment = true;
                break;
            }
            ast = ast.getNextSibling();
        }
        return allThroughComment || commentAsChildOfSameGroup(reliefPattern, currentCase);
    }
    ...
    private static boolean commentAsChildOfSameGroup(Pattern reliefPattern,
        ...
        boolean allThroughComment = false;
        DetailAST node = currentCase.getLastChild();
        while (true) {
            if (node.getType() == TokenTypes.COMMENT_CONTENT
                    && reliefPattern.matcher(node.getText()).find()) {
                allThroughComment = true;
            }
            if (node.equals(currentCase.getLastChild().getLastChild())) {
                break;
            }
