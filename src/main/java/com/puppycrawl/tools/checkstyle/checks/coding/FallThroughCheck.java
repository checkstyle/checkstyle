...
public class FallThroughCheck extends AbstractCheck {
    ...
    private boolean isReliefComment(DetailAST ast) {
        ...
        if (!ans) {
            ans = isReliefCommentBetweenMultipleComment(list);
        }
     ...
     * @return true if relief comment found
     */
    private boolean isReliefCommentBetweenMultipleComment(List<DetailAST> list) {
        boolean ans = false;
        Collections.reverse(list);
        for (int i = 0; i < list.size() - 1; i++) {
            if (list.get(i).getLineNo() != list.get(i + 1).getLineNo()) {
                break;
            }
            else {
                if (list.get(i + 1).getType() == TokenTypes.SINGLE_LINE_COMMENT
                        || list.get(i + 1).getType() == TokenTypes.BLOCK_COMMENT_BEGIN) {
                    ans = reliefPattern.matcher(list.get(i + 1).getFirstChild().getText()).find();
                    break;
                }
            }
        }
