...
public class FallThroughCheck extends AbstractCheck {
    ...
    private boolean hasFallThroughComment(DetailAST currentCase) {
        ...
        final List<DetailAST> commentAsChildOfSameGroup =
                getCommentContentNodes(currentCase.getLastChild(),
                        currentCase.getLastChild().getLastChild());
        commentContent.addAll(commentAsChildOfSameGroup);
        return isReliefComment(commentContent, reliefPattern);
    }

    /**
     * Find relief pattern in comment list.
     *
     * @param commentList list of comment to check
     * @param reliefPattern The regular expression pattern to match the comment text
     * @return true if relief comment is found
     */
    private static boolean isReliefComment(Iterable<DetailAST> commentList, Pattern reliefPattern) {
        boolean result = false;
        for (DetailAST detailAST : commentList) {
