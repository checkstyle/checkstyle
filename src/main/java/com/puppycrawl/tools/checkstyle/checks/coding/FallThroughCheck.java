...
public class FallThroughCheck extends AbstractCheck {
    ...
    private boolean hasFallThroughComment(DetailAST currentCase) {
        ...
        final DetailAST ast;
        final DetailAST semi = currentCase.getLastChild().getLastChild();
        ...
        return isReliefComment(ast, semi);
    }
     ...
     * @param semi semi token
     * @return true if relief comment found
     */
    private boolean isReliefComment(DetailAST ast, DetailAST semi) {
        boolean ans = false;
        final List<DetailAST> list = new LinkedList<>();
        ...
        while (currentAst != null) {
            if (currentAst.getNextSibling() != null
                ...
                if (currentAst.getLineNo() == semi.getLineNo()) {
                    break;
                }
                list.add(currentAst);
                break;
        ...
        if (!ans) {
            ans = isReliefCommentBetweenMultipleCommentAtLastLine(list);
        }
    ...
    /**
     * If the last line contains multiple comment then this method will find
     * relief comment amongst them.
     *
     * @param list list to check
     * @return true if relief comment found
     */
    private boolean isReliefCommentBetweenMultipleCommentAtLastLine(List<DetailAST> list) {
        Collections.reverse(list);
        boolean ans = false;
        for (int start = 0; start < list.size() - 1; start++) {
            if (list.get(start + 1).getType() == TokenTypes.BLOCK_COMMENT_BEGIN) {
                ans = reliefPattern.matcher(list.get(start + 1).getFirstChild().getText()).find();
                break;
            }
        }
        return ans;
