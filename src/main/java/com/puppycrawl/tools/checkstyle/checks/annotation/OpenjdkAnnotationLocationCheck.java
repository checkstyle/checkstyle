...
public class OpenjdkAnnotationLocationCheck extends AbstractCheck {
     ...
     *
     * @param methodNode ast of method.
     * @return true if a method is on a single line.
     */
    private static boolean isSingleLineMethod(DetailAST methodNode) {
        boolean result = false;
        if (methodNode.getType() == TokenTypes.METHOD_DEF) {
            final DetailAST lastToken = methodNode.getLastChild();
            if (lastToken.getType() == TokenTypes.SEMI) {
                result = TokenUtil.areOnSameLine(methodNode, lastToken);
            }
            else {
                final DetailAST rightCurly = lastToken.getLastChild();
                result = TokenUtil.areOnSameLine(methodNode, rightCurly);
            }
        }
