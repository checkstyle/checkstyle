 ...
 * not share a line with the target declaration, except for single-line methods, fields,
 * classes, annotation definitions, and constructors.
 * </div>
 *
 * <p>
 * Attention: Checkstyle ignores annotations placed among modifiers due to a technical limitation.
 * The parser cannot distinguish whether an annotation applies to the method itself
 * or to its return type.
 * </p>
 *
 * @since 13.9.0
 */
...
public class OpenjdkAnnotationLocationCheck extends AbstractCheck {
    ...
    private static boolean areAllOnSameLine(List<DetailAST> annotationList) {
        return annotationList.isEmpty()
                || annotationList.getFirst().getLineNo() == annotationList.getLast().getLineNo();
    }
    ...
    private static boolean areAllOnSeparateLines(List<DetailAST> annotationList) {
        boolean areOnSeparateLine = true;
        for (int index = 0; index < annotationList.size() - 1; index++) {
            if (annotationList.get(index).getLineNo()
                ...
                areOnSeparateLine = false;
            }
        }
        return areOnSeparateLine;
    }
     ...
     */
    private static boolean isAnyOnTargetLine(Iterable<DetailAST> annotationList,
    ...
    /**
     * Checks whether a target that permits same-line annotations is single line or not.
     *
    ...
    private static boolean isSingleLineTarget(DetailAST startOfTargetNode,
        ...
        if (TokenUtil.isOfType(targetNode,
                TokenTypes.METHOD_DEF,
                TokenTypes.ANNOTATION_FIELD_DEF,
                TokenTypes.VARIABLE_DEF,
                TokenTypes.CLASS_DEF,
