...
public class UnusedImportsCheck extends AbstractJavadocCheck {
    ...
    public void visitJavadocToken(DetailNode ast) {
        switch (ast.getType()) {
            ...
            case JavadocCommentsTokenTypes.PARAMETER_TYPE -> {
            ...
            case JavadocCommentsTokenTypes.THROWS_BLOCK_TAG -> {
                currentFrame.addReferencedType(ast
                        .findFirstToken(JavadocCommentsTokenTypes.IDENTIFIER).getText());
            }
        }
    }
    ...
    private void processIdent(DetailAST ast) {
        ...
        // FIX: Ignore IDENTs that are part of the import statement itself
        if (parentType == TokenTypes.IMPORT || parentType == TokenTypes.STATIC_IMPORT) {
            return;
        }
    ...
    private void processReference(DetailNode ast) {
        String referenceText = topLevelType(ast.getFirstChild().getText());
        boolean isClassValid = isValidIdentifier(referenceText);

        DetailNode memberReference = ast.
                findFirstToken(JavadocCommentsTokenTypes.MEMBER_REFERENCE);
        boolean isMemberValid = true; // Default to true if no member exists

        if (memberReference != null) {
            String memberText = memberReference.getFirstChild().getText();
    ...
    boolean isValidIdentifier(String s) {
        ...
        boolean isValid = Character.isJavaIdentifierStart(cp);
