 ...
 * Wrapping means displaying the contents of the current line on the next line.
 * It is as simple as adding a line break on the current line and continuing
 * it on the next line.
 * </p>
 * <p>
 * Rationale: Long chained method calls are difficult to read and comprehend and sometimes
 * require horizontal scrolling to read the complete method call chain. Wrapping them onto
 * the next line/s enhances readability.
 * </p>
 * <ul>
 * <li>
 * Property {@code identifierPattern} - Specify method calls to be checked.
 * By default, no method calls are checked.
 ...
 *
 *     Truth.assertWithSomeMessage("ex").that(1).isEqualTo(1); // ok, identifierPattern is different
 * }
 * </pre>
 * <p>
 ...
 * <pre>
 * void method() {
 *     Truth.assertWithSomeMessage("ex").that(1).isEqualTo(1);
 *                      // violation above, ''3' method calls on single line, max allowed is '1''
 *     Truth.assertWithSomeMessage("ex")
 *         .that(1).isEqualTo(1); // violation, 'Chained method call should be wrapped'
 *
 *     assertWithSomeMessage("ex").that(1).isEqualTo(1); // ok, identifierPattern is different
 * }
 * </pre>
 * <p>
 ...
 * void method() {
 *     assertWithSomeMessage("ex").that(1).isEqualTo(1);
 *                                // ok above, 3 method calls &lt;= maxSingleLineCalls
 *     assertWithSomeMessage("ex")
 *         .that(1).isEqualTo(1); // violation, 'Chained method call should be wrapped'
 * }
 * </pre>
 ...
 *                      // violation above, ''3' method calls on single line, max allowed is '1''
 *     assertWithSomeMessage("ex")
 *         .that(1).isEqualTo(1); // ok, 2 method calls on current line &lt;= maxCallsPerLine
 * }
 * </pre>
 * <p>
 * To configure the check to use custom identifierPattern, maxSingleLinesCalls and maxCallsPerLine:
 * </p>
 * <pre>
 * &lt;module name=&quot;ChainedMethodCallWrap&quot;&gt;
 *  &lt;property name=&quot;identifierPattern&quot; value=&quot;^assert.*$&quot; /&gt;
 *  &lt;property name=&quot;maxSingleLineCalls&quot; value=&quot;2&quot; /&gt;
 *  &lt;property name=&quot;maxCallsPerLine&quot; value=&quot;2&quot; /&gt;
 * &lt;/module&gt;
 * </pre>
 * <p>
 ...
 * <pre>
 * void method() {
 *     assertThat(1).isEqualTo(1); // ok, 2 method calls &lt;= maxSingleLineCalls
 *
 *     assertWithSomeMessage("ex")
 *         .that(1).isEqualTo(1); // ok, 2 method calls on current line &lt;= maxCallsPerLine
 *
 *     assertWithSomeMessage("").that(1) // ok, 2 method calls on current line &lt;= maxCallsPerLine
 *         .isEqualTo(1);
 * }
 * </pre>
 * <p>
...
public class ChainedMethodCallWrapCheck extends AbstractCheck {
    ...
    /**
     * Specify method calls to be checked. By default, no method calls are checked.
     */
    ...
    /**
     * Setter to specify method calls to be checked. By default, no method calls are checked.
     *
    ...
    private void checkMethodChainingSingleLine(DetailAST methodCallAst) {
        ...
        if (methodCallCount > maxSingleLineCalls) {
            log(childAst, MSG_TOO_MANY_SINGLE_LINE, methodCallCount, maxSingleLineCalls);
        }
    }
        ...
        while (TokenUtil.isOfType(childAst, METHOD_CHAIN_CONTINUATION_NODES)) {
            if (childAst.getType() == TokenTypes.METHOD_CALL) {
                ...
                if (currentLineOuterMostMethodAst.getLineNo() == methodCallLineNo) {
                    methodCallCount++;
                    if (methodCallCount > maxCallsPerLine) {
     ...
     * other siblings or might be of the next siblings. This method traverses all child nodes of
