...
public class ChainedMethodCallWrapCheck extends AbstractCheck {
    ...
    /**
     * Terminal children of ast of type {@link TokenTypes#METHOD_CALL}.
     */
    private static final int[] TERMINAL_CHILDREN_OF_METHOD_CALL = {
        TokenTypes.IDENT,
    ...
    /**
     * Relevant children of ast node of type {@link TokenTypes#TYPECAST}.
     */
    private static final int[] CHILD_OF_TYPECAST = {
    ...
    private static boolean isTopMostMethodCall(DetailAST methodCallAst) {
        ...
        while (!TokenUtil.isOfType(ancestorAst, TokenTypes.EXPR, TokenTypes.METHOD_CALL)) {
            ancestorAst = ancestorAst.getParent();
        }
        return ancestorAst.getType() == TokenTypes.EXPR;
    }
    ...
    public static boolean isSingleLineMethodChain(DetailAST methodCallAst) {
        ...
        while (!TokenUtil.isOfType(childAst, TERMINAL_CHILDREN_OF_METHOD_CALL)) {
    ...
    private static DetailAST getMethodCallDescendantAst(DetailAST ast) {
        ...
        if (childAst.getType() == TokenTypes.TYPECAST) {
            childAst = TokenUtil.findFirstTokenByPredicate(childAst,
                    typecastAst -> {
                        return TokenUtil.isOfType(typecastAst, CHILD_OF_TYPECAST);
                    }).orElse(childAst.getLastChild());
        }
        return childAst;
    }
}
