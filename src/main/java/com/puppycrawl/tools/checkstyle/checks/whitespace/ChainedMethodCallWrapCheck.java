...
public class ChainedMethodCallWrapCheck extends AbstractCheck {
    ...
    public boolean matchesIdentifierPattern(DetailAST methodCallAst) {
        ...
        String identifierPatternString = identifierPattern.pattern();
        if (!identifierPatternString.equals("^$")) {
            final int endOfLinePos = identifierPatternString.lastIndexOf("$");
            if (endOfLinePos != -1) {
                identifierPatternString = identifierPatternString.substring(0,
                        endOfLinePos) + ".*$";
            }
            else {
                identifierPatternString = identifierPatternString + ".*";
            }
        }
        return Pattern.matches(identifierPatternString, getCompleteMethodChainName);
    }
