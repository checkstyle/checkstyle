...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    public void validateSummaryTag(DetailNode ast) {
        if (getInlineSummary(ast).isEmpty()) {
            log(ast.getLineNumber(), MSG_SUMMARY_JAVADOC_MISSING);
        }
        else if (!period.isEmpty()) {
            final String inlineSummaryDoc = getInlineSummary(ast);
            if (isPeriodAtEnd(inlineSummaryDoc, period)) {
                log(ast.getLineNumber(), MSG_SUMMARY_FIRST_SENTENCE);
            }
    ...
    private static String removeHtmlTagsFromString(String summarySentence) {
        return summarySentence.replaceAll("<[^>]*>", "").trim();
    }
    ...
    private static String getDefaultInlineSummarySentence(DetailNode ast) {
        ...
        for (DetailNode child: ast.getChildren()) {
            ...
            if (child.getType() == JavadocTokenTypes.HTML_ELEMENT) {
                ...
                result.append(getStringInsideHtmlElemet(child));
     ...
     */
    private static String getStringInsideHtmlElemet(DetailNode htmlNode) {
        // This will never return NPE as htmlNode will at least one child
        // This child will have at least 3 children
        return htmlNode.getChildren()[0].getChildren()[1].getText();
    }
    ...
    /**
     * Checks if the html node is of type list.
     *
     * @param htmlNode html element node.
     * @return true, if html node of list type.
     */
    private static boolean isHtmlList(DetailNode htmlNode) {
        return HTML_LIST_TAGS.contains(getHtmlTagName(htmlNode));
    }
