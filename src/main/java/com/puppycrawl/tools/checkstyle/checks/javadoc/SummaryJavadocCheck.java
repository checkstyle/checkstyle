...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    private static boolean containsSummaryTag(DetailNode javadoc) {
        final DetailNode summaryNode = getInlineTag(javadoc);
        return summaryNode != null && isSummaryTag(summaryNode.getChildren());
    }

    /**
     * Finds and return inline tag node from a javadoc root node.
     *
     * @param javadoc Javadoc root node.
     * @return Inline summary node or null if no node is found.
     */
    private static DetailNode getInlineTag(DetailNode javadoc) {
        DetailNode summaryTag = null;
        ...
        for (DetailNode child: children) {
            ...
            // If nested inside html tag
                ...
                summaryTag = getInlineTagNodeWithinHtmlElement(child);
            }
        }
        return summaryTag;
    }
    ...
    private static boolean isSummaryTagWithinHtmlTag(DetailNode javadoc) {
        ...
        final DetailNode inlineTagNode = getInlineTagNodeInsideHtmlTagsOnly(javadoc);
        if (inlineTagNode != null) {
            contains = isSummaryTag(inlineTagNode.getChildren());
        }
        return contains;
    }
    ...
    private static boolean containsCorrectInlineTags(DetailNode ast) {
        ...
        for (DetailNode node : ast.getChildren()) {
            ...
            if (child.length > 1
                    ...
                    && getInlineTagsInsideAnotherInlineTag(child) != null
                    && !ALLOWED_INLINE_TAGS.contains(getInlineTagsInsideAnotherInlineTag(child)
                        .getText())) {
                contains = false;
                break;
            }
    ...
    private static DetailNode getInlineTagNodeInsideHtmlTagsOnly(DetailNode ast) {
        ...
        for (DetailNode child: ast.getChildren()) {
            if (child.getType() == JavadocTokenTypes.HTML_ELEMENT) {
                result = getInlineTagNodeWithinHtmlElement(child);
            }
        }
        return result;
    }
    ...
    /**
     * Get inline tag nodes which are inside another inline tag from a javadoc node.
     *
     * @param javadoc Inline tag DetailNode.
     * @return DetailNode with CUSTOM_NAME;
     */
    private static DetailNode getInlineTagsInsideAnotherInlineTag(DetailNode... javadoc) {
