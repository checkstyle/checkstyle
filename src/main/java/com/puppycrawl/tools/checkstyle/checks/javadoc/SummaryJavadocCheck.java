...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    private static String getVisibleContent(String summary) {
        ...
        String comment = "";
        if (matcher.find()) {
            comment = removeNonHtmlPrintableElements(matcher.group(1));
        }
        return comment.trim();
    }
    ...
    /**
     * Removes html tags, extra whitespaces and asterisks from a string.
     *
     * @param summarySentence string to clean.
     * @return string without html tags.
     */
    private static String removeNonHtmlPrintableElements(String summarySentence) {
        String javadocText = HTML_ELEMENTS.matcher(summarySentence).replaceAll("");
        javadocText = JAVADOC_MULTILINE_TO_SINGLELINE_PATTERN.matcher(javadocText)
                .replaceAll("");
        return javadocText;
    }
