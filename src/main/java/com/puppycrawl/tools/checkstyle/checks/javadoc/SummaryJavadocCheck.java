...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    public void visitJavadocToken(DetailNode ast) {
        ...
        AtomicBoolean shouldValidateUntaggedSummary = new AtomicBoolean(true);

        inlineTagNode.ifPresent(node -> {
            if (isSummaryTag(node)
                    && isDefinedFirst(node)) {
                shouldValidateUntaggedSummary.set(false);
        ...
        if (shouldValidateUntaggedSummary.get()
    ...
    private void validateUntaggedSummary(DetailNode ast) {
        ...
        if (summaryDoc.isEmpty()) {
            ...
            if (summaryDoc.contains(period)) {
                final String firstSentence = getFirstSentence(ast, period).orElse("");
    ...
    private static Optional<DetailNode> getInlineTagNode(DetailNode javadoc) {
        ...
        for (DetailNode child : javadoc.getChildren()) {
            if (isInlineTagPresent(child)) {
                result = Optional.of(getInlineTagNodeForAst(child));
                break;
            }
        }
        return result;
    ...
    private static Optional<String> getFirstSentence(DetailNode ast, String period) {
        ...
        for (String text : (Iterable<String>) streamTextParts(ast)::iterator) {
            final String sentenceEnding = findSentenceEnding(text, period).orElse("");
            ...
            if (sentenceEnding.isEmpty()) {
                ...
                final String sentence = String.join("", sentenceParts);
                result = Optional.of(sentence);
                break;
            }
        }
        return result;
