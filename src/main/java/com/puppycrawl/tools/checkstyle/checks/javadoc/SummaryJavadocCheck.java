...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    private static boolean appendUpToPeriod(String text, String period, StringBuilder result) {
        ...
        boolean foundPeriod = false;
        while (periodIndex >= 0) {
            if (isSentenceEndingPeriod(text, period, periodIndex)) {
                result.append(text, 0, periodIndex);
                foundPeriod = true;
                break;
            }
            else {
                periodIndex = text.indexOf(period, periodIndex + period.length());
            }
        }
        if (!foundPeriod) {
            result.append(text);
    ...
    /**
     * Check whether the period at the given index ends a sentence. Only requires whitespace
     * after a sentence ending period if we're using the default period. In the future this may
     * be replaced by use of Java's built-in BreakIterator class.
     *
     * @param text string the period was found in
     * @param period period character
     * @param periodIndex index of the found period
     * @return true if the period ends a sentence, false otherwise
     */
    private static boolean isSentenceEndingPeriod(String text, String period, int periodIndex) {
