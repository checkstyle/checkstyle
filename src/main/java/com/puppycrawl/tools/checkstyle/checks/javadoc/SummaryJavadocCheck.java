...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    private static boolean startsWithInheritDoc(DetailNode root) {
        ...
        while (i != children.length) {
            if ((children[i].getType() == JavadocTokenTypes.TEXT
                    || children[i].getType() == JavadocTokenTypes.HTML_ELEMENT)
                    && !CommonUtil.isBlank(children[i].getText())) {
                break;
            }
            if (children[i].getType() == JavadocTokenTypes.JAVADOC_INLINE_TAG
                    && children[i].getChildren()[1].getType()
