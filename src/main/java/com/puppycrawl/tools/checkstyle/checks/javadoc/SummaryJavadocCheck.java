...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    /**
     * Set of tags that define the html tag is of type list.
     */
    ...
    private static boolean containsSummaryTag(DetailNode javadoc) {
        ...
        for (DetailNode node : javadoc.getChildren()) {
            if (node.getType() == JavadocTokenTypes.JAVADOC_INLINE_TAG) {
                ...
                contains = isSummaryTag(child);
            }
        }

        if (isSummaryTagWithinHtmlTag(javadoc)) {
            contains = true;
        }
    ...
    private static boolean isSummaryTagWithinHtmlTag(DetailNode javadoc) {
        boolean contains = false;
        final DetailNode inlineTagNode = getInlineTagNodeInsideHtmlTags(javadoc);
        if (inlineTagNode != null) {
            final DetailNode[] htmlChild = inlineTagNode.getChildren();
            contains = isSummaryTag(htmlChild);
        }
        return contains;
    }
     ...
     */
    private static boolean isSummaryTag(DetailNode... ast) {
        ...
        // RCURLY and LCURLY.
        return ast[1].getType() == JavadocTokenTypes.CUSTOM_NAME
                && SUMMARY_TEXT.equals(ast[1].getText());
    }
    ...
    private static String getInlineFirstSentence(DetailNode ast) {
        ...
        final DetailNode inlineHtmlNode = getInlineTagNodeInsideHtmlTags(ast);
    ...
    /**
     * Returns inline tag Node inside html tags from an AST node.
     *
     * @param ast Root node.
     * @return DetailNode of inline tag.
     */
    private static DetailNode getInlineTagNodeInsideHtmlTags(DetailNode ast) {
        DetailNode result = null;
        for (DetailNode child: ast.getChildren()) {
            if (child.getType() == JavadocTokenTypes.HTML_ELEMENT) {
                result = getInlineTagNodeWithinHtml(child);
            }
        }
        return result;
    }
     ...
     * @return DetailNode of inline tag.
     */
    private static DetailNode getInlineTagNodeWithinHtml(DetailNode ast) {
        ...
        if (ast.getType() == JavadocTokenTypes.HTML_ELEMENT) {
            for (DetailNode nodeChild : ast.getChildren()[0].getChildren()) {
                if (nodeChild.getType() == JavadocTokenTypes.JAVADOC_INLINE_TAG) {
                    ...
                    inlineNode = getInlineTagNodeWithinHtml(nodeChild);
                }
            }
        }
        ...
        if (ast.getType() == JavadocTokenTypes.HTML_TAG) {
            for (DetailNode child : ast.getChildren()) {
                if (child.getType() == JavadocTokenTypes.JAVADOC_INLINE_TAG) {
                    ...
                    inlineNode = getInlineTagNodeWithinHtml(child);
                }
            }
        }
        return inlineNode;
     ...
     * This should be removed after Issue #
     *
    ...
    private static String getInlineHtmlSentence(DetailNode ast) {
        final StringBuilder result = new StringBuilder(256);
        final DetailNode inlineTagNode = getInlineTagNodeInsideHtmlTags(ast);
    ...
    private boolean containsForbiddenFragment(String firstSentence) {
        ...
        for (char letter : javadocText.toCharArray()) {
            final char print;
        ...
        return forbiddenSummaryFragments.matcher(result.toString()).find();
    }
