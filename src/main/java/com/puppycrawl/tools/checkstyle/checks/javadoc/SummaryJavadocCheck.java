...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
    ...
    private static Optional<DetailNode> getFirstInlineTag(DetailNode javadoc) {
        Optional<DetailNode> node = Arrays.stream(javadoc.getChildren())
                .filter(child -> child.getType() == JavadocTokenTypes.JAVADOC_INLINE_TAG)
                .findFirst();

        if (!node.isPresent()) {
            final Optional<DetailNode> htmlNode = Arrays.stream(javadoc.getChildren())
                    .filter(child -> child.getType() == JavadocTokenTypes.HTML_ELEMENT)
                    .findFirst();
            if (htmlNode.isPresent()) {
                node = Optional.ofNullable(getInlineTagNodeWithinHtmlElement(htmlNode.get()));
            }
        }
        return node;
    }
    ...
    private static boolean isSummaryTag(DetailNode javadoc) {
        return Arrays.stream(javadoc.getChildren())
                .anyMatch(child -> {
                    return child.getType() == JavadocTokenTypes.CUSTOM_NAME
                            && SUMMARY_TEXT.equals(child.getText());
                });
    }
