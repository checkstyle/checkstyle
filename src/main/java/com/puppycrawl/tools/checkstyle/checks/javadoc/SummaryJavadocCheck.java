...
public class SummaryJavadocCheck extends AbstractJavadocCheck {
        ...
        if (summaryDoc.isEmpty()) {
            ...
            if (summaryDoc.contains(period)) {
                ...
                if (firstSentence.isEmpty()) {
                    ...
                    final String firstSentenceWithoutPeriod =
                            firstSentence.substring(0, firstSentence.length() - 1);
                    if (containsForbiddenFragment(firstSentenceWithoutPeriod)) {
     ...
     * @return The first sentence up to and including the period,
     *         or empty if no sentence with a period was found.
     */
    private static String getFirstSentence(DetailNode ast, String period) {
        ...
        for (String text : (Iterable<String>) streamTextParts(ast)::iterator) {
            final String sentenceToPeriod = getSentenceToPeriod(text, period);
            if (sentenceToPeriod.isEmpty()) {
                sentenceParts.add(text);
            }
            else {
                sentenceParts.add(sentenceToPeriod);
    ...
    /**
     * Finds a sentence to period. If a sentence ending period was found, returns the whole string
     ...
     * @return The string up to and including the period,
     *         or empty if no sentence with a period was found.
     */
    private static String getSentenceToPeriod(String text, String period) {
