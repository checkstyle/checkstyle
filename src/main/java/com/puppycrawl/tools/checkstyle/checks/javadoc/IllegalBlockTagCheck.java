 ...
 * With the default pattern {@code $^} (matches nothing), any presence of the
 * configured tag is a violation. No violation is reported when there is no
 * Javadoc or when {@code tag} is not configured.
 * </div>
 *
 * @since 13.10.0
 */
...
public class IllegalBlockTagCheck extends AbstractJavadocCheck {
    ...
    private static final Pattern MATCH_NOTHING = Pattern.compile("$^");
        ...
        if (tag != null) {
            ...
            if (tagName.equals(tag)) {
                ...
                if (!tagTextPattern.matcher(content).find()) {
                    log(ast.getLineNumber(), MSG_ILLEGAL_PATTERN,
                            JavadocUtil.getTagName(ast));
                }
            }
        }
    }
