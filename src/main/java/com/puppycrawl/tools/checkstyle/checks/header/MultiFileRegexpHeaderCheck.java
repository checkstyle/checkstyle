...
public class MultiFileRegexpHeaderCheck
    ...
    private void logFirstMismatch(FileText fileText) {
        ...
        for (HeaderFileMetadata metadata : headerFilesMetadata) {
            ...
            if (!mismatchFound && VALID_LINE_HEADER_CHECKER != mismatchLine) {
                mismatchFound = true;
    ...
    private static int findFirstMismatch(FileText fileText, HeaderFileMetadata headerFileMetadata) {
        ...
        for (int i = 0; i < headerPatterns.size(); i++) {
            ...
            if (i < fileSize) {
                matched = headerPatterns.get(i).matcher(fileText.get(i)).matches();
            } else {
                matched = headerPatterns.get(i).pattern().isBlank();
            }
            if (matched) {
                lineMatches.set(i);
            }
        ...
        public static HeaderFileMetadata createFromFile(String headerFile) {
            ...
            try {
                ...
                final List<Pattern> patterns = readerLines.stream()
                        .map(line -> line.isEmpty() ? BLANK_LINE
                                : Pattern.compile(validateRegex(line)))
