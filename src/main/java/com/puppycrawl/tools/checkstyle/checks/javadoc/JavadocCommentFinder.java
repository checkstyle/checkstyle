...
public final class JavadocCommentFinder {
    ...
    public static DetailAST findJavadocComment(DetailAST ast) {
        ...
        if (javadoc == null && ast.getType() == TokenTypes.CTOR_DEF) {
            // For constructors of inner classes, check if the class has javadoc
            javadoc = findJavadocFromParentClass(ast);
        }
    ...
    private static DetailAST findJavadocInTypeParameters(DetailAST typeParams) {
        return typeParams.findFirstToken(TokenTypes.BLOCK_COMMENT_BEGIN);
    }
    ...
    private static DetailAST findJavadocInType(DetailAST type) {
        DetailAST javadoc = type.findFirstToken(TokenTypes.BLOCK_COMMENT_BEGIN);
    ...
    private static DetailAST findJavadocInDotNode(DetailAST dotNode) {
        DetailAST javadoc = dotNode.findFirstToken(TokenTypes.BLOCK_COMMENT_BEGIN);
    ...
    /**
     * Finds javadoc comment from the parent CLASS_DEF for a constructor.
     * This is used when a constructor has no explicit javadoc but the containing
     * class has javadoc that might be incorrectly intended for the constructor.
     *
     * @param constructor the CTOR_DEF node
     * @return the javadoc comment node from the parent class, or null if not found
     */
    private static DetailAST findJavadocFromParentClass(DetailAST constructor) {
        final DetailAST parent = constructor.getParent();
        final DetailAST classDef = parent.getParent();
        final DetailAST classModifiers = classDef.findFirstToken(TokenTypes.MODIFIERS);
        return findJavadocInModifiers(classModifiers);
    }
}
