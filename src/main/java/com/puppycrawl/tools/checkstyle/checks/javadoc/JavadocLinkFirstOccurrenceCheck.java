...
public class JavadocLinkFirstOccurrenceCheck extends AbstractJavadocCheck {
    ...
    private void collectLinkViolations(DetailNode node, Set<String> linkedNames) {
        ...
        while (child != null) {
            if (child.getType() == JavadocCommentsTokenTypes.JAVADOC_INLINE_TAG) {
    ...
    private static DetailNode findLinkTagChild(DetailNode inlineTag) {
        ...
        for (DetailNode child = inlineTag.getFirstChild();
             child != null; child = child.getNextSibling()) {
            if (child.getType() == JavadocCommentsTokenTypes.LINK_INLINE_TAG
                    || child.getType() == JavadocCommentsTokenTypes.LINKPLAIN_INLINE_TAG) {
                result = child;
                break;
            }
        }
    ...
    private static String getSimpleName(String qualifiedName) {
        ...
        if (lastDotIndex == -1) {
            ...
            result = qualifiedName.substring(lastDotIndex + 1);
        }
        return result;
    }
