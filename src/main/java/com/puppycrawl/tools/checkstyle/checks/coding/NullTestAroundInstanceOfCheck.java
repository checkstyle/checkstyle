...
public class NullTestAroundInstanceOfCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (landNode == null) {
            ...
            if (notEqualNode != null && instanceOfNode != null
                ...
                final DetailAST dotNode1 = getDotNode(notEqualNode);
                final DetailAST dotNode2 = getDotNode(instanceOfNode);
                final String string1;
                if (dotNode1 == null) {
                    string1 = getIdentNode(notEqualNode).getText();
                }
                else {
                    final DetailAST identNode1 = getIdentNode(dotNode1);
                    string1 = getText(identNode1);
                }
                final String string2;
                if (dotNode2 == null) {
                    string2 = getIdentNode(instanceOfNode).getText();
                }
                else {
                    final DetailAST identNode2 = getIdentNode(dotNode2);
                    string2 = getText(identNode2);
                }
    ...
    private static String getText(DetailAST ast) {
        return ast.getText() + ast.getNextSibling().getText();
    }

    private static DetailAST getDotNode(DetailAST ast) {
        return ast.findFirstToken(TokenTypes.DOT);
    }

    private static DetailAST getIdentNode(DetailAST ast) {
        ...
        if (notEqualNode != null
            ...
            if (nestedIfNode != null) {
                ...
                if (instanceOfNode != null
                        && getIdentNode(notEqualNode).getText()
                        .equals(getIdentNode(instanceOfNode).getText())) {
                    error = true;
                }
            }
        }
