...
public class LineWrappingHandler {
    ...
    private IndentLevel getWrappedLineIndent(DetailAST node, int currentIndent,
        ...
        final DetailAST call = getEnclosingCall(node);
        if (call != null) {
            final DetailAST dot = call.findFirstToken(TokenTypes.DOT);
    ...
    /**
     * Finds the method call whose arguments contain the given node. The search
     * stops at the first token that opens an indentation scope of its own, as a
     * node behind such a token is not wrapped relative to the call.
     *
     * @param node the node to find the enclosing call of.
     * @return the enclosing method call, or {@code null} if there is none.
     */
    private static DetailAST getEnclosingCall(DetailAST node) {
