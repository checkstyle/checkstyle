    ...
    private static boolean isCurrentNodeAnnotationLineWrappedInClassDef(DetailAST node,
        ...
        final boolean isPreviousToPreviousSiblingAccessModifier =
            Optional.ofNullable(parentNode).map(DetailAST::getPreviousSibling)
                .filter(sibling -> TokenUtil.isOfType(TokenTypes.LITERAL_PUBLIC,
                    TokenTypes.LITERAL_PRIVATE, TokenTypes.LITERAL_PROTECTED)).isPresent();
        return TokenUtil.isOfType(node, TokenTypes.AT)
    ...
    private void checkAnnotationIndentation(DetailAST atNode,
        ...
        while (firstNodesOnLines.size() > 1) {
            ...
            final boolean isParentNodeModifierOrAnnotation =
                TokenUtil.isOfType(parentNode, TokenTypes.ANNOTATION, TokenTypes.MODIFIERS);
