...
public class UnusedPrivateFieldCheck extends AbstractCheck {
    ...
    private void visitClassDef(DetailAST typeDef) {
        ...
        if (classScopeStack.isEmpty()) {
            ...
            topLevel = classScopeStack.peekLast().getTopLevelClassDef();
        }
        ...
        if (isField(varDef)) {
            ...
            if (modifiers.findFirstToken(TokenTypes.LITERAL_PRIVATE) != null) {
                ...
                if (!SERIAL_VERSION_UID.equals(fieldName)) {
                    final DetailAST currentClass = classScopeStack.peek().getClassDef();
    ...
    private void visitIdent(DetailAST identAst) {
        ...
        if (!TokenUtil.isOfType(parent, UNACCEPTABLE_PARENT_OF_IDENT)
    ...
    private DetailAST findClassByName(String name) {
        return classScopeStack.stream()
                .map(ClassScope::getClassDef)
                .filter(classDef -> isClassWithName(classDef, name))
                .findFirst()
    ...
    private static boolean isClassWithName(DetailAST classDef, String name) {
        ...
        return name.equals(ident.getText());
    }
    ...
    private void visitDot(DetailAST dotAst) {
        ...
        if (dotAst.getParent().getType() != TokenTypes.LITERAL_NEW
            ...
            if (isThisExpression(firstChild)) {
                ...
                final DetailAST topLevel =
                        classScopeStack.peek().getTopLevelClassDef();
    ...
    private DetailAST findEnclosingNonAnonClass() {
        return classScopeStack.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException(
                        "No enclosing non-anonymous class found"));
    }
    ...
    private static final class ClassScope {
        ...
        /** The CLASS_DEF ast for this scope. */
        private final DetailAST classDef;

        /** The top-level CLASS_DEF ancestor ast. */
        private final DetailAST topLevelClassDef;

        /**
         * Create a new ClassScope.
         *
         * @param classDef the CLASS_DEF ast
         * @param topLevelClassDef the top-level ancestor CLASS_DEF ast
         */
        private ClassScope(DetailAST classDef, DetailAST topLevelClassDef) {
            this.classDef = classDef;
            this.topLevelClassDef = topLevelClassDef;
        }

        /**
         * Get the CLASS_DEF ast.
         *
         * @return CLASS_DEF ast
         */
        private DetailAST getClassDef() {
            return classDef;
        }

        /**
         * Get the top-level CLASS_DEF ancestor ast.
         *
         * @return top-level CLASS_DEF ast
         */
        private DetailAST getTopLevelClassDef() {
