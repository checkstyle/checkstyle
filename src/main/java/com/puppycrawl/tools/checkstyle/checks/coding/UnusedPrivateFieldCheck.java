...
public class UnusedPrivateFieldCheck extends AbstractCheck {
    ...
    private DetailAST findClassByName(String name) {
        return classScopeStack.stream()
            ...
            .findFirst()
            .orElseGet(this::peekCurrentClassDef);
    }

    /**
     * Returns the current class definition from the top of the class scope stack.
     *
     * @return the current class definition
     */
    private DetailAST peekCurrentClassDef() {
        return classScopeStack.peek().classDef();
    }
    ...
    private DetailAST findEnclosingNonAnonClass() {
        ...
        if (classScopeStack.isEmpty()) {
            ...
            result = classScopeStack.peek().classDef();
        }
        return result;
    }
