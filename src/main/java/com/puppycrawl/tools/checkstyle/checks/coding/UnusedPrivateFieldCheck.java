...
/**
 * <div>
 * Check that a private or package-private field is declared, but never used.
 * </div>
 *
 * @since 13.9.0
 */
...
public class UnusedPrivateFieldCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        switch (ast.getType()) {
            case TokenTypes.OBJBLOCK, TokenTypes.COMPACT_COMPILATION_UNIT -> {
    ...
    /**
     * Reports all unused fields at the end of the file.
     *
    ...
    public void finishTree(final DetailAST rootAST) {
        for (final PendingField pending : pendingFields) {
            if (!pending.usedNames.contains(pending.entry.getKey())
                ...
                log(pending.entry.getValue(), MSG_PRIVATE_FIELD,
                    pending.entry.getKey());
            }
        }
    }

    /**
     * Collects private and package-private field declarations, whether the field is
     * a direct member of a class/enum/record body ({@code OBJBLOCK}), or a top-level
     * member of an implicitly declared class.
     *
    ...
    private void handleVariableDef(DetailAST ast) {
        ...
        if (parentType == TokenTypes.OBJBLOCK
            ...
            final boolean isPrivateField = isPackagePrivate(ast, modifiers);
            final boolean isIgnored = hasIgnoredAnnotation(ast);
    ...
    private boolean hasIgnoredAnnotation(final DetailAST variableDef) {
        ...
        if (!result) {
            final DetailAST fieldParent = variableDef.getParent();
            if (fieldParent.getType() != TokenTypes.COMPACT_COMPILATION_UNIT) {
                final DetailAST classDef = fieldParent.getParent();
     ...
     * by canonical name or by short name. Returns {@code false} without inspecting
     * anything if the node has no recognizable annotation holder (for example, the
     * synthetic enclosing node of an anonymous class), rather than assuming one
     * always exists.
     *
     * @param ast the node to inspect for a matching ignore annotation.
    ...
    /**
     * Checks whether a field has default (package-private) access.
     *
     * @param variableDef the VARIABLE_DEF node, used to find the enclosing type.
     * @param modifiers   the field's MODIFIERS node.
     * @return true if the field has default access and its enclosing type is a
     *         class, enum, record, or implicitly declared class (not an interface
     *         or annotation type).
     */
    private static boolean isPackagePrivate(final DetailAST variableDef,
                                             final DetailAST modifiers) {
        boolean result = false;
        final boolean hasNoAccessModifier =
                modifiers.findFirstToken(TokenTypes.LITERAL_PUBLIC) == null
                && modifiers.findFirstToken(TokenTypes.LITERAL_PROTECTED) == null;
        if (hasNoAccessModifier) {
            final DetailAST fieldParent = variableDef.getParent();
            if (fieldParent.getType() == TokenTypes.COMPACT_COMPILATION_UNIT) {
                result = true;
            }
            else {
                final DetailAST enclosingType = fieldParent.getParent();
                final int enclosingTypeTokenType = enclosingType.getType();
                result = enclosingTypeTokenType != TokenTypes.INTERFACE_DEF
                        && enclosingTypeTokenType != TokenTypes.ANNOTATION_DEF;
            }
        }
        return result;
    }
    ...
    /**
     * Holds a tracked field entry together with the set of names used
    ...
    private record PendingField(
            Map.Entry<String, DetailAST> entry,
