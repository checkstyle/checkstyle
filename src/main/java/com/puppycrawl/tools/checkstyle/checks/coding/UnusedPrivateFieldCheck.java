...
public class UnusedPrivateFieldCheck extends AbstractCheck {
    ...
    /**
     * Accumulates pending private fields along with their usage information for final reporting.
     */
    private final List<FieldData> pendingFields = new ArrayList<>();
        ...
        switch (ast.getType()) {
            case TokenTypes.OBJBLOCK -> {
                ...
                for (final Map.Entry<String, DetailAST> entry : classFields.entrySet()) {
                    final FieldData data = new FieldData(entry.getValue(), classUsed);
                    pendingFields.add(data);
                }
            }
            case TokenTypes.CTOR_DEF, TokenTypes.SLIST -> scopeStack.pop();
            default -> {
                // no action needed for other token types
            }
        }
    ...
    public void finishTree(final DetailAST rootAST) {
        for (final FieldData data : pendingFields) {
            final String name =
                    data.ast.findFirstToken(TokenTypes.IDENT).getText();
            if (!data.used.contains(name)
                    && !globalUsedFields.contains(name)) {
                log(data.ast, MSG_PRIVATE_FIELD, name);
            }
        }
    }
    ...
    /**
     * Holds a pending private field and the set of names
     * in which it was marked as used.
     */
    private static final class FieldData {

        /**
         * AST node of the field declaration.
         */
        private final DetailAST ast;

        /**
         * Names where this field was referenced.
         */
        private final Set<String> used;

        /**
         * Creates a new instance holding a field AST and its usage set.
         *
         * @param ast  the AST of the field declaration
         * @param used set of names where the field was referenced
         */
        private FieldData(final DetailAST ast, final Set<String> used) {
            this.ast = ast;
            this.used = used;
        }
    }
