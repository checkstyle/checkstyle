...
public class UnusedPrivateFieldCheck extends AbstractCheck {
     ...
     */
    private Set<String> ignoreAnnotationCanonicalNames = new HashSet<>();
    ...
    private void handleVariableDef(DetailAST ast) {
        ...
        if (parent.getType() == TokenTypes.OBJBLOCK) {
            ...
            if (isPrivateField && !isConstant && !isIgnored) {
                final DetailAST ident = ast.findFirstToken(TokenTypes.IDENT);
    ...
    /**
     * Checks whether a field has an annotation that should be ignored.
     *
     * @param variableDef the VARIABLE_DEF node.
     * @return true if the field has an annotation in ignoreAnnotationCanonicalNames.
     */
