...
public class TrailingCommentCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        final boolean isCommentEndOfLine = ast.getType() == TokenTypes.SINGLE_LINE_COMMENT
                || isBlockCommentEndOfLine(ast);

        final boolean isCommentOnOneLine = ast.getType() == TokenTypes.SINGLE_LINE_COMMENT
                || isBlockCommentOnOneLine(ast);

        final String commentContent = ast.getFirstChild().getText();
        final boolean isSingleLineAndMatchLegalPattern =
                isCommentOnOneLine && isCommentMatchLegalPattern(commentContent);

        final boolean isCommentLegal = !isCommentEndOfLine || isSingleLineAndMatchLegalPattern;
    ...
    /**
     * Check if the block comment ends a line.
     *
     * @param commentBeginAst the comment begin AST
     * @return true if the block comment ends a line
     */
    private boolean isBlockCommentEndOfLine(DetailAST commentBeginAst) {
        final DetailAST commentEnd = commentBeginAst.getFirstChild().getNextSibling();
        final String afterComment = getTextAfterBlockComment(commentEnd);
        return CommonUtil.isBlank(afterComment);
    }

    /**
     * Check if the block comment is on one line.
     *
     * @param commentBeginAst the comment begin AST
     * @return true if the block comment is on one line
     */
    private static boolean isBlockCommentOnOneLine(DetailAST commentBeginAst) {
        final DetailAST commentEnd = commentBeginAst.getFirstChild().getNextSibling();
        return TokenUtil.areOnSameLine(commentBeginAst, commentEnd);
    }

    /**
     * Check if commentContent matches the legalComment pattern.
     *
     * @param commentContent the content of a comment
     * @return true if commentContent matches the legalComment pattern
     */
    ...
    private String getTextAfterBlockComment(DetailAST commentEndAst) {
        ...
        // commentEndColumnNo can either point at the char right before '/', or 2 chars before '/'
        // /* Example comment */
        //                   ^^
        int offset = 2;
        if (commentEndLine.charAt(commentEndColumnNo + 1) != '/') {
            offset += 1;
        }
        return commentEndLine.substring(commentEndColumnNo + offset);
    }
