...
public class TrailingCommentCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (ast.getType() == TokenTypes.BLOCK_COMMENT_BEGIN
                && isAnotherCommentInSameLine(ast)) {
            checkBlockComment(ast);
        }
    }

    /**
     * Check is single line comment is legal.
     *
    ...
    private void checkBlockComment(DetailAST ast) {
        ...
        if (comment.contains(newLine)) {
            comment = comment.substring(0, comment.indexOf(newLine));
        }
        if (line.length() > (ast.getLastChild().getColumnNo() + 2)) {
        ...
        if (!(ast.getLineNo() == ast.getLastChild().getLineNo()
                && !CommonUtil.isBlank(line)) && !format.matcher(lineBefore).find()
                && !isCorrectBlockComment(ast, comment)) {
            log(ast, MSG_KEY);
        }
    }
     ...
     */
    private boolean isCorrectBlockComment(DetailAST ast, final String comment) {
        final boolean legal;

        // multi-line comment can not be legal
        if (legalComment == null
                || ast.getFirstChild().getLineNo() != ast.getLastChild().getLineNo()) {
            legal = false;
        }
        else {
     ...
     * @return true if the comment if legal.
     */
    private boolean checkSingleLine(String comment) {
        final boolean legal;
        if (legalComment == null) {
            legal = false;
        }
    ...
    /**
     * Checks if there is another comment in same line.
     *
     * @param ast Detail ast element to be checked.
     * @return true if no comment is found.
     */
    private boolean isAnotherCommentInSameLine(DetailAST ast) {
        DetailAST travelAst = ast;
        boolean isComment = true;
        final int lineNo = travelAst.getLineNo();
        while (travelAst != null && travelAst.getLineNo() == lineNo) {
            if (travelAst.getNextSibling() != null
                    && (isAnotherSingleLineCommentOnSameLine(travelAst)
                || isAnotherBlockCommentOnSameLine(travelAst))) {
                isComment = false;
                break;
            }
            else if (travelAst.getNextSibling() == null) {
                travelAst = travelAst.getParent();
            }
            else {
    ...
    /**
     * Checks if there is Single Line comment on same line.
     *
     * @param ast Root node.
     * @return true, if Single Line comment present.
     */
    private boolean isAnotherSingleLineCommentOnSameLine(DetailAST ast) {
        return ast.getNextSibling().getType() == TokenTypes.SINGLE_LINE_COMMENT
                && ast.getLineNo() == ast.getNextSibling().getLineNo();
    }
     ...
     *
     * @param ast Root node.
     * @return true, if Block comment present.
     */
    private boolean isAnotherBlockCommentOnSameLine(DetailAST ast) {
        return ast.getNextSibling().getType() == TokenTypes.BLOCK_COMMENT_BEGIN
                && ast.getLineNo() == ast.getNextSibling().getLineNo();
    }
}
