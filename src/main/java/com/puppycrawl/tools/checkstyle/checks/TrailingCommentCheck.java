...
public class TrailingCommentCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        if (ast.getType() == TokenTypes.SINGLE_LINE_COMMENT) {
            // Cpp comment
            if (!isBeforeCommentLegal(ast) && !isCommentMatchLegalPattern(commentContent)) {
                log(ast, MSG_KEY);
            }
        }
        else {
            /* C comment (block comment) */
            ...
            if (isCommentEndOfLine) {
                ...
                final boolean isCommentLegal = !isMultiLine
                        && isCommentMatchLegalPattern(commentContent);
                if (!isBeforeCommentLegal(ast) && !isCommentLegal) {
                    log(ast, MSG_KEY);
                }
            }
        }
