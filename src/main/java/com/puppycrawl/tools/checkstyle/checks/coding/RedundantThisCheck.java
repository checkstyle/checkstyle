...
public class RedundantThisCheck extends AbstractCheck {
     ...
     */
    private final Set<String> paramtersInMethod = new HashSet<>();
        ...
        switch (ast.getType()) {
            case TokenTypes.SLIST -> {
                if (scopeStack.isEmpty()) {
                    ...
                    final Set<String> scopeVariables = new HashSet<>(scopeStack.peek());
                    scopeStack.push(scopeVariables);
                }
            }
            ...
            default -> checkUnnecessaryThis(ast);
        }
    }
    ...
    private void checkUnnecessaryThis(DetailAST ast) {
        ...
        final DetailAST parent = ast.getParent();
        final boolean isVariable = nextSibling != null
                && nextSibling.getType() == TokenTypes.IDENT
        ...
        else if (nextSibling != null && grandParent.getType() == TokenTypes.METHOD_CALL
                && checkMethodCall) {
    ...
    private boolean isThisVariableRedundant(DetailAST nextSibling) {
        ...
        return !paramtersInMethod.contains(variable)
                && (scopeStack.isEmpty() || !scopeStack.peek().contains(variable));
    }

    /**
     * Collects the parameter variables of the current method.
     *
     * @param paramters list of paramters
     */
    private void collectParameters(DetailAST paramters) {
        DetailAST child = paramters.getFirstChild();
        while (child != null) {
            if (child.getType() == TokenTypes.PARAMETER_DEF) {
                final String parameterVariable = child.findFirstToken(TokenTypes.IDENT).getText();
                paramtersInMethod.add(parameterVariable);
            }
