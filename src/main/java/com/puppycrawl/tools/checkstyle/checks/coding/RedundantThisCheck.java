...
public class RedundantThisCheck extends AbstractCheck {
     ...
     */
    private boolean isFieldReference(DetailAST literalThis) {
     ...
     */
    private boolean isMethodCallReference(DetailAST literalThis) {
     ...
     */
    private boolean isPatternVariableInScope(String name, DetailAST usageAst) {
        boolean result = false;
        DetailAST child = usageAst;
        DetailAST parent = usageAst.getParent();
        while (parent != null && !result
                && !TokenUtil.isOfType(parent, TokenTypes.METHOD_DEF, TokenTypes.CTOR_DEF)) {
            result = isPatternVariableVisibleAt(parent, child, name);
            child = parent;
     ...
     */
    private boolean isPatternVariableVisibleAt(
     ...
     */
    private boolean isPatternInCaseLabel(DetailAST switchRule, String name) {
     ...
     */
    private boolean isThenBranch(DetailAST literalIf, DetailAST child) {
     ...
     */
    private boolean isPositivePatternMatch(DetailAST literalIf, String name) {
     ...
     */
    private boolean hasPrecedingNegatedPatternIf(DetailAST child, String name) {
        boolean result = false;
    ...
    /**
     * Finds the {@code LITERAL_INSTANCEOF} node that is the whole condition of
     * {@code literalIf}, optionally wrapped in a single logical negation
     * ({@code !}). Anything more complex (e.g. {@code &&}, {@code ||}) is not
     * recognized and yields {@code null}.
     *
     * @param literalIf the {@code LITERAL_IF} node
     * @return a match describing the {@code LITERAL_INSTANCEOF} node and
     *     whether it was negated, or {@code null} if the condition is not a
     *     simple (possibly negated) instanceof pattern
     */
    private InstanceOfMatch findInstanceOfCore(DetailAST literalIf) {
        ...
        if (node.getType() == TokenTypes.LITERAL_INSTANCEOF) {
            ...
            if (inner.getType() == TokenTypes.LITERAL_INSTANCEOF) {
     ...
     *     chain, or {@code null}
     */
    private DetailAST unwrapExpr(DetailAST ast) {
     ...
     */
    private String patternVariableName(DetailAST patternOwner) {
        String name = null;
     ...
     */
    private boolean thenBranchCompletesNormally(DetailAST literalIf) {
        final DetailAST rparen = literalIf.findFirstToken(TokenTypes.RPAREN);
     ...
     *     completion statement
     */
    private boolean isAbruptCompletionStatement(DetailAST stmt) {
        DetailAST last = stmt;
