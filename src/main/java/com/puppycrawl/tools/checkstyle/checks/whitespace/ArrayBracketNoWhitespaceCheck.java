...
public class ArrayBracketNoWhitespaceCheck extends AbstractCheck {
    ...
    private static DetailAST findNextToken(DetailAST rightBracket) {
        ...
        while (current.getLineNo() == lineNo) {
            ...
            // scan all next siblings
            DetailAST sibling = current;
            while (sibling != null && sibling.getLineNo() == lineNo) {
                if (sibling.getColumnNo() > columnNo) {
                    if (nextToken == null
                            || sibling.getColumnNo() < nextToken.getColumnNo()) {
                        nextToken = sibling;
                    }
                    // siblings are in order, no need to scan further
                    break;
                }
