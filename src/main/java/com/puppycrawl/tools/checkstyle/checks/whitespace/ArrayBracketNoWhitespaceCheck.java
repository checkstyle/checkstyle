...
public class ArrayBracketNoWhitespaceCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        // Check left bracket
        final boolean isPrecededByAnnotation = isPrecededByAnnotation(ast);
        final boolean isPrecededByWhitespace = precededByWhitespace(ast);
        if (!isPrecededByAnnotation && isPrecededByWhitespace) {
            log(ast, MSG_WS_PRECEDED, LEFT_BRACKET);
        }
        else if (isPrecededByAnnotation && !isPrecededByWhitespace) {
            log(ast, MSG_WS_NOT_PRECEDED, LEFT_BRACKET);
        }
        ...
        processRightBracket(ast, rightBracket);
    }

    /**
     * Processes the right bracket for whitespace rules.
     *
     ...
     */
    private void processRightBracket(DetailAST ast, DetailAST rightBracket) {
    ...
    /**
     * Iteratively scans the subtree rooted at {@code root} using a depth-first
     * traversal and returns the token that is on the same line as the bracket,
     * starts at or after {@code column}, and has the smallest column number
     * among all such candidates. An iterative approach is used to avoid
     * {@link StackOverflowError} on deeply nested ASTs.
     *
     * @param root the subtree root to scan
     ...
     */
    private static DetailAST findClosestTokenAfter(DetailAST root, int line, int column) {
        final Deque<DetailAST> stack = new ArrayDeque<>();
        stack.push(root);

        DetailAST closest = null;
        while (!stack.isEmpty()) {
            final DetailAST node = stack.pop();

            if (isTokenAfter(node, line, column)
                    && (closest == null || node.getColumnNo() <= closest.getColumnNo())) {
                closest = node;
            }
     ...
     */
    private static boolean isPostfix(int type) {
