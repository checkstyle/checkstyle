...
public class ArrayBracketNoWhitespaceCheck extends AbstractCheck {
    ...
    /**
     * Finds the next token after a right bracket by climbing to the root of
     * the AST and scanning next-sibling chains at every level for a same-line
     * token whose column exceeds the bracket's column. The candidate with the
     * smallest qualifying column is returned.
     *
    ...
    private static DetailAST findNextToken(DetailAST rightBracket) {
        ...
        while (current != null) {
            ...
            final DetailAST fromSiblings =
                getNextSameLine(current.getNextSibling(), lineNo, columnNo);
