...
public class ArrayBracketNoWhitespaceCheck extends AbstractCheck {
    ...
    private static DetailAST findNextToken(DetailAST rightBracket) {
        ...
        final int lineNo = rightBracket.getLineNo();
        DetailAST current = rightBracket;
        DetailAST nextToken = null;

        while (current.getLineNo() == lineNo) {
            final DetailAST parent = current.getParent();
            final DetailAST fromSiblings = getNextSameLine(current, lineNo, columnNo);
            if (fromSiblings != null
                    && (nextToken == null
                            || fromSiblings.getColumnNo() < nextToken.getColumnNo())) {
                nextToken = fromSiblings;
            }
            nextToken = getNextTokenFromParent(parent, lineNo, columnNo, nextToken);
            current = parent;
        }

        if (nextToken == null) {
            nextToken = scanAncestorSiblings(current, lineNo, columnNo);
        }
        return nextToken;
    }

    /**
     * Climbs ancestors that are on a different line than {@code lineNo}
     * and scans their next siblings for a same-line token after {@code columnNo}.
     * This handles multi-line declarations where the enclosing statement node
     * (e.g. {@code VARIABLE_DEF}) started on an earlier line but its sibling
     * (e.g. {@code SEMI}) is still on {@code lineNo}.
     *
     * @param start the node at which the same-line traversal stopped
     * @param lineNo the target line number
     * @param columnNo the column of the right bracket
     * @return the closest same-line token after {@code columnNo}, or {@code null}
     */
    private static DetailAST scanAncestorSiblings(DetailAST start,
        ...
        while (result == null) {
            ...
            result = getNextSameLine(current.getNextSibling(), lineNo, columnNo);
            if (result == null) {
                result = getNextTokenFromParent(parent, lineNo, columnNo, null);
            }
            current = parent;
        }
        return result;
    ...
    /**
     * Scans a sibling chain starting at {@code sibling} and returns the first
     * token on {@code lineNo} whose column exceeds {@code columnNo}, or
     * {@code null} if none is found before passing {@code lineNo}.
     *
     * @param sibling the first sibling to inspect (may be {@code null})
     * @param lineNo the target line number
     * @param columnNo the column of the right bracket
     * @return the matching sibling, or {@code null}
     */
    private static DetailAST getNextSameLine(DetailAST sibling,
            int lineNo, int columnNo) {
        DetailAST result = null;
        DetailAST current = sibling;
        while (current != null && result == null) {
            if (current.getLineNo() == lineNo && current.getColumnNo() > columnNo) {
                result = current;
            }
            else if (current.getLineNo() > lineNo) {
                break;
            }
            else {
    ...
    /**
     * Checks if the parent token is a better candidate for the next token.
     *
     * @param parent the parent token
     * @param lineNo the line number of the right bracket
     * @param columnNo the column number of the right bracket
     * @param nextToken the current best candidate for the next token
     * @return the new best candidate for the next token
     */
    private static DetailAST getNextTokenFromParent(DetailAST parent, int lineNo, int columnNo,
            DetailAST nextToken) {
        DetailAST result = nextToken;
        if (parent.getLineNo() == lineNo && parent.getColumnNo() > columnNo
                && (result == null || parent.getColumnNo() < result.getColumnNo())) {
            result = parent;
        }
        return result;
    }
