 ...
 * <li>Allow underscores to separate segements and digits.</li>
...
public class GoogleMethodNameCheck extends AbstractCheck {
     ...
     * <ul>
     * <li>{@code ^} - Start of string</li>
     * <li>{@code (?![a-z][A-Z])} - Negative lookahead: rejects names like "fO"
     * where single lowercase is immediately followed by uppercase</li>
     * <li>{@code [a-z]} - Must start with lowercase letter</li>
     * <li>{@code [a-z0-9]++} - One or more lowercase letters or digits
     * (ensures minimum 2 characters)</li>
     * <li>{@code (?:[A-Z][a-z0-9]*+)*+} - Zero or more camelCase humps, each
     * uppercase followed by zero or more lowercase/digits</li>
    ...
    private static final Pattern REGULAR_METHOD_NAME_PATTERN = Pattern
            .compile("^(?![a-z][A-Z])[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?$");
    ...
    private static final Pattern TEST_METHOD_NAME_PATTERN = Pattern.compile(
            "^(?![a-z][A-Z])(?:[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?$"
                    + "|[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?"
                    + "(?:_(?![a-z][A-Z])[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?)+)$");
    /**
