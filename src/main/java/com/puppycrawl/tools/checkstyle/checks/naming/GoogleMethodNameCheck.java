 ...
 * </div>
 *
 * <p>Regular method names must:</p>
 * <ul>
 * <li>Be at least 2 characters long.</li>
 * <li>Start with a lowercase letter.</li>
 * <li>Not have a single lowercase letter followed by uppercase.</li>
 * <li>Contain only letters and digits and underscores.</li>
 * </ul>
 *
 * <p>Test method names must:</p>
 * <ul>
 * <li>Be at least 2 characters long per segment.</li>
 * <li>Start each segment with a lowercase letter.</li>
 * <li>Not have a single lowercase letter followed by uppercase in any segment.</li>
 * <li>Allow underscores to separate segment and digits.</li>
 * <li>Not have leading, trailing, or consecutive underscores.</li>
 * </ul>
 *
 * <p>
 * Notes:
 * Methods annotated with {@code @Override} are ignored, as they must
 * match the name defined in the parent class or interface.
 * </p>
 *
 * @since 13.9.0
 */
...
public class GoogleMethodNameCheck extends AbstractCheck {
     ...
     */
    public static final String MSG_KEY_CHARACTER_USAGE = "google.method.name.character.usage";
    ...
    /**
     * Pattern for validating characters allowed in a method name.
     *
     * <p>
     * Accepts any non-empty string consisting solely of alphanumeric characters
     * and underscores, in any order.
     *
     * <p>
     * Explanation:
     * <ul>
     * <li>{@code ^} - Start of string</li>
     * <li>{@code [0-9a-zA-Z_]} - Character class matching:
     *   <ul>
     *     <li>{@code 0-9} - Any digit</li>
     *     <li>{@code a-z} - Any lowercase letter</li>
     *     <li>{@code A-Z} - Any uppercase letter</li>
     *     <li>{@code _} - Underscore</li>
     *   </ul>
     * </li>
     * <li>{@code +} - One or more of the above characters (empty strings are rejected)</li>
     * <li>{@code $} - End of string</li>
     * </ul>
     */
    private static final Pattern VALID_CHARACTERS_IN_METHOD_NAME = Pattern
            .compile("^[0-9a-zA-Z_]+$");
     ...
     * <ul>
     * <li>{@code ^} - Start of string</li>
     * <li>{@code [a-z]} - Must start with a single lowercase letter</li>
     * <li>{@code [a-z0-9]++} - One or more lowercase letters or digits possessively
     * (ensures the initial lowercase block is at least 2 characters long)</li>
     * <li>{@code (?:[A-Z][a-z0-9]++)*+} - Zero or more camelCase humps possessively,
     * where each uppercase letter must be followed by one or more lowercase letters or digits</li>
     * <li>{@code [A-Z]?} - An optional single uppercase letter allowed at the very end</li>
     * <li>{@code $} - End of string</li>
     * </ul>
     */
    private static final Pattern REGULAR_METHOD_NAME_PATTERN = Pattern
            .compile("^[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?$");
    /**
     * Pattern for valid test method names in Google style.
     * <ul>
     * <li>Single segment (no underscore): follows regular method naming rules</li>
     * <li>Multi-segment (with underscore): each segment must be a valid lowerCamelCase name</li>
     * <li>Each segment must start with lowercase</li>
     * <li>Each segment must be at least 2 characters long</li>
     * <li>No segment may start with a single lowercase followed by uppercase (e.g., "fO")</li>
     * </ul>
     */
    private static final Pattern TEST_METHOD_NAME_PATTERN = Pattern.compile(
            "^(?:[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?$"
                    + "|[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?"
                    + "(?:_[a-z][a-z0-9]++(?:[A-Z][a-z0-9]++)*+[A-Z]?)+)$");
    ...
    /**
     * Matches invalid underscore usage for test methods: leading, trailing, or double.
     */
    private static final Pattern INVALID_UNDERSCORE_PATTERN_TEST =
        Pattern.compile("^_|_$|__|[a-zA-Z]_\\d|\\d_[a-zA-Z]");

    /**
     * Set of JUnit test annotation names that indicate a test method.
     */
    ...
    /** Creates a new {@code GoogleMethodNameCheck} instance. */
    ...
    public void visitToken(DetailAST ast) {
        if (!AnnotationUtil.hasOverrideAnnotation(ast)) {
            ...
            if (VALID_CHARACTERS_IN_METHOD_NAME.matcher(methodName).matches()) {
                if (methodName.startsWith("test") || hasTestAnnotation(ast)) {
                ...
                log(nameAst, MSG_KEY_CHARACTER_USAGE, methodName);
            }
        }
    }
