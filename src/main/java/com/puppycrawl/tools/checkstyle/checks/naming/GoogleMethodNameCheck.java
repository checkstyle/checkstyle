 ...
 * enforce Google Java Style and handles test method annotations internally.
 * </p>
 *
 * @since 12.3.1
 */
...
public class GoogleMethodNameCheck extends AbstractCheck {
     ...
     */
    public static final String MSG_KEY = "google.method.name";
     ...
     * Explanation:
     * <ul>
     * <li>{@code ^(?![a-z]$)} - Negative lookahead: cannot be single lowercase
     * char</li>
    ...
    /**
     * Pattern to detect if name ends with a valid numbering suffix.
     * A valid numbering suffix is underscore followed by digits only at the end.
     */
    private static final Pattern NUMBERING_SUFFIX_PATTERN = Pattern.compile("(?:_[0-9]+)+$");

    /**
     * Pattern to detect invalid underscore usage - underscore between letter and digit
     * or between digit and letter.
     */
    private static final Pattern INVALID_UNDERSCORE_PATTERN =
            Pattern.compile("[a-zA-Z]_[0-9]|[0-9]_[a-zA-Z]");
    ...
    private void validateRegularMethodName(DetailAST nameAst, String methodName) {
        if (INVALID_UNDERSCORE_PATTERN.matcher(methodName).find()) {
            log(nameAst, MSG_KEY, methodName);
        }
        else {
    ...
    private void validateTestMethodName(DetailAST nameAst, String methodName) {
        if (!TEST_METHOD_NAME_PATTERN.matcher(methodName).matches()) {
            log(nameAst, MSG_KEY, methodName);
        }
    }
}
