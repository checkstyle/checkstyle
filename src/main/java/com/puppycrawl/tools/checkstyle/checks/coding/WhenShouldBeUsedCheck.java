 ...
 * </p>
 * <p>
 * Java 21 has introduced enhancements for switch statements and expressions
...
public class WhenShouldBeUsedCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        final DetailAST statementList = getStatementListOfCase(ast);
        final boolean hasSingleIfStatement = statementList != null
                                                && hasSingleIfStatementInCaseBlock(statementList);
        final boolean hasPatternLabel = hasPatternLabel(ast);
        if (hasPatternLabel && hasSingleIfStatement) {
            log(ast, MSG_KEY);
        }
     ...
     */
    private static DetailAST getStatementListOfCase(DetailAST caseAST) {
    ...
    /**
     * Check if the case block has a single if statement.
     *
     * @param statementList the AST node representing {@code SLIST} of the current case
     * @return true if the case block has a single if statement, false otherwise
     */
    private static boolean hasSingleIfStatementInCaseBlock(DetailAST statementList) {
        final int[] acceptableStatementsInCaseBlock = {
            TokenTypes.LITERAL_BREAK,
            TokenTypes.EMPTY_STAT,
            TokenTypes.RCURLY,
        };
        boolean hasSingleIfStatementInCaseBlock = false;
        boolean hasIfBranch = false;
        DetailAST ast = statementList.getFirstChild();

        while (ast != null) {
            if (ast.getType() == TokenTypes.LITERAL_IF && !hasIfBranch) {
                hasSingleIfStatementInCaseBlock =
                        ast.findFirstToken(TokenTypes.LITERAL_ELSE) == null;
                hasIfBranch = true;
            }
            else if (!TokenUtil.isOfType(ast, acceptableStatementsInCaseBlock)) {
                hasSingleIfStatementInCaseBlock = false;
                break;
            }
            ast = ast.getNextSibling();
        }
        return hasSingleIfStatementInCaseBlock;
    }
