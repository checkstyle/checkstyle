...
public class NoWhitespaceAfterCheck extends AbstractCheck {
    ...
    private static boolean shouldCheckWhitespaceAfter(DetailAST ast) {
        ...
        if (sibling != null) {
            ...
            else if (sibling.getType() == TokenTypes.ANNOTATIONS) {
    ...
    private static DetailAST getTypeLastNode(DetailAST ast) {
        ...
        final Optional<DetailAST> objectArray = Optional.ofNullable(getIdentLastToken(ast));

        if (isPrecededByTypeArgs) {
            typeLastNode = parent.findFirstToken(TokenTypes.TYPE_ARGUMENTS)
                    .findFirstToken(TokenTypes.GENERIC_END);
        }
        else if (objectArray.isPresent()) {
            // Object array type
            typeLastNode = objectArray.get();
        }
        else if (tokenType == TokenTypes.ARRAY_DECLARATOR) {
            typeLastNode = ast.getParent().getFirstChild();
        }
        else {
            // primitive array type
            typeLastNode = ast.getFirstChild();
        }
    ...
    private static DetailAST getIdentLastToken(DetailAST ast) {
        final DetailAST result;
        final DetailAST dot = ast.getParent().findFirstToken(TokenTypes.DOT);
        // method call case
        if (dot == null) {
