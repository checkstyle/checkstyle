...
public class RequireThisCheck extends AbstractCheck {
     ...
     */
    private static boolean isAnyParentResource(DetailAST ast) {
        DetailAST currentNode = ast.getParent();
        boolean isAnyParentResource = false;
        while (currentNode != null && currentNode.getType() != TokenTypes.METHOD_DEF) {
            if (currentNode.getType() == TokenTypes.RESOURCE) {
