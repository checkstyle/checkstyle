...
public class RequireThisCheck extends AbstractCheck {
    ...
    private static DetailAST getBlockEndToken(DetailAST blockNameIdent, DetailAST blockStartToken) {
        ...
        if (blockNameIdentParent.getType() == TokenTypes.CASE_GROUP) {
            ...
            final List<DetailAST> rcurlyTokens = getAllTokensOfType(blockNameIdent,
                    TokenTypes.RCURLY);
    ...
    private static boolean isReturnedVariable(AbstractFrame currentFrame, DetailAST ident) {
        ...
        if (!returnsInsideBlock.isEmpty()) {
            returnedVariable = isAstInside(returnsInsideBlock.get(returnsInsideBlock.size() - 1), ident);
        }
     ...
     */
    private static List<DetailAST> getAllTokensWhichAreEqualToCurrent(DetailAST ast, DetailAST token,
                                                                     int endLineNumber) {
        DetailAST vertex = ast;
        final List<DetailAST> result = new ArrayList<>();
        final Deque<DetailAST> stack = new ArrayDeque<>();
