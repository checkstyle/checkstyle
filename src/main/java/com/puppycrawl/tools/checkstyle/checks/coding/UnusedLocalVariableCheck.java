...
public class UnusedLocalVariableCheck extends AbstractCheck {
    ...
    private void visitTypeDeclarationToken(DetailAST typeDeclAst) {
        if (isNonLocalTypeDeclaration(typeDeclAst)) {
            ...
            final TypeDeclDesc currClass = new TypeDeclDesc(qualifiedName, depth, typeDeclAst);
            depth++;
            typeDeclarations.push(currClass);
            typeDeclAstToTypeDeclDesc.put(typeDeclAst, currClass);
        }
    }
    ...
    private TypeDeclDesc getSuperClassOfAnonInnerClass(DetailAST literalNewAst) {
        ...
        final List<TypeDeclDesc> classesWithSameName = typeDeclWithSameName(shortNameOfClass);
