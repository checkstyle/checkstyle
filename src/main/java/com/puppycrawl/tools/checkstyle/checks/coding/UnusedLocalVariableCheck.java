...
public class UnusedLocalVariableCheck extends AbstractCheck {
     ...
     */
    private static boolean isIdentUsedForVariable(DetailAST identAst, VariableDesc variableDesc) {
    ...
    private static void setScopeOfPatternVariable(DetailAST instanceOfAst,
        ...
        while (TokenUtil.isOfType(ancestorOfInstanceOfAst.getType(), TokenTypes.LAND,
                TokenTypes.LNOT, TokenTypes.QUESTION)) {
            if (shouldScopeBeUpdated(patternVariable, ancestorOfInstanceOfAst,
                    scopeLineStart, scopeLineEnd)) {
                final int[] updatedScope = getUpdatedScopeValues(ancestorOfInstanceOfAst);
                scopeLineEnd = updatedScope[PatternVariableScope.LINE_END];
                scopeColumnEnd = updatedScope[PatternVariableScope.COLUMN_END];
            }
    ...
    /**
     * Should scope of pattern variable be updated.
     *
     * @param patternVariable DetailAst of type {@link TokenTypes#LITERAL_INSTANCEOF}
     * @param ancestorOfInstanceOfAst ancestor of {@link TokenTypes#LITERAL_INSTANCEOF} node
     * @param scopeLineStart starting line of scope
     * @param scopeLineEnd ending line of scope
     * @return true if scope of pattern variable should be updated else false
     */
    private static boolean shouldScopeBeUpdated(DetailAST patternVariable,
                                                DetailAST ancestorOfInstanceOfAst,
                                                int scopeLineStart, int scopeLineEnd) {
        return (!(ancestorOfInstanceOfAst.getType() == TokenTypes.QUESTION
                && (ancestorOfInstanceOfAst.getLineNo() < scopeLineStart
                || ancestorOfInstanceOfAst.getLineNo() == scopeLineStart
                && ancestorOfInstanceOfAst.getColumnNo() < patternVariable.getColumnNo()))
                && ancestorOfInstanceOfAst.getLineNo() >= scopeLineEnd);
    }

    /**
     * Get the updated scopeLineEnd and scopeColumnEnd values.
     *
     * @param ancestorOfInstanceOfAst ancestor of {@link TokenTypes#LITERAL_INSTANCEOF} node
     * @return updated scopeLineEnd and scopeColumnEnd values
     */
    private static int[] getUpdatedScopeValues(DetailAST ancestorOfInstanceOfAst) {
        final int scopeLineEnd;
        final int scopeColumnEnd;
        DetailAST childAst = ancestorOfInstanceOfAst.getLastChild();

        // Temporary comment, I feel while(true) condition is more clear,
        // but if a flag would be considered better, I will modify the condition.
        while(true) {
        ...
        scopeLineEnd = childAst.getLineNo();
        scopeColumnEnd = childAst.getColumnNo();

        // Additional -1 added to comply with indexes in PatternVariableScope.
        return new int[] {-1, scopeLineEnd, scopeColumnEnd};
    }
    ...
    private static void setScopeOfPatternVariableExcludingLineAndColumn(
        ...
        final boolean IsAFullScopeVariable = ast.getType() == TokenTypes.EXPR
                && !TokenUtil.isOfType(grandParent.getType(), TokenTypes.ELIST,
                TokenTypes.LITERAL_DO);

        boolean isATernaryOperatorExpression = ast.getFirstChild().getType() ==
                TokenTypes.QUESTION;
        ...
        if(!isATernaryOperatorExpression) {
            isATernaryOperatorExpression = ast.getFirstChild().getType() ==
                    TokenTypes.QUESTION;
        }
