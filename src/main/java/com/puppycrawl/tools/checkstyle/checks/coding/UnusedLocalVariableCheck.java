...
public class UnusedLocalVariableCheck extends AbstractCheck {
    ...
    /**
     * Keeps track of all the classes, interfaces, annotated interfaces, records
    ...
    /**
     * Visit ast of type {@link TokenTypes#CLASS_DEF} or {@link TokenTypes#ENUM_DEF} or
     * {@link TokenTypes#INTERFACE_DEF} or {@link TokenTypes#ANNOTATION_DEF} or
     * {@link TokenTypes#RECORD_DEF}.
     *
     * @param ast ast
     */
    ...
    private static boolean isNonLocalClassOrInterface(DetailAST ast) {
        return ast.getParent().getType() != TokenTypes.SLIST
                && TokenUtil.isTypeDeclaration(ast.getType());
    }
    ...
    private void modifyVariablesStack(ClassDesc obtainedClass,
        ...
        if (obtainedClass != null) {
            final Deque<VariableDesc> instAndClassVarDeque = classAstToClassDesc
                    .get(obtainedClass.getClassOrInterfaceAst())
    ...
    private List<ClassDesc> classesWithSameName(String className) {
        return classAstToClassDesc.values().stream()
                .filter(classDesc -> {
                    return classDesc.getQualifiedName()
                            .endsWith(PACKAGE_SEPARATOR.concat(className));
                })
