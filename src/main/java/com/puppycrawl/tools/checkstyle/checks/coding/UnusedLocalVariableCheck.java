...
public class UnusedLocalVariableCheck extends AbstractCheck {
     ...
     * containing it.
     */
    private final Map<DetailAST, ClassDesc> outerClassOfLocalAnonInnerClasses;
    ...
    public UnusedLocalVariableCheck() {
        ...
        outerClassOfLocalAnonInnerClasses = new HashMap<>();
    ...
    public void beginTree(DetailAST root) {
        ...
        outerClassOfLocalAnonInnerClasses.clear();
     ...
     */
    private DetailAST getBlockContainingLocalAnonInnerClass(DetailAST literalNewAst) {
    ...
    private void addInstanceVarOrClassVar(DetailAST varDefAst) {
        ...
        if (parentAst.getType() == TokenTypes.OBJBLOCK
            ...
            final VariableDesc desc = new VariableDesc(ident.getText(),
                    varDefAst.findFirstToken(TokenTypes.TYPE), findScopeOfVariable(varDefAst));
            classesWithInstVarAndClassVar.get(parentAst.getParent()).addInstOrClassVar(desc);
        }
    }
    ...
    private void visitLocalAnonInnerClass(DetailAST literalNewAst) {
        outerClassOfLocalAnonInnerClasses.put(literalNewAst, classes.peek());
        if (blockContainingLocalAnonInnerClass == null) {
            blockContainingLocalAnonInnerClass = getBlockContainingLocalAnonInnerClass(literalNewAst);
    ...
    private ClassDesc getSuperClassOfAnonInnerClass(DetailAST literalNewAst) {
        ...
        if (!classesWithSameName.isEmpty()) {
            obtainedClass = getTheNearestClass(
                    outerClassOfLocalAnonInnerClasses.get(literalNewAst).getQualifiedName(),
                    classesWithSameName);
        }
    ...
    private void modifyVariablesStack(ClassDesc obtainedClass,
        ...
        if (obtainedClass != null) {
            final Deque<VariableDesc> instAndClassVarDeque = classesWithInstVarAndClassVar.get(
