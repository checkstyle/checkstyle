...
public class UnusedLocalVariableCheck extends AbstractCheck {
        ...
        if (TokenUtil.isOfType(ast, SCOPES)) {
            while (!variables.isEmpty() && variables.peek().getScope() == ast) {
                ...
                if (!variableDesc.isUsed()
                        && !variableDesc.isInstanceVarInLocalAnonymousInnerClass()) {
    ...
    private void visitVariableDefToken(DetailAST varDefAst) {
        final boolean isInstanceVarInLocalAnonymousInnerClass =
                isInstanceVarInLocalAnonymousInnerClass(varDefAst);
     ...
     */
    private static boolean isInstanceVarInLocalAnonymousInnerClass(DetailAST variableDefAst) {
        final DetailAST objBlockAst = variableDefAst.getParent();
        final DetailAST literalNewAst = objBlockAst.getParent();
        final DetailAST grandParent = literalNewAst.getParent().getParent();
        return objBlockAst.getType() == TokenTypes.OBJBLOCK
                && literalNewAst.getType() == TokenTypes.LITERAL_NEW
                && TokenUtil.isOfType(TokenTypes.SLIST, grandParent.getParent().getType(),
    ...
    private void checkIdentifierAst(DetailAST identAst) {
        for (VariableDesc variableDesc : variables) {
            if (identAst.getText().equals(variableDesc.getName())
                ...
                if (!variableDesc.isInstanceVarInLocalAnonymousInnerClass()) {
    ...
    private static final class VariableDesc {
         ...
         */
        private final boolean instanceVarInLocalAnonymousInnerClass;
         ...
         * @param instanceVarInLocalAnonymousInnerClass Is an instance variable in local
         *                                              anonymous inner class.
         */
        /* package */ VariableDesc(String name, DetailAST typeAst, DetailAST scope,
                boolean instanceVarInLocalAnonymousInnerClass) {
            this.name = name;
            this.typeAst = typeAst;
            this.scope = scope;
            this.instanceVarInLocalAnonymousInnerClass = instanceVarInLocalAnonymousInnerClass;
        }
         ...
         */
        public boolean isInstanceVarInLocalAnonymousInnerClass() {
            return instanceVarInLocalAnonymousInnerClass;
        }
    }
}
