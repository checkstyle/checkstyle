...
public class UnusedLocalVariableCheck extends AbstractCheck {
    ...
    /**
     * Maps classes and interfaces to their respective ClassDesc objects.
     */
    private final Map<DetailAST, ClassDesc> classesWithInstVarAndClassVar;

    /**
     * Maps local anonymous inner class to the ClassDesc object of the class
     * containing it.
     */
    private final Map<DetailAST, ClassDesc> outerClassOfAnonInnerClasses;
     ...
     */
    private DetailAST blockContainingAnonInnerClass;
    ...
    public UnusedLocalVariableCheck() {
        ...
        classesWithInstVarAndClassVar = new HashMap<>();
        outerClassOfAnonInnerClasses = new HashMap<>();
        anonInnerClassHolders = new ArrayDeque<>();
        blockContainingAnonInnerClass = null;
        packageName = "";
        depth = 0;
    }
    ...
    public void beginTree(DetailAST root) {
        // No need to set blockContainingAnonInnerClass to null, if
        // its value gets changed during the check then it is changed back to null
        // inside the check only.
        variables.clear();
        classes.clear();
        classesWithInstVarAndClassVar.clear();
        outerClassOfAnonInnerClasses.clear();
    ...
    public void leaveToken(DetailAST ast) {
        if (blockContainingAnonInnerClass == null
                && TokenUtil.isOfType(ast, SCOPES)) {
            logViolations(ast, variables);
        }
        else if (ast.getType() == TokenTypes.COMPILATION_UNIT) {
            leaveCompilationUnit();
        }
        else if (ast.equals(blockContainingAnonInnerClass)) {
            blockContainingAnonInnerClass = null;
        }
     ...
     */
    private DetailAST getBlockContainingAnonInnerClass(DetailAST literalNewAst) {
    ...
    private void addInstanceVarOrClassVar(DetailAST varDefAst) {
        ...
        if (parentAst.getType() == TokenTypes.OBJBLOCK
            ...
            final VariableDesc desc = new VariableDesc(ident.getText(),
                    varDefAst.findFirstToken(TokenTypes.TYPE), findScopeOfVariable(varDefAst));
            classesWithInstVarAndClassVar.get(parentAst.getParent())
                    .getInstanceAndClassVarStack().push(desc);
        }
    }
    ...
    private void visitClassOrInterfaceToken(DetailAST ast) {
        if (isNonLocalClassOrInterface(ast)) {
            ...
            classesWithInstVarAndClassVar.put(ast, currClass);
        }
    }
     ...
     */
    private boolean isNonLocalClassOrInterface(DetailAST ast) {
    ...
    private void visitLocalAnonInnerClass(DetailAST literalNewAst) {
        outerClassOfAnonInnerClasses.put(literalNewAst, classes.peek());
        if (blockContainingAnonInnerClass == null) {
            blockContainingAnonInnerClass = getBlockContainingAnonInnerClass(literalNewAst);
            anonInnerClassHolders.add(blockContainingAnonInnerClass);
        }
    }

    /**
     * Leave the ast node of type {@link TokenTypes#COMPILATION_UNIT}.
    ...
    private void leaveCompilationUnit() {
        while (!anonInnerClassHolders.isEmpty()) {
            ...
            iterateOverBlockContainingAnonInnerClass(holder, new ArrayDeque<>());
        }
    }
    ...
    private ClassDesc getSuperClassOfAnonInnerClass(DetailAST literalNewAst) {
        ...
        if (!classesWithSameName.isEmpty()) {
            obtainedClass = getTheNearestClass(
                    outerClassOfAnonInnerClasses.get(literalNewAst).getQualifiedName(),
                    classesWithSameName);
        }
     ...
     */
    public String getShortNameOfAnonInnerClass(DetailAST literalNewAst) {
        final StringBuilder shortNameOfClass = new StringBuilder(256);
        DetailAST identAst = literalNewAst.findFirstToken(TokenTypes.IDENT);
        if (identAst == null) {
            shortNameOfClass.insert(0,
                    extractQualifiedName(literalNewAst.findFirstToken(TokenTypes.DOT)));
        }
        else {
            shortNameOfClass.insert(0, identAst.getText());
        }
        DetailAST probableLiteralNewAst = literalNewAst.getPreviousSibling();
        while (probableLiteralNewAst != null
                && probableLiteralNewAst.getType() == TokenTypes.LITERAL_NEW) {
            identAst = probableLiteralNewAst.findFirstToken(TokenTypes.IDENT);
            if (identAst == null) {
                shortNameOfClass.insert(0, extractQualifiedName(
                        probableLiteralNewAst.findFirstToken(TokenTypes.DOT)) + PACKAGE_SEPARATOR);
            }
            else {
                shortNameOfClass.insert(0, identAst.getText() + PACKAGE_SEPARATOR);
            }
            probableLiteralNewAst = probableLiteralNewAst.getPreviousSibling();
        }
        return shortNameOfClass.toString();
    }
    ...
    private void modifyVariablesStack(ClassDesc obtainedClass,
        ...
        if (obtainedClass != null) {
            final Deque<VariableDesc> instVarDeque = classesWithInstVarAndClassVar.get(
                    obtainedClass.getClassOrInterfaceAst()).getInstanceAndClassVarStack();
            final DetailAST updatedScope = literalNewAst.getLastChild();
            final Deque<VariableDesc> instVarDequeToBeAdded = new ArrayDeque<>();
            instVarDeque.forEach(instVar -> {
                ...
                instVarDequeToBeAdded.push(variableDesc);
            });
            instVarDequeToBeAdded.forEach(variablesStack::push);
        }
    }
    ...
    private List<ClassDesc> classesWithSameName(String className) {
        return classesWithInstVarAndClassVar.values().stream()
                .filter(classDesc -> {
     ...
     * @param variablesStack stack of all the relevant variables in the scope
     */
    private void iterateOverBlockContainingAnonInnerClass(
    ...
    private static boolean isInsideLocalAnonInnerClass(DetailAST literalNewAst) {
        final DetailAST parentAst = literalNewAst.getParent();
        final DetailAST lastChild = literalNewAst.getLastChild();
        final DetailAST ancestor = parentAst.getParent().getParent();
        return lastChild != null
                && lastChild.getType() == TokenTypes.OBJBLOCK
                && TokenUtil.isOfType(TokenTypes.SLIST,
                ancestor.getType(),
                ancestor.getParent().getType(),
                ancestor.getParent().getParent().getType());
    }
    ...
    private static class ClassDesc {
        ...
        /**
         * Get the instance and class variables stack.
         *
         * @return instance and class variables stack
         */
        public Deque<VariableDesc> getInstanceAndClassVarStack() {
            return instanceAndClassVarStack;
        }
    }
}
