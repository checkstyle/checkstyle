...
public class UnusedLocalVariableCheck extends AbstractCheck {
            ...
            case TokenTypes.VARIABLE_DEF:
                if (ast.getParent().getType() != TokenTypes.OBJBLOCK) {
                    ...
                    final VariableDesc desc = new VariableDesc(ident.getText(), ast,
                            findScopeOfVariable(ast));
                    variables.push(desc);
                }
                break;
            case TokenTypes.IDENT:
                if (!TokenUtil.isOfType(ast.getParent(),
        ...
        if (TokenUtil.isOfType(ast, SCOPES)) {
            while (!variables.isEmpty() && variables.peek().getScope() == ast) {
                ...
                if (!variableDesc.isUsed()) {
                    ...
                    log(currAst.findFirstToken(TokenTypes.TYPE), MSG_UNUSED_LOCAL_VARIABLE, variableDesc.name);
                }
            }
        }
    }
    ...
    private static final class VariableDesc {
         ...
         */
        /* package */ VariableDesc(String name, DetailAST variableAst, DetailAST scope) {
            this.name = name;
