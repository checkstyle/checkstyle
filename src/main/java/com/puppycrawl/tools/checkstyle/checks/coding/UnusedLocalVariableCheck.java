...
public class UnusedLocalVariableCheck extends AbstractCheck {
    ...
    private static boolean isStandAloneIncrementOrDecrement(DetailAST identAst) {
        ...
        return TokenUtil.isOfType(parent, INCREMENT_AND_DECREMENT_TOKENS)
                ...
                && !isPrefixOrPostfixOnRightHandSide(identAst);
    }

    /**
     * Checks whether the DetailAst of type {@link TokenTypes#IDENT} is used
     * in prefix or postfix expression on the right-hand side.
     *
     * @param identAst DetailAst of type {@link TokenTypes#IDENT}
     * @return true if identAst is used in prefix or postfix expression
     *         on the right-hand side.
     */
    private static boolean isPrefixOrPostfixOnRightHandSide(DetailAST identAst) {
        final DetailAST grandParent = identAst.getParent().getParent();
        return TokenUtil.isOfType(grandParent.getParent(),
