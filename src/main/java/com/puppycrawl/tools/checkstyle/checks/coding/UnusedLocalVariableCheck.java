...
public class UnusedLocalVariableCheck extends AbstractCheck {
        ...
        switch (ast.getType()) {
            case TokenTypes.DOT:
                final boolean checkIdentToken = !TokenUtil.findFirstTokenByPredicate(ast,
                                dotAst -> {
                                    return TokenUtil.isOfType(dotAst,
            ...
            case TokenTypes.VARIABLE_DEF:
                if (ast.getParent().getType() != TokenTypes.OBJBLOCK) {
                    ...
                    final VariableDesc desc = new VariableDesc(ident.getText(),
                            ast.findFirstToken(TokenTypes.TYPE), findScopeOfVariable(ast));
                    variables.push(desc);
                }
                break;
            case TokenTypes.IDENT:
                ...
                if (!TokenUtil.isOfType(parentAst, UNACCEPTABLE_PARENT_OF_IDENT)) {
                    if (parentAst.getType() == TokenTypes.METHOD_REF) {
                        if (shouldCheckMethodRefIdent(ast)) {
    ...
    /**
     * Whether ident token present as child of {@link TokenTypes#METHOD_REF} should
     * be checked or not.
     *
     * @param identAst DetailAst of type {@link TokenTypes#IDENT}
     * @return true if ident token present as child of {@link TokenTypes#METHOD_REF} should
     *         be checked else false
     */
    public static boolean shouldCheckMethodRefIdent(DetailAST identAst) {
        final DetailAST parent = identAst.getParent();
        return parent.getFirstChild() == identAst
