...
public class UnnecessaryNullCheckWithInstanceOfCheck extends AbstractCheck {
    ...
    private static DetailAST checkForUnnecessaryNullCheck(DetailAST nullComparisonNode) {
        ...
        final boolean found = currentParent.getType() == TokenTypes.LAND
            && isRedundantNullCheckWithInstanceof(currentParent, nullComparisonNode);
        ...
        if (node.getType() == TokenTypes.DOT
            ...
            while (firstChild != null) {
                if (firstChild.getType() == TokenTypes.IDENT
                    && variableName.equals(firstChild.getText())) {
                    found = true;
                    break;
                }

                if (containsVariableDereference(firstChild, variableName)) {
                    found = true;
                    break;
                }
