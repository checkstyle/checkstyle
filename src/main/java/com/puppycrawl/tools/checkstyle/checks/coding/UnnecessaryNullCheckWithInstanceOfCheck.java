...
public class UnnecessaryNullCheckWithInstanceOfCheck extends AbstractCheck {
     ...
     */
    @Nullable
    private static DetailAST checkForUnnecessaryNullCheck(DetailAST instanceOfNode) {
        DetailAST currentParent = instanceOfNode;

        while (currentParent.getParent().getType() == TokenTypes.LAND) {
            currentParent = currentParent.getParent();
        }
        final DetailAST found = findRedundantNullCheck(currentParent, instanceOfNode);
        DetailAST result = null;
        if (found != null) {
            result = found.getFirstChild();
        }
        return result;
    }
    ...
    private static boolean isNullCheckRedundant(DetailAST instanceOfIdent,
        ...
        return (isNullLiteral(nullCheckNode.getFirstChild().getNextSibling())
                || isNullLiteral(nullCheckNode.getFirstChild()))
                && nullCheckIdent != null && instanceOfIdent.getText()
                .equals(nullCheckIdent.getText());
    }
     ...
     */
    private static DetailAST findRedundantNullCheck(DetailAST logicalAndNode,
        DetailAST instanceOfNode) {
        DetailAST currentChild = logicalAndNode.getFirstChild();
        DetailAST nullCheck = null;
        final DetailAST instanceOfIdent = instanceOfNode.findFirstToken(TokenTypes.IDENT);

        if (instanceOfIdent != null && !containsVariableDereference(logicalAndNode,
                instanceOfIdent.getText())) {
            while (currentChild != null) {
                if (isNotEqual(currentChild)
                        && isNullCheckRedundant(instanceOfIdent, currentChild)) {
                    nullCheck = currentChild;
                }
                else if (nullCheck == null && currentChild.getType() == TokenTypes.LAND) {
                    nullCheck = findRedundantNullCheck(currentChild, instanceOfNode);
                }
