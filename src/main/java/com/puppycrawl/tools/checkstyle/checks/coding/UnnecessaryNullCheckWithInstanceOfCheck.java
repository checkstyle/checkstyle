...
public class UnnecessaryNullCheckWithInstanceOfCheck extends AbstractCheck {
    ...
    private static DetailAST checkForUnnecessaryNullCheck(DetailAST nullComparisonNode) {
        DetailAST currentParent = nullComparisonNode;
        ...
        final boolean found = isRedundantNullCheckWithInstanceof(currentParent, nullComparisonNode);
     ...
     * @return true, if the instance node is found and the null check is redundant, false otherwise
     */
    private static boolean isRedundantNullCheckWithInstanceof(DetailAST logicalAndNode,
        DetailAST nullCheckNode) {
        DetailAST currentChild = logicalAndNode.getFirstChild();
        boolean found = false;
        boolean variableDereferenced = false;
        final DetailAST nullCheckIdent = nullCheckNode.findFirstToken(TokenTypes.IDENT);
        while (currentChild != null && nullCheckIdent != null) {

            if (isInstanceofCheck(currentChild) && isNullCheckRedundant(nullCheckIdent,
                currentChild)) {
                found = true;
            }
            else if (containsVariableDereference(currentChild, nullCheckIdent.getText())) {
                variableDereferenced = true;
                break;
            }
            else if (!found && currentChild.getType() == TokenTypes.LAND) {
                found = isRedundantNullCheckWithInstanceof(currentChild,
                    nullCheckNode);
            }
            currentChild = currentChild.getNextSibling();
        }
        return found && !variableDereferenced;
    }
    ...
    /**
     * Returns true if the given node is followed by an argument list,
     * which usually means it's a method call.
     *
     * @param node the AST node to check
     * @return true if the next sibling is an ELIST, false otherwise
     */
    private static boolean isMethod(DetailAST node) {
        return node.getNextSibling() != null
            && node.getNextSibling().getType() == TokenTypes.ELIST;
    }
}
