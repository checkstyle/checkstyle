...
public final class VerticalAlignmentHandler {
    ...
    private static List<Integer> getAllowedVerticalAlignmentColumns(DetailAST node) {
        ...
        ).forEach(opt -> opt.ifPresent(columns::add));
        return columns;
    }

    private static Optional<Integer> getAllowedColumnForParameterDefinition(
            DetailAST node) {
        ...
        if (findAncestorByPredicate(
            ...
            result = findAncestorByPredicate(node,
    ...
    private static Optional<Integer> getAllowedColumnForExpressionInElist(
            DetailAST node) {
        Optional<Integer> result = Optional.empty();
        if (findAncestorByPredicate(
                node,
    ...
    private static Optional<Integer> getAllowedColumnForExpressionInArrayInit(
            DetailAST node) {
        Optional<Integer> result = Optional.empty();
        if (findAncestorByPredicate(
                node,
    ...
    private static Optional<Integer> getAllowedColumnForAnnotationMemberValuePair(
            DetailAST node) {
        Optional<Integer> result = Optional.empty();
        if (findAncestorByPredicate(
                node,
    ...
    private static Optional<Integer> getAllowedColumnForArithmeticBooleanExpression(
            DetailAST node) {
        Optional<Integer> result = findAncestorByPredicate(
                node,
                ancestor -> TokenUtil.isOfType(ancestor, TokenTypes.EXPR)
        ).map(expr -> getTopLeftmostNodeInTree(expr).getColumnNo());
        return result;
    ...
    private static Optional<Integer> getAllowedColumnForTernaryExpression(
            DetailAST node) {
        Optional<Integer> result = Optional.empty();
    ...
    private static Optional<Integer> getAllowedColumnForCommaSeparatedVar(
            DetailAST node) {
        Optional<Integer> result = Optional.empty();
        ...
        if (varDefOpt.isPresent()) {
            ...
            if (TokenUtil.isOfType(prevSibling, TokenTypes.COMMA)) {
                ...
                if (TokenUtil.isOfType(prevVarDef, TokenTypes.VARIABLE_DEF)) {
                    ...
                    if (ident != null) {
                        result = Optional.of(ident.getColumnNo());
                    }
                }
            }
        }
    ...
    private static Optional<Integer> getAllowedColumnForChainedCallOrField(
            DetailAST node) {
        Optional<Integer> result = Optional.empty();
        if (TokenUtil.isOfType(node, TokenTypes.DOT)) {
            ...
            if (TokenUtil.isOfType(firstChild,
                ...
                result = Optional.of(cur.getColumnNo());
            }
        }
        return result;
    }
