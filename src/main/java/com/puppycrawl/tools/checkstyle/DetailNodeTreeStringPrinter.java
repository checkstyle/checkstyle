...
public final class DetailNodeTreeStringPrinter {
    ...
    private static DetailNode parseFile(File file) throws IOException {
        ...
        final FileText fileText = new FileText(file, name);
        final CharSequence fullText = fileText.getFullText();
        final String string = fullText.toString();
        final DetailAST blockCommentNode = ParserUtil.createBlockCommentNode(string);
        return parseJavadocAsDetailNode(blockCommentNode);
    }
