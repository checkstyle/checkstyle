...
public final class InlineConfigUtils {
    ...
    /** Config start delimiter for .properties target files. */
    public static final String PROPERTIES_CONFIG_START = "#xml";

    /** Config end delimiter for .properties target files. */
    public static final String PROPERTIES_CONFIG_END = "#/xml";
        ...
        if (!lines.isEmpty()) {
            ...
            if (first.startsWith(JAVA_CONFIG_PREFIX)) {
                ...
                result = new MatchedDelimiter(XML_TARGET_CONFIG_END, true);
                    ...
                    && PROPERTIES_CONFIG_START.equals(first)) {
                result = new MatchedDelimiter(PROPERTIES_CONFIG_END, true);
            }
        }
        return result;
    }
    ...
    public static String describeExpectedDelimiters(String filePath) {
        ...
        if (filePath.endsWith(PROPERTIES_FILE_EXTENSION)) {
            builder.append(DELIMITER_SEPARATOR).append(PROPERTIES_CONFIG_START)
                    .append("\" (properties-comment style)");
        }
     ...
     * can locate the end delimiter and know whether the config content should be parsed
     * as XML module config or the legacy bare key=value format.
     *
     * @param end the end delimiter to search for.
