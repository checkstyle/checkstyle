...
public final class CodePointUtil {
    ...
    public static int[] stripLeading(int... codePoints) {
        int startIndex = 0;
        while (startIndex < codePoints.length && CommonUtil.isCodePointWhitespace(codePoints, startIndex)) {
            startIndex++;
        }
    ...
    public static int[] trim(int... codePoints) {
        int[] strippedCodePoints = stripTrailing(codePoints);
        strippedCodePoints = stripLeading(strippedCodePoints);
