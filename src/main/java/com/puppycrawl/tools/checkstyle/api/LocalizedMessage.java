...
public final class LocalizedMessage implements Serializable {
     ...
     *
     * @param newLocale the locale to use for localization.
     */
    public static void setLocale(Locale newLocale) {
        clearCache();
        if (Locale.ENGLISH.getLanguage().equals(newLocale.getLanguage())) {
            locale = Locale.ROOT;
        }
        else {
            locale = newLocale;
    ...
    private ResourceBundle getBundle(String bundleName) {
        return BUNDLE_CACHE.computeIfAbsent(bundleName, name -> {
            return ResourceBundle.getBundle(
                    name, locale, sourceClass.getClassLoader(), new Utf8Control());
        });
    }
