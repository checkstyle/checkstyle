...
public class Checker extends AutomaticBean implements MessageDispatcher, RootModule {
    ...
    public void addFileSetCheck(FileSetCheck fileSetCheck) throws CheckstyleException {
        ...
        if (childContext != null && fileSetCheck instanceof AutomaticBean) {
            ((AutomaticBean) fileSetCheck).contextualize(childContext);
        }
    ...
    public void addBeforeExecutionFileFilter(BeforeExecutionFileFilter filter)
            throws CheckstyleException {
        if (childContext != null && filter instanceof AutomaticBean) {
            ((AutomaticBean) filter).contextualize(childContext);
        }
        beforeExecutionFileFilters.addBeforeExecutionFileFilter(filter);
    }
    ...
    public final void addListener(AuditListener listener) throws CheckstyleException {
        if (childContext != null && listener instanceof AutomaticBean) {
            ((AutomaticBean) listener).contextualize(childContext);
        }
        listeners.add(listener);
    }
