...
public class SarifLogger extends AbstractAutomaticBean implements AuditListener {
    ...
    private static Map<String, String> getMessages(ModuleDetails moduleDetails) {
        ...
        try {
            ...
            // Return empty map if class or resource bundle cannot be found.
            // This can happen for third-party checks where metadata exists
            // but the check class or its resource bundle is not on the classpath.
            // Coverage Note: This defensive code path is intentionally untested as it would
            // require creating artificial metadata for non-existent classes, which is not
            // a realistic scenario in normal checkstyle operation.
        }
        return result;
    }
