...
public class SarifLogger extends AutomaticBean implements AuditListener {
    ...
    /** Content for result representing an error with source location. */
    private final String resultSourceLocation;
    ...
    public SarifLogger(
        ...
        resultSourceLocation =
            readResource("/com/puppycrawl/tools/checkstyle/sarif/ResultSourceLocation.template");
        resultFileOnly =
    ...
    public void auditFinished(AuditEvent event) {
        ...
        final StringBuilder renderedResults = new StringBuilder();
        for (int i = 0; i < results.size(); ++i) {
            if (i > 0) {
                renderedResults.append(',');
            }
            renderedResults.append('\n');
            renderedResults.append(results.get(i));
        }
        final String rendered = report
            .replace("${version}", String.valueOf(version))
            .replace("${results}", renderedResults.toString());
    ...
    public void addError(AuditEvent event) {
        results.add(resultSourceLocation
            .replace("${severityLevel}", renderSeverityLevel(event.getSeverityLevel()))
            .replace("${uri}", event.getFileName())
            .replace("${column}", Integer.toString(event.getColumn()))
            .replace("${line}", Integer.toString(event.getLine()))
            .replace("${message}", escape(event.getMessage()))
            .replace("${ruleId}", event.getViolation().getKey())
        );
    ...
    public void addException(AuditEvent event, Throwable throwable) {
        ...
        if (event.getFileName() == null) {
            results.add(resultErrorOnly
                .replace("${severityLevel}", renderSeverityLevel(event.getSeverityLevel()))
                .replace("${message}", escape(stringWriter.toString()))
            );
        }
        else {
