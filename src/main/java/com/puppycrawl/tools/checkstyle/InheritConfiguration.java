...
public final class InheritConfiguration extends DefaultConfiguration {
    ...
    static void doMergeParent(final Configuration parent, final DefaultConfiguration current)
        ...
        final Map<String, String> currentAttributeMap = current.getTheAttributeMap();
        ...
        if (parent instanceof DefaultConfiguration) {
            messageMap = ((DefaultConfiguration) parent).getTheMessages();
        } else {
        ...
        final List<Configuration> currentChildren = current.getTheChildren();
    ...
    private static Map<String, Configuration> toChildrenMap(final Configuration... children) {
        ...
        if (children.length < 1) {
            ...
            for (Configuration child : children) {
                String id = ((DefaultConfiguration) child).getTheAttributeMap().get("id");
                if (id == null) {
                    id = child.getName();
                }
                map.put(id, child);
