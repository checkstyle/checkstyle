...
public class InheritConfiguration extends DefaultConfiguration {
    ...
    static void doMergeParent(final Configuration parent, final DefaultConfiguration current) throws CheckstyleException {
        ...
        final Collection<String> addKeys = CollectionUtils.subtract(parents.keySet(), currents.keySet());
        for (final String id : addKeys) {
            current.children.add(parents.get(id));
        }
        final Collection<String> mergeKeys = CollectionUtils.intersection(parents.keySet(), currents.keySet());
