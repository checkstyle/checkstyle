...
public class XpathQueryGenerator {
    ...
    /**
     * Controls true escape characters. True escape characters need '&'
     * before but it also requires XML 1.1 until
     * https://github.com/checkstyle/checkstyle/issues/5168.
     *
     * @param chr character from parent method encode() .
     * @param string StringBuilder from parent method encode().
     */
    private static void fixEncodedSentence(char chr, StringBuilder string) {
        //
        if (Character.isISOControl(chr)) {
            string.append("#x");
            string.append(Integer.toHexString(chr));
            string.append(';');
        }
        else {
            string.append(chr);
