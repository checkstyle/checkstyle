...
public final class JavaAstVisitor extends JavaParserBaseVisitor<DetailAstImpl> {
    ...
    public DetailAstImpl visitMultiLambdaParams(JavaParser.MultiLambdaParamsContext ctx) {
        ...
        DetailAstImpl parameter = createImaginary(TokenTypes.PARAMETER_DEF, ctx.id(0).start);
        DetailAstImpl modifiers = createImaginary(TokenTypes.MODIFIERS, ctx.id(0).start);
        DetailAstImpl type = createImaginary(TokenTypes.TYPE, ctx.id(0).start);
        parameters.addChild(parameter);
        parameter.addChild(modifiers);
        parameter.addChild(type);
        parameter.addChild(create(ctx.id(0).start));

        for (int i = 0; i < ctx.COMMA().size(); i++) {
            parameters.addChild(create(ctx.COMMA(i)));
            parameter = createImaginary(TokenTypes.PARAMETER_DEF, ctx.id(i + 1).start);
            modifiers = createImaginary(TokenTypes.MODIFIERS, ctx.id(i + 1).start);
            type = createImaginary(TokenTypes.TYPE, ctx.id(i + 1).start);
            parameters.addChild(parameter);
            parameter.addChild(modifiers);
            parameter.addChild(type);
            parameter.addChild(create(ctx.id(i + 1).start));
        }
        return parameters;
    }
