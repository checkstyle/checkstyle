...
public class ParentModuleMacro extends AbstractMacro {
    ...
    private static void createParentModuleParagraph(XdocSink sink, Class<?> clss, String moduleName)
        ...
        final String parentModule = getParentModule(clss, moduleName);
    ...
    private static String getParentModule(Class<?> moduleClass, String moduleName)
        ...
        while (parentClass != null) {
            if (parentClass == AbstractCheck.class
                    || parentClass == TreeWalkerFilter.class) {
                parentModuleName = "TreeWalker";
                break;
            }
            else if (parentClass == AbstractFileSetCheck.class
                    || parentClass == Filter.class) {
                parentModuleName = "Checker";
                break;
            }
            else {
    ...
    private static Path getTemplatePath(String moduleName) throws MacroExecutionException {
        try {
            final String fileNamePattern = ".*[\\\\/]" + moduleName.toLowerCase() + "\\..*";
            ...
            final String message = String.format(Locale.ROOT, "Could not find template for %s",
                    moduleName);
