    ...
    private static void writePropertyTypeCell(Sink sink, String propertyName,
        ...
        if (SiteUtil.TOKENS.equals(propertyName)) {
            ...
            if (PropertyType.TOKEN_ARRAY.getDescription().equals(type)) {
                ...
                final String encodedType = URLEncoder.encode(type, StandardCharsets.UTF_8);

                final String encTypeF = encodedType
                        .replace("%5B", ".5B")
                        .replace("%5D", ".5D");

                final String url =
                        String.format(Locale.ROOT, URL_F, relativePathToPropertyTypes, encTypeF);
