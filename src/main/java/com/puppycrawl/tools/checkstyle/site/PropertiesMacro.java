...
public class PropertiesMacro extends AbstractMacro {
    ...
    public void execute(Sink sink, MacroRequest request) throws MacroExecutionException {
        ...
        setModuleName(CommonUtil.getFileNameWithoutExtension(moduleFile.getName()));
    ...
    /**
     * Setter for the current module name.
     *
     * @param moduleName the name of the module.
     */
    private static void setModuleName(String moduleName) {
    ...
    private static void writeTablePropertiesRows(Sink sink)
        ...
        final Set<String> allProperties = SiteUtil
    ...
    private static void writePropertyTypeCell(Sink sink, String propertyName,
        ...
        if (SiteUtil.TOKENS.equals(propertyName)) {
            ...
            final List<Integer> tokens = SiteUtil.subtractTokens(
                    check.getAcceptableJavadocTokens(), check.getRequiredJavadocTokens());
            sink.text("subset of javadoc tokens");
            writeListOfJavadocTokens(sink, tokens);
        }
        else {
    ...
    /**
     * Write a list of javadoc tokens with links to the TokenTypes.html file.
     *
     * @param sink sink to write to.
     * @param javadocTokens the list of javadoc tokens to write.
     * @throws MacroExecutionException if link to the TokenTypes.html file cannot be constructed.
     */
    private static void writeListOfJavadocTokens(Sink sink, List<Integer> javadocTokens)
            throws MacroExecutionException {
        for (int index = 0; index < javadocTokens.size(); index++) {
            final int token = javadocTokens.get(index);
            final String tokenName = JavadocUtil.getTokenName(token);
            sink.rawText(INDENT_LEVEL_16);
            if (index != 0) {
                sink.text(SiteUtil.COMMA_SPACE);
            }
    ...
    private static void writePropertyDefaultValueCell(Sink sink, String propertyName,
        ...
        if (SiteUtil.TOKENS.equals(propertyName)) {
            ...
            final AbstractJavadocCheck check = (AbstractJavadocCheck) instance;
            final List<Integer> tokens = SiteUtil.subtractTokens(
                    check.getDefaultJavadocTokens(), check.getRequiredJavadocTokens());
            writeListOfJavadocTokens(sink, tokens);
        }
        else {
