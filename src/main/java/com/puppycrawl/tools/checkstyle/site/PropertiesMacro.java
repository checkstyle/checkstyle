...
public class PropertiesMacro extends AbstractMacro {
    ...
    /** The name of the tokens property. */
    ...
    private static void writeTablePropertiesRows(Sink sink, String moduleName)
        ...
        for (String property : properties) {
            ...
            writePropertyRow(sink, moduleName, property,
                    javadocs.get(property), clss, instance, javadocs.get(moduleName));
        }
    }
    ...
    private static void writePropertyRow(Sink sink, String moduleName, String propertyName,
        ...
        writePropertyDescriptionCell(sink, propertyName, moduleName, propertyJavadoc);
        writePropertyTypeCell(sink, propertyName, moduleName, field, instance);
        writePropertyDefaultValueCell(sink, propertyName, instance, field, moduleName);
        writePropertySinceVersionCell(
                sink, propertyName, moduleJavadoc, moduleName, propertyJavadoc);
    ...
    private static void writePropertyDescriptionCell(Sink sink, String propertyName,
        ...
        final String description = SiteUtil
                .getPropertyDescription(propertyName, javadoc, moduleName);
    ...
    private static void writePropertyTypeCell(Sink sink, String propertyName,
        ...
        if (TOKENS.equals(propertyName)) {
            ...
            final List<Integer> tokens = SiteUtil.subtractTokens(check.getAcceptableTokens(),
                    check.getRequiredTokens());
            sink.text("subset of tokens");
            writeListOfTokens(sink, tokens);
        }
        else {
            final String type = SiteUtil.getType(field, propertyName, moduleName, instance);
    ...
    private static void writePropertyDefaultValueCell(Sink sink, String propertyName,
        ...
        if (TOKENS.equals(propertyName)) {
            ...
            final String defaultValue = SiteUtil.getDefaultValue(
                    propertyName, field, moduleName, instance);
            sink.rawText("<code>");
    ...
    private static void writePropertySinceVersionCell(Sink sink, String propertyName,
                                                      DetailNode moduleJavadoc,
                                                      String moduleName, DetailNode propertyJavadoc)
            throws MacroExecutionException {
        sink.rawText(INDENT_LEVEL_14);
        sink.tableCell();
        final String sinceVersion = SiteUtil.getSinceVersion(
                moduleName, moduleJavadoc, propertyName, propertyJavadoc,
