 ...
 */
@FileStatefulCheck
public class ClassAndPropertiesJavadocScraper extends AbstractJavadocCheck {
    ...
    private static Map<String, DetailNode> modulePropertySetterJavadocs = new TreeMap<>();
    ...
    public static void setModuleName(String moduleName) {
        ClassAndPropertiesJavadocScraper.moduleName = moduleName;
    }
    ...
    public void visitJavadocToken(DetailNode ast) {
        ...
        if (BlockCommentPosition.isOnMethod(blockCommentAst)) {
            // TODO: It might not be ceratin that those parents are exactly method modifier and method def
            // Better to get direct METHOD_DEF parent and CLASS_DEF
            ...
            if (CheckUtil.isSetterMethod(methodDef) && isMethodOfScrapedModule(methodDef)) {
                modulePropertySetterJavadocs.put(methodName, ast);
            }
        }
        else if (BlockCommentPosition.isOnClass(blockCommentAst)) {
            // Traverse up and get class_def
            DetailAST node = blockCommentAst.getParent();
