...
public class NotesMacro extends AbstractMacro {
    ...
    public void execute(Sink sink, MacroRequest request) throws MacroExecutionException {
        ...
        final int notesStartIndex = getNotesStartIndex(moduleJavadoc) - 1;
        final int notesEndIndex = getNotesEndIndex(moduleJavadoc, propertyNames);

        final Pattern notesLine = Pattern.compile("\r?\n\\s?" + NOTES);
        final String moduleNotes = notesLine.matcher(JavadocMetadataScraper.constructSubTreeText(
            moduleJavadoc, notesStartIndex, notesEndIndex)).replaceAll("");
        ...
        for (DetailNode node : moduleJavadoc.getChildren()) {
            if (node.getType() == JavadocTokenTypes.HTML_ELEMENT) {
                ...
                if (paragraphNode != null && JavadocMetadataScraper.isChildNodeTextMatches(
                    paragraphNode, Pattern.compile("\\s*" + NOTES + "$"))) {
