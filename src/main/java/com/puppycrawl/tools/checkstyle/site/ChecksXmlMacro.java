...
public class ChecksXmlMacro extends AbstractMacro {
    ...
    public void execute(Sink sink, MacroRequest request) throws MacroExecutionException {
        ...
        if (summarySentence == null) {
            textToWrap = "";
        } else {
            textToWrap = summarySentence.trim();
        }
        ...
        if (description != null) {
            ...
            for (int index = 0; index < description.length(); index++) {
                if (description.charAt(index) == '.') {
                    final boolean endOfSentence = index == description.length() - 1
    ...
    private static String wrapText(String text, int wrapLimit) {
        ...
        while (remaining.length() > wrapLimit) {
            ...
            if (breakIndex <= 0 || breakIndex > remaining.length()) {
                breakIndex = Math.min(wrapLimit, remaining.length());
            }

            if (breakIndex > 0 && breakIndex <= remaining.length()) {
                wrapped.append(remaining, 0, breakIndex);
                wrapped.append('\n');
            }
            remaining = remaining.substring(Math.max(0, breakIndex)).trim();
        }
