...
public class DescriptionMacro extends AbstractMacro {
    ...
    public void execute(Sink sink, MacroRequest request) throws MacroExecutionException {
        ...
        if (propertiesList.isEmpty()) {
            ...
            descriptionEndIndex = getParentIndexOf(somePropertyModuleNode.get());
        }

        String moduleDescription = constructSubTreeText(moduleJavadoc, 0, descriptionEndIndex - 1);
        String[] moduleDescriptionLinesSplit = moduleDescription.split("\n");

        sink.rawText(moduleDescriptionLinesSplit[0]);
        for (int i = 1; i < moduleDescriptionLinesSplit.length; i++) {
            final String currentLine = moduleDescriptionLinesSplit[i].trim();
            ...
            if (currentLine.startsWith("<")
                ...
                sink.rawText(INDENT_LEVEL_8 + currentLine);
            }
            else {
                sink.rawText(INDENT_LEVEL_10 + currentLine);
            }
