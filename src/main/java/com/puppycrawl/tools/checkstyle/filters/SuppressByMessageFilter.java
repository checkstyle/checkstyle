 ...
 * </p>
 * <p>
 * SuppressByMessageFilter can suppress violations for checks that have Treewalker or Checker as
 * parent module.
 * </p>
 * <ul>
 * <li>
 ...
 * a violation.
 * Type is {@code java.util.Pattern}.
 ...
 *         a = a;            // violation suppressed by filter
 *         b = 0;            // ok
 *         foo(5);           // ok
 *     }
 *
 *     public void foo(int c) {
 *         c = c;            // violation suppressed by filter
 *     }
 * }
 * </pre>
 * <p>
 ...
 *     public InputSuppressByMessageFilter(int a) {
 *         a = a;            // violation suppressed by filter
 *         b = 0;            // violation suppressed by filter
 *         foo(5);           // violation
 *     }
 *
 *     public void foo(int c) {
 ...
 * @since 8.43
 */
public class SuppressByMessageFilter
        extends AutomaticBean
        implements Filter {

    /** Specify check pattern to suppress. */
    private Pattern checkFormat = null;

    /** Specify message pattern to suppress. */
    private Pattern messageFormat = null;

    /**
     * Setter to define RegExp for matching against the check format.
     *
    ...
    /**
     * Setter to define RegExp for matching against the violation message format.
     *
    ...
    private boolean matchCheckFormat(AuditEvent event) {
        if (checkFormat != null && !checkFormat.toString().equals("")) {
            return checkFormat.matcher(event.getSourceName()).find();
        }
        return false;
    }
    ...
    private boolean matchMessageFormat(AuditEvent event) {
        if (messageFormat != null && !messageFormat.toString().equals("")) {
            return messageFormat.matcher(event.getMessage()).find();
        }
        return false;
    }
