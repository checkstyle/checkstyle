 ...
 *
 * In order to make 'TEXT_BLOCK_TEMPLATE_MID' token work for these lexemes, we
 * would need to add more semantic predicates to both tokens,
 * which would be more complex than just reusing the 'STRING_TEMPLATE_MID' token.
 * Additionally, we would complicate lexer decisions and likely hurt performance.
 */
