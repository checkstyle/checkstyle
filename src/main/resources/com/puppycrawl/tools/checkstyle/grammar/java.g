...

{
    ...
    /**
    * This boolean value tracks if we are in a switch rule. Along with the
    * IDENT to id rule at the end of the parser, this value helps us
    * to know if "yield" is an IDENT, method call, class, field, etc. or
    * if it is a java 13+ yield statement.
    */
    private boolean inSwitchRuleOrStatement = false;
}
...
traditionalStatement
    ...
    :    compoundStatement
        ...
        |  {this.inSwitchRuleOrStatement}? yieldStatement
...
switchBlock
    :   (LCURLY ( switchRule )+ RCURLY)=>(LCURLY ( switchRule )+ RCURLY)
        |
        (LCURLY ( casesGroup )* RCURLY)=>(LCURLY ( casesGroup )* RCURLY)
    ;
...
caseSList
    :
        (
            ...
            {this.inSwitchRuleOrStatement=true;}// Need to do this here to make sure that we don't
                                                // allow yield statements to be matched otherwise.
                statement
        )+
        {this.inSwitchRuleOrStatement=false;}
...
switchRule
    :   (   {this.inSwitchRuleOrStatement=true;}: // Need to do this here to make sure that we don't
        ...
        {this.inSwitchRuleOrStatement=false;}
        {## = #(#[CASE_GROUP, "CASE_GROUP"], se, sb, st);}
    ;
...
yieldKey:   {!this.inSwitchRuleOrStatement}? "yield" {#yieldKey.setType(IDENT);};
