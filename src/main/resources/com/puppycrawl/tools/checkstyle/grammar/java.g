...

{
    ...
    */
    private boolean inSwitchRule = false;
}
...
traditionalStatement
    ...
    :    compoundStatement
        ...
        // Yield statement, must be in a switchRule to use "yield" after Java 13.
        |  {this.inSwitchRule}? yieldStatement
...
switchRule
    :   (   {this.inSwitchRule=true;}: // Need to do this here to make sure that we don't allow
                                       // yield statements to be matched otherwise.
                (switchLabeledExpr)=>     se:switchLabeledExpr
        |       (switchLabeledBlk)=>      sb:switchLabeledBlk
        |       (switchLabeledThrow)=>    st:switchLabeledThrow
        )
        {this.inSwitchRule=false;}
