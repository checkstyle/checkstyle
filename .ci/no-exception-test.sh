...
openjdk17-with-checks-nonjavadoc-error)
  ...
  cd .ci-temp/contribution/checkstyle-tester
  cp ../../../config/openjdk-projects-to-test-on.config openjdk-projects-to-test-on.config
  sed -i '/  <!-- Filters -->/r ../../../config/openjdk17-excluded.files' checks-nonjavadoc-error.xml
  export MAVEN_OPTS="-Xmx2048m"
...
no-exception-lucene-and-others-javadoc)
  ...
  checkout_from https://github.com/checkstyle/contribution
  cp config/projects-for-no-exception-javadoc.config .ci-temp/contribution/checkstyle-tester
  cd .ci-temp/contribution/checkstyle-tester
