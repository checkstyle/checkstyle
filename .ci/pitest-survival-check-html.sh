...
pitest-javadoc)
  ...
  "HtmlTag.java.html:<td class='covered'><pre><span  class='survived'>        final int endOfText = Math.min(startOfText + MAX_TEXT_LEN, text.length());AbstractJavadocCheck.java.html:<td class='covered'><pre><span  class='survived'>                            JavadocUtil.getTokenName(javadocTokenId), getClass().getName());</span></pre></td></tr></span></pre></td></tr>"
