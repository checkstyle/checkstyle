...
install-custom-jdk)
  if [[ -n "${CUSTOM_JDK_VERSION}" ]]; then
    echo "Installing ${CUSTOM_JDK_VERSION}...";
    ...
    echo "deb [signed-by=/usr/share/keyrings/adoptium.asc] \
      https://packages.adoptium.net/artifactory/deb $(lsb_release --codename) main" |
      sudo tee /etc/apt/sources.list.d/adoptium.list
    sudo apt-get update
    sudo apt-get install "${CUSTOM_JDK_VERSION}"
  fi
  ;;

remove-custom-jdk)
  if [[ -n "${CUSTOM_JDK_VERSION}" ]]; then
    sudo apt-get remove "${CUSTOM_JDK_VERSION}"
  fi
  ;;
