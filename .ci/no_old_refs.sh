...
API_ISSUE_LINK_PREFIX="https://api.github.com/repos"
GITHUB_HOST="https://github.com"

# collect issues where full link is used
grep -Pohr "(after|[Tt]il[l]?) $GITHUB_HOST/[a-z]+/[a-z]+/issues/\d{1,5}" . \
...
for issue in $(sort -u $MENTIONED_ISSUES); do
  STATE=$(curl -s -H "Authorization: token $READ_ONLY_TOKEN" "$API_ISSUE_LINK_PREFIX/$issue" \
   | jq '.state' | xargs)
