...
no-exception-struts)
  ...
  groovy ./diff.groovy --listOfProjects projects-for-wercker.properties \
      --config checks-nonjavadoc-error.xml --checkstyleVersion ${CS_POM_VERSION} --mode single \
      --allowExcludes
  cd ../../
  removeFolderWithProtectedFiles contribution
  ;;
...
no-exception-checkstyle-sevntu-javadoc)
  ...
  sed -i'' 's/#sevntu-checkstyle/sevntu-checkstyle/' projects-for-wercker.properties
  groovy ./diff.groovy --listOfProjects projects-for-wercker.properties \
      --config checks-only-javadoc-error.xml --checkstyleVersion ${CS_POM_VERSION} --mode single \
      --allowExcludes
  cd ../../
  removeFolderWithProtectedFiles contribution
  ;;
...
no-warning-imports-guava)
  ...
  groovy ./diff.groovy --listOfProjects $PROJECTS --config $CONFIG \
      --checkstyleVersion ${CS_POM_VERSION} --mode single --allowExcludes
