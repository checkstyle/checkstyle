...
no-exception-struts)
  ...
  groovy ./diff.groovy --listOfProjects projects-for-wercker.properties \
      --patchConfig checks-nonjavadoc-error.xml --checkstyleVersion ${CS_POM_VERSION}  \
      --allowExcludes --mode single
  cd ../../
  removeFolderWithProtectedFiles contribution
  ;;
...
no-exception-checkstyle-sevntu-javadoc)
  ...
  sed -i'' 's/#sevntu-checkstyle/sevntu-checkstyle/' projects-for-wercker.properties
  groovy ./diff.groovy --listOfProjects projects-for-wercker.properties \
      --patchConfig checks-only-javadoc-error.xml --checkstyleVersion ${CS_POM_VERSION}  \
      --allowExcludes --mode single
  cd ../../
  removeFolderWithProtectedFiles contribution
  ;;
