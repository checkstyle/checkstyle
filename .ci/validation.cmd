...
if "%OPTION%" == "sevntu" (
  call mvn -e --no-transfer-progress package verify -DskipTests -DskipITs -Dpmd.skip=true^
...
if "%OPTION%" ==  "verify_without_checkstyle" (
  call mvn -e --no-transfer-progress package verify -Dcheckstyle.ant.skip=true -Dcheckstyle.skip=true^
    || goto :ERROR
  goto :END_CASE
)

if "%OPTION%" ==  "site_without_verify" (
  call mvn -e --no-transfer-progress package -Pno-validations site^
    || goto :ERROR
  goto :END_CASE
)
