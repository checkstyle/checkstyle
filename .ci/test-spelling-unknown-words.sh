...
set -e
contrib_repo=https://github.com/jsoref/checkstyle-contribution.git
temp=`pwd`/.ci-temp
mkdir -p $temp
...
if [ ! -e $dict ]; then
  ...
  curl -s https://rpmfind.net/linux/fedora/linux/development/rawhide/Everything/aarch64/os/Packages/w/words-3.0-28.fc28.noarch.rpm > $words_rpm
...
find . \
  ...
  $word_splitter 2>/dev/null > $run_output
