...
SUPPRESSION_LIST=(
  config/google-java-format/suppressions/suppress-diff-lt-100.txt
  config/google-java-format/suppressions/suppress-diff-gt-100-lt-500.txt
...
MISMATCH_CONTENT=$(cat "${EXCLUDED_LIST[@]}" \
  | while read -r path; do
    ...
    if is_suppressed "Input${name}.java";then
      continue
    fi
    if (( diff > 500 )); then
      echo "Difference in Input${name}.java and InputFormatted${name}.java is more than 500 bytes ${diff}"
    elif (( diff > 100 )); then
      echo "Difference in Input${name}.java and InputFormatted${name}.java is more than 100 bytes ${diff}"
    elif (( diff > 50 )); then
      echo "Difference in Input${name}.java and InputFormatted${name}.java is more than 50 bytes ${diff}"
    fi
  done)
