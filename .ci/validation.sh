...
EOF
  cat > .ci-temp/suppressions.xml <<'EOF'
<?xml version="1.0"?>
...
EOF
  ...
  if [ "$(grep -c '\[ERROR\]' .ci-temp/output.log)" != "2" ] \
      || ! grep -q "Zahl '12345'" .ci-temp/output.log; then
  ...
  rm .ci-temp/Input.java .ci-temp/suppressions.xml .ci-temp/config.xml \
