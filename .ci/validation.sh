...
no-error-hibernate-search)
  ...
  mvn -e --no-transfer-progress clean install -DskipTests=true -Dmaven.compiler.failOnWarning=false \
     -Dcheckstyle.skip=true -Dforbiddenapis.skip=true \
     -Dversion.com.puppycrawl.tools.checkstyle=${CS_POM_VERSION}
  mvn -e --no-transfer-progress checkstyle:check -Dversion.com.puppycrawl.tools.checkstyle=${CS_POM_VERSION}
  cd ../
