...
while IFS= read -r CURRENT_XDOC_PATH; do
  ...
  SUBSECTION_ID=$(
    echo "$DIFF_CONTEXT" | grep -Eo '<a id="[^"]+"' \
    | head -1 | sed 's/<a id="\([^"]*\)"/\1/'
  )
  ...
  # Fallback: If no anchor id is found, check for the section name attribute.
  if [[ -z "$SUBSECTION_ID" ]]; then
    echo "No anchor id found; checking for section name attribute..."
    SECTION_NAME=$(
      echo "$DIFF_CONTEXT" | grep -Ei 'name="[^"]+"' \
      | head -1 | sed 's/name="\([^"]*\)"/\1/'
    )
    ...
    if [[ -n "$SECTION_NAME" ]]; then
      if echo "$SECTION_NAME" | grep -q '[^A-Za-z0-9_-]'; then
        ...
        SUBSECTION_ID=$(python3 -c \
          "import urllib.parse; print(urllib.parse.quote('$SECTION_NAME').replace('%','.'))")
      else
