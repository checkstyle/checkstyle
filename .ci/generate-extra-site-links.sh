...
while IFS= read -r CURRENT_XDOC_PATH; do
  ...
  # Primary: Extract the section's name attribute using grep -Po.
  SECTION_NAME=$(echo "$DIFF_CONTEXT" | grep -Po '(?<=name=")[^"]+(?=")' | head -1)
  ...
  if [[ -n "$SECTION_NAME" ]]; then
    if echo "$SECTION_NAME" | grep -q '[^A-Za-z0-9_-]'; then
      ...
      # Remove newline characters before encoding to avoid unwanted .0A
      SUBSECTION_ID=$(echo "$SECTION_NAME" | tr -d '\n\r' | jq -sRr @uri | sed 's/%/./g')
    else
    ...
    # Fallback: If no section name is found, try for an anchor id.
    echo "No section name found; checking for anchor id..."
    SUBSECTION_ID=$(
      echo "$DIFF_CONTEXT" | grep -Eo 'id="[^"]+"' \
      | head -1 | sed 's/id="\([^"]*\)"/\1/'
    )
