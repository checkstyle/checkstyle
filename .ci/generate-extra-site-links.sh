...
while IFS= read -r CURRENT_XDOC_PATH; do
  ...
  # Try to extract anchor id from an <a> tag.
  SUBSECTION_ID=$(
    echo "$DIFF_CONTEXT" | grep -Eo '<a id="[^"]+"' | head -1 | sed 's/<a id="\([^"]*\)"/\1/'
  )
  if [[ -n "$SUBSECTION_ID" ]]; then
    echo "Found anchor id in diff context: $SUBSECTION_ID"
  fi

  # Fallback: If no anchor id is found, check for the section's name attribute.
  if [[ -z "$SUBSECTION_ID" ]]; then
    echo "No anchor id found; checking for section name attribute..."
    SECTION_NAME=$(
      echo "$DIFF_CONTEXT" | grep -Ei 'name="[^"]+"' | head -1 | sed 's/name="\([^"]*\)"/\1/'
    )
...
print(urllib.parse.quote(sys.argv[1]).replace('%','.'))" "$SECTION_NAME")
      ...
      echo "Warning: No anchor id or section name found in $CURRENT_XDOC_PATH"
    fi
  fi

  CURRENT_XDOC_NAME=$(
    echo "$CURRENT_XDOC_PATH" | sed 's/src\/site\/xdoc\/\(.*\)\.xml/\1/' | sed 's/.vm//'
  )
