...
while IFS= read -r CURRENT_XDOC_PATH; do
  ...
  # Attempt 1: Find all id attributes in diff context on either
  # <subsection> or <section> tags.
  ANCHOR_LIST=$(
    echo "$DIFF_CONTEXT" | grep -Ei '<(subsection|section)' | \
    grep -Eo 'id="[^"]+"' | sed 's/id="\([^"]*\)"/\1/' | \
    sed 's/^[[:space:]]*//;s/[[:space:]]*$//'
  )
  if [[ -n "$ANCHOR_LIST" ]]; then
    echo "Found ids: $ANCHOR_LIST"
    for anchor in $ANCHOR_LIST; do
      echo "$AWS_FOLDER_LINK/$(echo "$CURRENT_XDOC_PATH" | sed 's/src\/site\/xdoc\/\(.*\)\.xml/\1/' \
        | sed 's/.vm//').html#$anchor" >> .ci-temp/message
      echo "Added link: $AWS_FOLDER_LINK/$(echo "$CURRENT_XDOC_PATH" | sed 's/src\/site\/xdoc\/\(.*\)\.xml/\1/' \
        | sed 's/.vm//').html#$anchor"
    done
  else
    # Fallback: If no id is found in diff context, check for section's name.
    echo "No id found; checking for section name attribute..."
    SECTION_NAME=$(
      echo "$DIFF_CONTEXT" | grep -Ei 'name="[^"]+"' | head -1 | \
      sed 's/name="\([^"]*\)"/\1/'
    )
