...
while IFS= read -r CURRENT_XDOC_PATH; do
  ...
  # Attempt 1: Check for an id on either a <subsection> or <section>
  SUBSECTION_ID=$(
  ...
  if [[ -n "$SUBSECTION_ID" ]]; then
    echo "Found id in diff context: $SUBSECTION_ID"
  fi

  # Fallback: If no id is found, check for the section's name attribute.
