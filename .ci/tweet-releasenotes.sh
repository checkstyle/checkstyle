...
check_env_vars () {
  for variable; do
    : "${!variable:?$variable must not be empty}"
  done
}

if ! check_env_vars "$TWITTER_CONSUMER_KEY" "$TWITTER_CONSUMER_SECRET" "$TWITTER_ACCESS_TOKEN" \
   "$TWITTER_ACCESS_TOKEN_SECRET"; then
    exit 1
fi
...
java -jar contribution/releasenotes-builder/target/releasenotes-builder-1.0-all.jar \
     ...
     -generateTwit \
     -twitterTemplate $BUILDER_TEMPLATE_DIR/twitter.template \
     -publishTwit \
